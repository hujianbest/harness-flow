...
按 Artifact Layout（详见 `docs/df-workflow-shared-conventions.md`），新 work item 用 `templates/df-work-item-readme-template.md` / `templates/df-progress-template.md` / `templates/df-traceability-template.md` 创建骨架；已存在则核对 `progress.md` canonical 字段完整性。缺 Work Item ID 或所属组件 → 阻塞，回需求负责人。
...
| `docs/df-workflow-shared-conventions.md` | 工件路径、canonical 字段、handoff 字段、requirement.md 必含项 |
