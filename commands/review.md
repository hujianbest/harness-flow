---
description: Request a HarnessFlow review. The router dispatches hf-reviewer to the correct hf-*-review skill (discovery / spec / design / ui / tasks / test / code / traceability) based on artifact context.
---
...
3. The router consults `skills/hf-workflow-router/references/review-dispatch-protocol.md` and dispatches `agents/hf-reviewer.md` to one of:
...
4. `hf-reviewer` runs as an **independent author/reviewer separation** role; do not let it edit the artifact under review.
