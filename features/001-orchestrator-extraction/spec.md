...
| HYP-004 | FSM 转移表 + reviewer dispatch 协议从分布在多个 references 文件，集中到 orchestrator agent 后仍能 progressive disclosure（不撑爆 token 预算） | Feasibility | 每个 session 都付高额 token 成本；需要继续拆分 | 中-高 | design 阶段量化：orchestrator 主文件 ≤ 300 行（**tentative engineering aim**），references 按需加载；落到 `hf-design` 的 trade-off 评估 | 否 |
...
- **需求陈述**: 系统必须在仓库根 `agents/` 目录下提供 `hf-orchestrator.md` 文件，作为 always-on agent persona。该文件包含编排者身份声明、operating loop（步骤 1–10）、hard stops 列表、FSM 转移表入口或引用、reviewer dispatch 协议入口或引用、skill catalog（24 个 hf-* + 1 个 entry shell 的 description-line 索引）、verification 自检清单。
- **验收标准**:
  ...
  - **Given** orchestrator 文件已落盘，**When** 检查文件正文，**Then** 包含至少一段"editor 自报身份"（标识性内容），用于 § 3 smoke test 的判定。
  - **Given** 一个新 HF session 启动并加载 orchestrator 后，**When** 它收到一条 user message，**Then** 它按 operating loop 步骤 1（识别 entry vs runtime recovery）→ 步骤 2（读 minimal evidence）→ 步骤 6（pick canonical next skill）的顺序响应（与现状 `using-hf-workflow` § 1 + `hf-workflow-router` § 2 / § 7 语义等价）。
...
- **验收标准**:
  - **Given** Cursor 加载本仓库为 workspace，**When** 用户新建 chat session，**Then** `.cursor/rules/harness-flow.mdc` 内容已自动注入 system context 且其 body 引用 `agents/hf-orchestrator.md`（不再仅引用 `skills/using-hf-workflow/`）。
  - **Given** Claude Code 在本仓库中被启动，**When** 用户发起新对话，**Then** `CLAUDE.md` 已自动注入并包含"加载 `agents/hf-orchestrator.md`"的指令；或 `.claude-plugin/plugin.json` 注册项使 orchestrator 自动激活（取一即可，schema 不允许时降级为前者）。
  - **Given** OpenCode 加载本仓库，**When** 用户启动新 session，**Then** `AGENTS.md` 已自动注入并包含 orchestrator stub 段。
  - **Given** 任一上述宿主中新 session 已启动，**When** 用户输入"who are you / 你是什么 agent"类问题，**Then** agent 响应里能引用 `agents/hf-orchestrator.md` 中的标识性内容（用于 § 3 smoke test 通过判定）。
...
- **验收标准**:
  ...
  - **Given** 回归脚本（`features/001-orchestrator-extraction/verification/regression-diff.sh` 或等价）执行完毕，**Then** exit code = 0 且 stdout 含 "PASS" 行。
...
- **验收标准**:
  - **Given** v0.6.0 commit 后的仓库快照，**When** 检查 `README.md` Scope Note，**Then** 含一段说明 v0.6.0 引入 `agents/hf-orchestrator.md`、保留 24 个 hf-* skill 数量不变、保留 7 条 slash 命令不变、`hf-release` 行为不变。
  - **Given** v0.6.0 commit 后的 setup docs，**When** 检查任一 setup doc，**Then** "如何启用 HF" 段已从"加载 entry shell + router"改为"orchestrator agent 自动加载，无需手动操作"。
...
- **Quality Attribute Scenario**:
  ...
  - **Response Measure**: 从 session 创建到 orchestrator persona 加载完成的时间 ≤ 该宿主下"加载现状 entry shell + router"的时间 + 20%（即新方案不显著拖慢加载）
- **Acceptance**:
  - **Given** Cursor / Claude Code / OpenCode 任一宿主在本仓库新建 session，**When** session 启动，**Then** 第一轮用户响应中 agent 已具备 orchestrator persona 行为（identity check 通过），且使用者主观感知不到加载耗时显著增加
  - **Given** smoke test 记录，**When** 评估 3 宿主的加载体验，**Then** 不存在任一宿主的"加载明显变慢"主观投诉（写入 `features/001-orchestrator-extraction/verification/smoke-3-clients.md`）
