...
- 脚本的依赖闭包：bash 4+（macOS 自带 3.x 也要兼容）+ POSIX coreutils；不要求宿主机器装 Node、Python、PowerShell、jq 等额外工具。
...
| Leading Indicator 1: install 后 setup 文档的 verify 步骤通过率 | install 完成后，立即按 `docs/opencode-setup.md §2` / `docs/cursor-setup.md §3` 的 verify 文本检查（`.opencode/skills/<name>/SKILL.md` / `.cursor/rules/harness-flow.mdc` 存在且能读到 `using-hf-workflow` + `hf-workflow-router`），100% 通过 | 端到端测试脚本里直接 ls / grep |
...
- 提供 `install.sh`（bash）：`--target cursor|opencode|both` × `--topology copy|symlink` × `--dry-run` × `--verbose` × `--force`
- 提供 `uninstall.sh`（bash）：基于 install 阶段写的 manifest，反向清理；支持 `--dry-run`
...
- 需求陈述: 在 `--topology symlink` 下，系统必须将宿主目标路径创建为指向 HF 仓库内对应路径的符号链接；在 `--topology copy`（默认）下，系统必须执行 `cp -R` 复制。
- 验收标准:
  ...
  - Given `--target opencode --topology copy --host /tmp/host`，When 执行脚本，Then `/tmp/host/.opencode/skills` 是普通目录，且 `find /tmp/host/.opencode/skills -name SKILL.md | wc -l` ≥ 24（HF 当前 24 个 hf-* + 1 个 using-hf-workflow ≥ 25；此处保守取 24 兼容未来增删）。

### FR-003 写入 `.harnessflow-install-manifest.json` 记录所有放入文件

- 优先级: Must
- 来源: HYP-002；场景 D 卸载需要
- 需求陈述: install 完成后，系统必须在宿主仓库根目录写入 `.harnessflow-install-manifest.json`，记录本次 install 的 target、topology、HF 仓库 git commit、放入或链接的所有文件/目录路径列表。
- 验收标准:
  - Given install 已成功完成，When 读取 `.harnessflow-install-manifest.json`，Then 文件存在，是合法 JSON，且包含字段 `version` / `installed_at` / `hf_commit` / `target` / `topology` / `entries[]`（entries 数组每条至少含 `path` 与 `kind`，`kind ∈ {file, dir, symlink}`）。
...
- Response: 脚本必须捕获错误，反向回滚已落盘的部分文件（基于 in-memory 已记录条目），不留下"半装"状态污染宿主仓库
...
- Response: 脚本能正常运行
- Response Measure: 在 macOS bash 3.2 + Linux bash 5.x 两个环境下，6 个 e2e scenario 至少有 1 个跑通（design 决定 manifest 不依赖 jq 的写法）；脚本顶部 shebang `#!/usr/bin/env bash`；`set -euo pipefail`；不调用 jq / python / node

Acceptance:
- Given 仅有 bash + POSIX 工具的环境；When 执行 install.sh；Then 退出码 0；脚本源码 grep 不到 `jq`、`python`、`node`、`npm` 字样（除注释 / 文档外）。
