...
### `vendor_skills()`（OpenCode target）

```bash
vendor_skills_opencode() {
    local dest="$HOST/.opencode/skills"
    op MKDIR "$HOST/.opencode"
    INSTALLED+=("dir:$HOST/.opencode")
    if [ "$TOPOLOGY" = "symlink" ]; then
        op LN "$HF_REPO/skills" "$dest"
        ENTRIES+=("symlink:.opencode/skills")
        INSTALLED+=("symlink:$dest")
    else
        op CP "$HF_REPO/skills" "$dest"
        ENTRIES+=("dir:.opencode/skills")
        INSTALLED+=("dir:$dest")
    fi
}
```

### `vendor_skills_cursor()`（Cursor target，按 ADR-007 D4）

```bash
vendor_cursor() {
    op MKDIR "$HOST/.cursor/rules"
    INSTALLED+=("dir:$HOST/.cursor")
    INSTALLED+=("dir:$HOST/.cursor/rules")
    if [ "$TOPOLOGY" = "symlink" ]; then
        op LN "$HF_REPO/skills" "$HOST/.cursor/harness-flow-skills"
        op LN "$HF_REPO/.cursor/rules/harness-flow.mdc" "$HOST/.cursor/rules/harness-flow.mdc"
        ENTRIES+=("symlink:.cursor/harness-flow-skills" "symlink:.cursor/rules/harness-flow.mdc")
        INSTALLED+=("symlink:$HOST/.cursor/harness-flow-skills" "symlink:$HOST/.cursor/rules/harness-flow.mdc")
    else
        op CP "$HF_REPO/skills" "$HOST/.cursor/harness-flow-skills"
        op CP "$HF_REPO/.cursor/rules/harness-flow.mdc" "$HOST/.cursor/rules/harness-flow.mdc"
        ENTRIES+=("dir:.cursor/harness-flow-skills" "file:.cursor/rules/harness-flow.mdc")
        INSTALLED+=("dir:$HOST/.cursor/harness-flow-skills" "file:$HOST/.cursor/rules/harness-flow.mdc")
    fi
}
```
...
rollback() {
    ...
    # reverse iteration of INSTALLED, rm each entry
    local i
    for ((i=${#INSTALLED[@]}-1; i>=0; i--)); do
        ...
        case "$kind" in
            ...
            dir)          rmdir "$path" 2>/dev/null || true ;;  # only if empty
        esac
    done
...
注：bash 3.2 不支持 `for ((..))` 是不正确的——bash 3.x 已支持算术 for 循环（POSIX 2008 没有，但 bash 自身 3.0 起即可）。
...
```json
{
  ...
  "entries": [
    ...
    {"kind": "file", "path": ".cursor/rules/harness-flow.mdc"},
    {"kind": "symlink", "path": ".cursor/harness-flow-skills"},
...
| # | target | topology | 验证项 |
|---|---|---|---|
| 1 | opencode | copy | `find <host>/.opencode/skills -mindepth 2 -maxdepth 2 -name SKILL.md \| wc -l` ≥ 24 + manifest entries[] 含 `dir:.opencode/skills` + readme 存在 |
| 2 | opencode | symlink | `readlink <host>/.opencode/skills` 解析回 `<HF-repo>/skills` + manifest entries[] 含 `symlink:.opencode/skills` |
| 3 | cursor | copy | `find <host>/.cursor/harness-flow-skills -mindepth 2 -maxdepth 2 -name SKILL.md \| wc -l` ≥ 24 + `<host>/.cursor/rules/harness-flow.mdc` 存在 + manifest entries[] 含两条 |
...
- **#10 NFR-004 grep**：`grep -E '\b(jq|python|node|npm)\b' install.sh uninstall.sh` 排除注释 / 文档段后输出为空
...
| 失败 | 影响 | 缓解 |
|---|---|---|
| `cp` 中途失败（磁盘满 / 权限错） | 半装状态污染宿主仓库 | §11 `trap rollback ERR` + INSTALLED 反向清理（NFR-002）|
