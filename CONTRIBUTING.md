...
This document is intentionally short. HarnessFlow is currently a **single-maintainer** pre-release (`v0.4.0`). The goal of this guide is to make small, well-scoped contributions easy to land and to keep larger changes from blocking on review forever.

## Before You Start

1. **Read [`README.md`](README.md)** (or [`README.zh-CN.md`](README.zh-CN.md)) end-to-end. Pay particular attention to the **Scope Note** at the top — `v0.4.0` is deliberately narrow (Claude Code + OpenCode + Cursor only; main chain ends at `hf-finalize`; v0.4.0 added `hf-release` as a release-tier standalone skill (engineer-level release / version cut), but the other 5 ops/release skills (deployment / monitoring / rollback / etc.) remain out of scope; no new clients, no personas). Many "missing feature" reports are actually documented scope choices.
2. **Read the relevant ADR**. Release scope decisions are locked in [`docs/decisions/ADR-004-hf-release-skill.md`](docs/decisions/ADR-004-hf-release-skill.md) (hf-release standalone skill + /release command; hf-shipping-and-launch stays v0.5+), [`docs/decisions/ADR-003-release-scope-v0.3.0.md`](docs/decisions/ADR-003-release-scope-v0.3.0.md) (Cursor-only client expansion; no new skills; no personas), [`docs/decisions/ADR-002-release-scope-v0.2.0.md`](docs/decisions/ADR-002-release-scope-v0.2.0.md) (含 D11 校准撤回 D2/D3/D4/D8 的说明) and [`docs/decisions/ADR-001-release-scope-v0.1.0.md`](docs/decisions/ADR-001-release-scope-v0.1.0.md). If your contribution conflicts with an ADR decision, please open an issue first to discuss whether the ADR should be revisited.
...
- Setup instructions for clients that v0.4.0 doesn't yet support (Gemini CLI / Windsurf / GitHub Copilot / Kiro), **clearly marked** as additions, not as v0.4.0 commitments.
...
- New `hf-*` skills, especially deployment / ops / monitoring skills (out of scope per ADR-001 D1 + ADR-002 D1 + ADR-003 D2 + ADR-004 D2; remaining 5 ops/release skills tracked for v0.5+; v0.4.0 already added `hf-release` for the engineer-level version-cut slice).
- Major restructuring of an existing `SKILL.md` (per ADR-001 D11 + ADR-003 D2 + ADR-004 the 24 skills are deliberately maintained as-is; only `Common Rationalizations` (required) and `和其他 Skill 的区别` (forbidden) are hard rules per ADR-002 D9 / D10).
- Anything that touches `docs/principles/` (ADR-001 D11 + ADR-003 D9 + ADR-004 mark these as design references; treat them as authored documents, not editable specs).
- New `agents/` personas (`hf-staff-reviewer` / `hf-qa-engineer` / `hf-security-auditor`) — ADR-002 D11 + ADR-003 D3 + ADR-004 keep these deferred to v0.5+.
...
- **No CI yet.** v0.4.0 still ships without GitHub Actions. PR authors should run `cd examples/writeonce && npm test && npx tsc --noEmit` locally and paste the output in the PR. CI remains a v0.5+ work item.
...
- **`SKILL.md` lint is advisory.** `scripts/audit-skill-anatomy.py` exists since v0.2.0 (ADR-002 D5) but is **advisory** (does not block PR merge); ADR-003 D8 / ADR-004 keep this stance. Please be careful when editing `SKILL.md` files and follow the implicit structure used by neighbouring skills.
- **No real-environment install smoke as a release hard gate.** ADR-002 D11 revoked the proposed hard gate; ADR-003 D7 / ADR-004 keep this stance for v0.4.0 (3 clients still small enough that maintainer self-verification is sufficient). The first real Cursor / Claude Code / OpenCode install verification on a clean user environment is community-driven; please open an issue if anything install-related breaks.
- **No formal release cadence.** v0.4.0 is a pre-release; patch releases (`v0.4.x`) ship when meaningful fixes accumulate, not on a fixed schedule. Note: maintainer-side release cuts now use the new `hf-release` skill itself (dogfood); see `skills/hf-release/SKILL.md`.
...
- `feature_request.md` — for new behavior. Please check the Scope Note and ADR-004 (then ADR-003, ADR-002, ADR-001) first; many "missing features" are deliberate scope choices for v0.4.0.
