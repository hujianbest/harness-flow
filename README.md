...
>
> - **Version**: `v0.6.0`, marked as a **pre-release** on GitHub Releases. v0.6.0 is a **minor release** that introduces **`agents/hf-orchestrator.md`** as the always-on agent persona, replacing `using-hf-workflow` + `hf-workflow-router` (now deprecated aliases through v0.6.x). This locks the **HF three-layer architecture invariant** per [`docs/decisions/ADR-007-orchestrator-extraction-and-skill-decoupling.md`](docs/decisions/ADR-007-orchestrator-extraction-and-skill-decoupling.md) D1: **Doer Skills** (12) / **Reviewer-Gate Skills** (11) / **Orchestrator Agent** (1). The orchestrator is auto-loaded by every supported host (Cursor `.cursor/rules/harness-flow.mdc` / Claude Code `CLAUDE.md` + `.claude-plugin/plugin.json` agent registration / OpenCode `AGENTS.md`). v0.6.0 does **not** grow the leaf skill set (still **24** `hf-*`), does **not** add slash commands (still **7**), does **not** change closeout pack schema / reviewer return verdict vocabulary / `hf-release` standalone behavior, and does **not** modify any leaf skill content. ADR-007 D3 lays out a 6-step landing path; **only Step 1 (build orchestrator + 3-host stubs + deprecate-alias old skills) is in v0.6.0 scope**. Steps 2–6 (leaf skill `Next Action` field downgrade / Hard Gate stratification / cross-skill reference removal / physical deletion of old skills) are deferred to future increments per ADR-007 D3.
> - **v0.5.x main (carried over)**: v0.5.0 added a closeout HTML companion report to `hf-finalize` (every closeout now produces `features/<active>/closeout.html` alongside `closeout.md`), with a stdlib-only `skills/hf-finalize/scripts/render-closeout-html.py` renderer (workflow timeline rail + tests + coverage rings + searchable evidence matrix; WCAG 2.2 AA, dark/light, printable, anti-AI-slop disciplined per `skills/hf-ui-design/references/anti-slop-checklist.md`). v0.5.1 was a vendoring patch that moved the renderer from repo-root `scripts/` to `skills/hf-finalize/scripts/`. v0.4.0's `hf-release` (release-tier standalone skill) is unchanged in v0.6.0.
> - **Officially supported clients**: **Claude Code**, **OpenCode**, and **Cursor** (unchanged from v0.3.0). The 4 remaining client expansions (Gemini CLI / Windsurf / GitHub Copilot / Kiro) stay deferred to v0.7+.
> - **Main chain still ends at `hf-finalize`** — that is **engineering-level closeout** for a single feature/workflow. v0.6.0 only extracts orchestration into a separate agent persona; it does not extend the main chain. `hf-release` is a release-tier standalone skill that **aggregates** multiple `workflow-closeout` features into a vX.Y.Z release; it is **not** part of the main chain. Release pipelines, deployment, observability, incident response, security hardening, performance gating, debugging-and-error-recovery, and deprecation-and-migration are still **v0.7+ planned `hf-shipping-and-launch` / etc. (not yet implemented)**.
> - This narrow surface is a deliberate choice (ADR-001 D1 / ADR-002 D1 / ADR-003 D2 / ADR-004 D2 / ADR-005 D7 / **ADR-007 D6 / D7** — "P-Honest, narrow but hard"). HarnessFlow refuses to disguise "code merged / engineering closeout" as "shipped to production"; the v0.5.0 closeout HTML is a **visual rendering** of the closeout pack, not a deployment record; the v0.6.0 orchestrator is a **structural refactor** that strengthens skill standalone-usability without expanding scope.
>
> See `docs/decisions/ADR-007-orchestrator-extraction-and-skill-decoupling.md` for the v0.6.0 minor scope; `docs/decisions/ADR-006-skill-anatomy-v2-and-vendoring-fix.md` for v0.5.1; `docs/decisions/ADR-005-release-scope-v0.5.0.md` for v0.5.0; `docs/decisions/ADR-004-hf-release-skill.md` (hf-release standalone skill), `docs/decisions/ADR-003-release-scope-v0.3.0.md` (Cursor addition), `docs/decisions/ADR-002-release-scope-v0.2.0.md` (含 D11 校准说明) and `docs/decisions/ADR-001-release-scope-v0.1.0.md` for lineage.
