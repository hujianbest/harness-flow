...
>
> - **Version**: `v0.6.0`, marked as a **pre-release** on GitHub Releases. v0.6.0 is a **minor release** on top of v0.5.1 that introduces repo-root `install.sh` + `uninstall.sh` + `tests/test_install_scripts.sh` (per [`docs/decisions/ADR-007-install-scripts-topology-and-manifest.md`](docs/decisions/ADR-007-install-scripts-topology-and-manifest.md) + [`docs/decisions/ADR-008-release-scope-v0.6.0.md`](docs/decisions/ADR-008-release-scope-v0.6.0.md)): a one-command bash installer for vendoring HF into any host repository, covering Cursor / OpenCode / both targets × copy / symlink topologies = 6 combinations + 2 negative-path scenarios = **14 e2e PASS**. Manifest-based uninstall preserves user-added skills (HYP-002 Blocking validated). Pure bash 3.2+ compatible with **zero new runtime dependencies** (no jq / python / node / npm; NFR-004 enforced). Manual `cp -R` / `ln -s` fallback remains documented. v0.5.x lineage descriptions below remain accurate.
> - **v0.5.x lineage**: v0.5.0 added a **closeout HTML companion report** to `hf-finalize` (every closeout now produces `features/<active>/closeout.html` alongside `closeout.md`); v0.5.1 patched a vendoring bug by moving the renderer from repo-root `scripts/` to `skills/hf-finalize/scripts/` (per [`docs/decisions/ADR-006-skill-anatomy-v2-and-vendoring-fix.md`](docs/decisions/ADR-006-skill-anatomy-v2-and-vendoring-fix.md) — HF skill anatomy v2: 4 subdirs `SKILL.md` + `references/` + `evals/` + `scripts/`).
> - **v0.6.0 does not grow the skill set** (still **24** `hf-*` + `using-hf-workflow`), does **not** add slash commands (still **7**), and does **not** touch the main-chain FSM or router transition map. install scripts are repo-root entrypoints, **not** hf-* skills (per ADR-008 D2 / D8) — they live at the repo root because their audience is users vendoring HF (not maintainers); HF skill anatomy v2 stays unchanged.
...
> - This narrow surface is a deliberate choice (ADR-001 D1 / ADR-002 D1 / ADR-003 D2 / ADR-004 D2 / ADR-005 D7 / ADR-008 D5 — "P-Honest, narrow but hard"). HarnessFlow refuses to disguise "code merged / engineering closeout" as "shipped to production"; the v0.5.0 closeout HTML is a **visual rendering** of the closeout pack, **not** a deployment record; v0.6.0 install scripts are a **vendor-time installer**, **not** a deployment / rollout system.
>
> See `docs/decisions/ADR-008-release-scope-v0.6.0.md` for the v0.6.0 release scope decisions; `docs/decisions/ADR-007-install-scripts-topology-and-manifest.md` for the install scripts engineering decisions; `docs/decisions/ADR-006-skill-anatomy-v2-and-vendoring-fix.md` (v0.5.1 patch scope); `docs/decisions/ADR-005-release-scope-v0.5.0.md` (v0.5.0); `docs/decisions/ADR-004-hf-release-skill.md` (hf-release standalone skill), `docs/decisions/ADR-003-release-scope-v0.3.0.md` (Cursor addition), `docs/decisions/ADR-002-release-scope-v0.2.0.md` (含 D11 校准说明) and `docs/decisions/ADR-001-release-scope-v0.1.0.md` for lineage.
