...
> ## Scope Note (v0.5.0 pre-release)
>
> - **Version**: `v0.5.0`, marked as a **pre-release** on GitHub Releases. v0.5.0 is a **reviewer-experience** release: it adds a **closeout HTML companion report** to `hf-finalize` (every closeout now produces `features/<active>/closeout.html` alongside `closeout.md`), plus a new stdlib-only `skills/hf-finalize/scripts/render-closeout-html.py` that renders it (workflow timeline rail + tests + coverage rings + searchable evidence matrix; WCAG 2.2 AA, dark/light, printable, anti-AI-slop disciplined per `skills/hf-ui-design/references/anti-slop-checklist.md`). v0.5.0 does **not** grow the skill set (still **24** `hf-*` + `using-hf-workflow`), does **not** add slash commands (still **7**), and does **not** touch the main-chain FSM or router transition map. v0.4.0's `hf-release` (release-tier standalone skill) is unchanged.
...
>
> See `docs/decisions/ADR-005-release-scope-v0.5.0.md` for the full v0.5.0 release scope decisions; `docs/decisions/ADR-004-hf-release-skill.md` (hf-release standalone skill), `docs/decisions/ADR-003-release-scope-v0.3.0.md` (Cursor addition), `docs/decisions/ADR-002-release-scope-v0.2.0.md` (含 D11 校准说明) and `docs/decisions/ADR-001-release-scope-v0.1.0.md` for lineage.
