...
>
> - **Version**: `v0.5.0`, marked as a **pre-release** on GitHub Releases. v0.5.0 is a **reviewer-experience** release: it adds a **closeout HTML companion report** to `hf-finalize` (every closeout now produces `features/<active>/closeout.html` alongside `closeout.md`), plus a new stdlib-only `scripts/render-closeout-html.py` that renders it (workflow timeline rail + tests + coverage rings + searchable evidence matrix; WCAG 2.2 AA, dark/light, printable, anti-AI-slop disciplined per `skills/hf-ui-design/references/anti-slop-checklist.md`). v0.5.0 does **not** grow the skill set (still **24** `hf-*` + `using-hf-workflow`), does **not** add slash commands (still **7**), and does **not** touch the main-chain FSM or router transition map. v0.4.0's `hf-release` (release-tier standalone skill) is unchanged.
