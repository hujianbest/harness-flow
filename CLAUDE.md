...
On every session, **read `agents/hf-orchestrator.md` and adopt that persona for the rest of the session**. The orchestrator is HarnessFlow's always-on agent persona that:
...
The 24 `hf-*` skills are SOPs to be invoked by the orchestrator; they are not routing authorities. The orchestrator's progressive-disclosure references (FSM transition map / dispatch protocol / reviewer return contract / evidence guides / etc.) live at `agents/references/*.md`.
...
HarnessFlow's main chain ends at `hf-finalize` (engineering-level closeout). It is NOT production deployment, observability, incident response, or post-launch ops. The v0.6.0 introduction of `agents/hf-orchestrator.md` (HF three-layer architecture invariant per ADR-007 D1: Doer Skills / Reviewer-Gate Skills / Orchestrator Agent) does **not** grow the leaf skill set (still 24 `hf-*`), does **not** add slash commands (still 7), does **not** change closeout pack schema / reviewer verdict vocabulary / `hf-release` behavior. See `docs/decisions/ADR-007-orchestrator-extraction-and-skill-decoupling.md` for the full rationale.

## Setup details

See [`docs/claude-code-setup.md`](docs/claude-code-setup.md) for installation flow + plugin marketplace.
