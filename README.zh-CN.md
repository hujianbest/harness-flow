...
> ## 范围声明（v0.5.0 pre-release）
>
> - **版本**：`v0.5.0`，在 GitHub Releases 上标记为 **pre-release**。v0.5.0 是 **reviewer 体验切面** 版本——给 `hf-finalize` 的输出契约新增一份 **closeout HTML 工作总结报告**（每次 closeout 都会在 `features/<active>/closeout.html` 同步落盘），并新增 stdlib-only 渲染脚本 `skills/hf-finalize/scripts/render-closeout-html.py`（HF 主链节点 timeline rail + tests + coverage rings + 可搜索可排序 evidence matrix；WCAG 2.2 AA、暗亮主题、可打印、按 `skills/hf-ui-design/references/anti-slop-checklist.md` 反 AI slop 自检 S1-S8 全条覆盖）。v0.5.0 **不**扩 skill 集合（仍 **24** `hf-*` + `using-hf-workflow`），**不**新增 slash 命令（仍 **7**），**不**改主链 FSM 或 router transition map。v0.4.0 引入的 `hf-release`（release-tier standalone skill）保持不动。
...
>
> 完整 v0.5.0 发版范围决策见 `docs/decisions/ADR-005-release-scope-v0.5.0.md`；v0.4.0 沿革（hf-release 引入）见 `docs/decisions/ADR-004-hf-release-skill.md`；v0.3.0 沿革（Cursor 加入）见 `docs/decisions/ADR-003-release-scope-v0.3.0.md`；v0.2.0 沿革（含 D11 校准说明）见 `docs/decisions/ADR-002-release-scope-v0.2.0.md`；v0.1.0 沿革见 `docs/decisions/ADR-001-release-scope-v0.1.0.md`。
