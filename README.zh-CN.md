...
> ## v0.1.0 范围声明（pre-release）
>
> 这是**首个对外预发布版本**，范围由 [`docs/decisions/ADR-001-release-scope-v0.1.0.md`](docs/decisions/ADR-001-release-scope-v0.1.0.md) 锁定，并刻意按 **P-Honest** 原则保持窄而硬（窄、硬、少承诺面积）。
>
> **v0.1.0 包含**
> - 23 个 `hf-*` workflow skill + `using-hf-workflow` 公共入口 —— 从 `hf-product-discovery` 到 `hf-finalize` 的完整主链，加分支节点（`hf-hotfix` / `hf-increment`）和经验沉淀节点 `hf-bug-patterns`。
> - 6 条短别名 slash 命令：`/hf` `/spec` `/plan` `/build` `/review` `/ship`（`/plan` 合并 design + tasks，router 按工件证据决定派发）。
> - 集成入口仅 **Claude Code** 与 **OpenCode**。
> - `examples/writeonce/` quickstart：把主链端到端跑一遍。
>
> **v0.1.0 不包含**
> - **没有 release / ops 类 skill** —— 没有 `hf-security-hardening`、`hf-performance-gate`、`hf-deprecation-and-migration`、`hf-shipping-and-launch`、`hf-ci-cd-and-automation`、`hf-debugging-and-error-recovery`、`hf-browser-runtime-evidence`。主链终点是 `hf-finalize`（工程级 closeout）；部署管线 / 可观测 / 事故响应 / 度量回流 / 上线后运维不在 v0.1.0 范围。
> - **不支持 Cursor / Gemini CLI / Windsurf / Copilot / Kiro** —— 仓库本身在这些客户端中可用，但它们不在 v0.1.0 的"已支持平台"承诺范围。延后到 v0.2+。
> - **没有自动化 SKILL.md anatomy / 质量基线** —— `docs/principles/` 下的文档是设计参考，不是合规基线，v0.1.0 不带 audit 脚本。
> - **没有跨 SKILL.md 的 anti-rationalization 表** —— 延后到发版后视实际反馈再做。
> - **不预占 npm / PyPI / 任何 marketplace 包名**。
>
> 完整的 v0.1.0 release notes 见 [`CHANGELOG.md`](CHANGELOG.md)。
...
MIT 许可证（见 [LICENSE](LICENSE)）——可自由用于个人项目、团队和工具。

## 致谢

HarnessFlow 站在一长串工程与产品发现实践的肩膀上。下表中每条都是一手来源，其思想被编码进 HF 的一个或多个节点；HF 把它们改写并组合成一套 agent 驱动的工作流，而不是重新发明它们。

| 来源 | HF 借用了什么 | 落到哪里 |
|---|---|---|
| [forrestchang/andrej-karpathy-skills](https://github.com/forrestchang/andrej-karpathy-skills) | 横切 LLM coding 行为基线（Think Before Coding / Simplicity First / Surgical Changes / Goal-Driven Execution） | `docs/principles/coding-principles.md`，已吸收进每个 `hf-*` SKILL.md |
| [Software Engineering at Google](https://abseil.io/resources/swe-book) + [Google engineering practices](https://google.github.io/eng-practices/) | code-review 规范、change sizing、test pyramid 取向 | `hf-code-review`、`hf-test-driven-dev`、`hf-test-review` |
| Eric Evans — *Domain-Driven Design* | 战略建模：Bounded Context / Ubiquitous Language / Context Map | `hf-design`（战略层） |
| Vaughn Vernon — *Implementing Domain-Driven Design* | 战术模式：Aggregate / VO / Repository / Domain Service / Application Service / Domain Event | `hf-design`（战术层） |
| Alberto Brandolini — *Event Storming* | 通过 domain event 把 spec 与 design 衔接 | `hf-design` |
| Kent Beck — *Test-Driven Development* + Two Hats | Canon TDD、Two Hats 纪律、opportunistic + preparatory refactoring | `hf-test-driven-dev`、`hf-code-review`（refactor hygiene） |
| Martin Fowler — *Refactoring*、*Patterns of Enterprise Application Architecture* | Front Controller pattern、opportunistic / preparatory refactoring 框架 | `using-hf-workflow`、`hf-test-driven-dev` |
| Robert C. Martin — *Clean Architecture*、SOLID | 实现节点的 Clean Architecture conformance 与 SOLID 检查 | `hf-test-driven-dev`、`hf-code-review` |
| Michael Fagan — Fagan Inspection | 作者/评审者角色分离、结构化 walkthrough | 所有 `hf-*-review` 节点 |
| Simon Brown — C4 Model | 架构图绘制约定 | `hf-design` |
| Gernot Starke — ARC42 | 架构文档骨架 | `hf-design` |
| ISO/IEC 25010 + Quality Attribute Scenarios | 质量属性模型 + NFR 场景语言 | `hf-specify`（NFR）、`hf-design`（NFR 承接） |
| Microsoft — STRIDE | 轻量威胁建模 | `hf-design` |
| Jakob Nielsen — Heuristic Evaluation | UX 启发式 checklist | `hf-ui-design`、`hf-ui-review` |
| W3C WAI — WCAG 2.2 AA | 无障碍基线 | `hf-ui-design`、`hf-ui-review` |
| PMI — PMBOK | 项目 closeout / handoff pack 思路 | `hf-finalize` |
| Tony Ulwick / Clayton Christensen — Jobs-to-be-Done | 结果导向的发现 | `hf-product-discovery` |
| Teresa Torres — Opportunity Solution Tree | 发现结构 | `hf-product-discovery` |

如果发现 HF 用了某种方法但漏了致谢，请提 issue。
