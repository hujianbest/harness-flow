...
| `hf-design` | ADR、C4 Model、Risk-Driven Architecture、YAGNI + Complexity Matching、ARC42、DDD Strategic Modeling (Bounded Context / Ubiquitous Language / Context Map)、Event Storming (spec→design bridge)、Quality Attribute Scenarios (NFR 承接)、STRIDE 轻量威胁建模 |
...
## 仓库导航（顶层导航行；hf-finalize 同步）

- **Active feature**: 无（features/001-hf-doc-freshness-gate 已 closed @ 2026-04-23）
- **最近 closeout**: [features/001-hf-doc-freshness-gate](features/001-hf-doc-freshness-gate/closeout.md) — 引入 `hf-doc-freshness-gate` skill (workflow-closeout, 2026-04-23)
- **ADR 索引**:
  - [ADR-0001](docs/adr/0001-record-architecture-decisions.md) 启用 ADR pool（accepted）
  - [ADR-0002](docs/adr/0002-hf-doc-freshness-gate-as-independent-node.md) hf-doc-freshness-gate 作为独立 gate（accepted）
  - [ADR-0003](docs/adr/0003-doc-freshness-gate-router-position-parallel-tier.md) router 位置 P3 sequential（accepted）
