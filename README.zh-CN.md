...
- **横切行为基线**（宪法层文档，非 workflow 节点）：`skills/principles/coding-principles.md` — Think Before Coding / Simplicity First（YAGNI）/ Surgical Changes / Goal-Driven Execution；通过 `AGENTS.md` § Soul docs 被每个 `hf-*` skill 自动继承，改写自 [forrestchang/andrej-karpathy-skills](https://github.com/forrestchang/andrej-karpathy-skills)
...
| 文档 | 核心原则 |
|---|---|
| `skills/principles/coding-principles.md` | Think Before Coding、Simplicity First (YAGNI)、Surgical Changes、Goal-Driven Execution — 改写自 [forrestchang/andrej-karpathy-skills](https://github.com/forrestchang/andrej-karpathy-skills) |

这 4 条原则落在宪法层 `skills/principles/`（随 skills 包发布），**不是**独立 `hf-*` skill。每个 `hf-*` skill 通过 `AGENTS.md` § Soul docs 自动继承。它们**不**进入 canonical `Next Action Or Recommended Skill` 受控词表，**不**给任何节点的 Workflow 加额外步骤，**不**替代 review / gate / approval / finalize 判断。
...
> **范围说明**：当前 Workflow Shape 的终点是 `hf-finalize`（工程级 closeout）。**发布与上线**侧（部署管线 / 可观测 / 事故响应 / 度量回流 / 上线后运维）当前**不是**主链一等阶段。这与 `skills/principles/soul.md` 的"现状脚注"一致——HF 必须把这一缺口显式抛回用户，而**不能**悄悄把"代码合并 / 工程 closeout"当作"上线"。
...
- `skills/principles/` 存放 HF 宪法层 9 份原则文档（每个 skill 通过 `AGENTS.md` § Soul docs 自动继承）。
...
6. `skills/principles/methodology-coherence.md`（方法论协作 / 反替代规则 / Phase × profile 激活表）
...
这个仓库包含当前 HF skill family、共享文档、模板，以及支撑这套 pack 的原则文档（含方法论协作与 phase / profile 激活地图 `skills/principles/methodology-coherence.md`）。
