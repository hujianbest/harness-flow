...
>
> - **版本**：`v0.6.0`，在 GitHub Releases 上标记为 **pre-release**。v0.6.0 是 v0.5.1 之上的 **minor release**，引入仓库根 `install.sh` + `uninstall.sh` + `tests/test_install_scripts.sh`（详见 [`docs/decisions/ADR-007-install-scripts-topology-and-manifest.md`](docs/decisions/ADR-007-install-scripts-topology-and-manifest.md) + [`docs/decisions/ADR-008-release-scope-v0.6.0.md`](docs/decisions/ADR-008-release-scope-v0.6.0.md)）：一条命令把 HF vendor 进任意宿主仓库，覆盖 Cursor / OpenCode / both 三个 target × copy / symlink 两个 topology = 6 个组合 + 2 个 negative-path scenario = **14 e2e PASS**。Manifest-based uninstall 保留用户自加的 skill（HYP-002 Blocking 已验证）。纯 bash 3.2+ 兼容、**零新增运行时依赖**（不引入 jq / python / node / npm；NFR-004 锁定）。手工 `cp -R` / `ln -s` fallback 仍保留作为高级用户路径。v0.5.x 沿革描述（下方）仍然准确。
> - **v0.5.x 沿革**：v0.5.0 给 `hf-finalize` 输出契约新增 **closeout HTML 工作总结报告**（每次 closeout 同步落盘 `features/<active>/closeout.html`）；v0.5.1 修复 vendoring 缺陷——把渲染脚本从仓库根 `scripts/` 物理迁移到 `skills/hf-finalize/scripts/`（详见 [`docs/decisions/ADR-006-skill-anatomy-v2-and-vendoring-fix.md`](docs/decisions/ADR-006-skill-anatomy-v2-and-vendoring-fix.md)——HF skill anatomy v2：4 类子目录 `SKILL.md` + `references/` + `evals/` + `scripts/`）。
> - **v0.6.0 不扩 skill 集合**（仍 **24** `hf-*` + `using-hf-workflow`），**不**新增 slash 命令（仍 **7**），**不**改主链 FSM 或 router transition map。install scripts 是仓库级入口，**不是** hf-* skill（按 ADR-008 D2 / D8）——它们落仓库根而非 `skills/<name>/scripts/`，因为受众是 vendor HF 的用户（不是 maintainer）；HF skill anatomy v2 立场不变。
> - **正式支持的客户端**：**Claude Code**、**OpenCode**、**Cursor**（与 v0.3.0 一致）。其余 4 家延后客户端（Gemini CLI / Windsurf / GitHub Copilot / Kiro）继续延后到 v0.7+。
> - **主链终点仍是 `hf-finalize`**——**单 feature 工程级 closeout**。`hf-release` 是 release-tier 独立 skill，**汇总**多个 `workflow-closeout` 的 feature 成 vX.Y.Z 版本切片，**不**在主链中。发布管线、部署、可观测、事故响应、安全加固、性能门禁、debugging-and-error-recovery、deprecation-and-migration **仍然是 v0.7+ planned `hf-shipping-and-launch` / 等等（当前尚未实现）**。
> - 这种窄而硬的范围是刻意选择（ADR-001 D1 / ADR-002 D1 / ADR-003 D2 / ADR-004 D2 / ADR-005 D7 / ADR-008 D5 — "P-Honest，窄而硬"）。HarnessFlow 拒绝把"代码合并 / 工程 closeout"伪装成"上线到生产"；v0.5.0 的 closeout HTML 是 closeout pack 的**视觉渲染**，**不是**部署记录；v0.6.0 的 install scripts 是 **vendor 时安装器**，**不是**部署 / rollout 系统。
>
> v0.6.0 范围决策见 `docs/decisions/ADR-008-release-scope-v0.6.0.md`；install scripts 工程层决策见 `docs/decisions/ADR-007-install-scripts-topology-and-manifest.md`；v0.5.1 patch 范围决策见 `docs/decisions/ADR-006-skill-anatomy-v2-and-vendoring-fix.md`；完整 v0.5.0 发版范围决策见 `docs/decisions/ADR-005-release-scope-v0.5.0.md`；v0.4.0 沿革（hf-release 引入）见 `docs/decisions/ADR-004-hf-release-skill.md`；v0.3.0 沿革（Cursor 加入）见 `docs/decisions/ADR-003-release-scope-v0.3.0.md`；v0.2.0 沿革（含 D11 校准说明）见 `docs/decisions/ADR-002-release-scope-v0.2.0.md`；v0.1.0 沿革见 `docs/decisions/ADR-001-release-scope-v0.1.0.md`。
