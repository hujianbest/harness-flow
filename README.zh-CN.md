...
>
> - **版本**：`v0.6.0`，在 GitHub Releases 上标记为 **pre-release**。v0.6.0 是 **minor release**，引入 **`agents/hf-orchestrator.md`** 作为 always-on agent persona，替代 `using-hf-workflow` + `hf-workflow-router`（后两者在 v0.6.x 兼容期保留为 deprecated alias）。这一版锁定 **HF 三层架构 invariant**（详见 [`docs/decisions/ADR-007-orchestrator-extraction-and-skill-decoupling.md`](docs/decisions/ADR-007-orchestrator-extraction-and-skill-decoupling.md) D1）：**Doer Skills**（12）/ **Reviewer-Gate Skills**（11）/ **Orchestrator Agent**（1）。orchestrator 由每个支持宿主的 always-on 注入机制自动加载（Cursor `.cursor/rules/harness-flow.mdc` / Claude Code `CLAUDE.md` + `.claude-plugin/plugin.json` agent 注册 / OpenCode `AGENTS.md`）。v0.6.0 **不**扩 leaf skill 集合（仍 **24** `hf-*`），**不**新增 slash 命令（仍 **7**），**不**改 closeout pack schema / reviewer return verdict 词表 / `hf-release` standalone 行为，**不**修改任何 leaf skill 内容。ADR-007 D3 给出 6 步落地路径；**v0.6.0 范围严格限定为 Step 1**（建 orchestrator + 三宿主 stub + 旧 skill 转 deprecated alias）。Step 2-6（leaf skill `Next Action` 字段降级 / Hard Gate 分级 / 跨 hf-* 引用清理 / 物理删除旧 skill）延后到后续 increments，per ADR-007 D3。
> - **v0.5.x 主线（延续）**：v0.5.0 给 `hf-finalize` 输出契约新增 closeout HTML 工作总结报告（每次 closeout 都会在 `features/<active>/closeout.html` 同步落盘），并提供 stdlib-only 渲染脚本 `skills/hf-finalize/scripts/render-closeout-html.py`（HF 主链节点 timeline rail + tests + coverage rings + 可搜索可排序 evidence matrix；WCAG 2.2 AA、暗亮主题、可打印、按 `skills/hf-ui-design/references/anti-slop-checklist.md` 反 AI slop 自检 S1-S8 全条覆盖）。v0.5.1 是 vendoring patch（脚本物理位置从仓库根 `scripts/` 迁到 `skills/hf-finalize/scripts/`）。v0.4.0 引入的 `hf-release`（release-tier standalone skill）在 v0.6.0 保持不动。
> - **正式支持的客户端**：**Claude Code**、**OpenCode**、**Cursor**（与 v0.3.0 一致）。其余 4 家延后客户端（Gemini CLI / Windsurf / GitHub Copilot / Kiro）继续延后到 v0.7+。
> - **主链终点仍是 `hf-finalize`**——**单 feature 工程级 closeout**。v0.6.0 仅把 orchestration 从 leaf skill 抽出为独立 agent persona；不扩展主链。`hf-release` 是 release-tier 独立 skill，**汇总**多个 `workflow-closeout` 的 feature 成 vX.Y.Z 版本切片，**不**在主链中。发布管线、部署、可观测、事故响应、安全加固、性能门禁、debugging-and-error-recovery、deprecation-and-migration **仍然是 v0.7+ planned `hf-shipping-and-launch` / 等等（当前尚未实现）**。
> - 这种窄而硬的范围是刻意选择（ADR-001 D1 / ADR-002 D1 / ADR-003 D2 / ADR-004 D2 / ADR-005 D7 / **ADR-007 D6 / D7** — "P-Honest，窄而硬"）。HarnessFlow 拒绝把"代码合并 / 工程 closeout"伪装成"上线到生产"；v0.5.0 的 closeout HTML 是 closeout pack 的**视觉渲染**，不是部署记录；v0.6.0 的 orchestrator 是**结构性重构**，加强 skill 独立可调用性而不扩张范围。
>
> v0.6.0 minor 范围决策见 `docs/decisions/ADR-007-orchestrator-extraction-and-skill-decoupling.md`；v0.5.1 patch 见 `docs/decisions/ADR-006-skill-anatomy-v2-and-vendoring-fix.md`；v0.5.0 见 `docs/decisions/ADR-005-release-scope-v0.5.0.md`；v0.4.0 沿革（hf-release 引入）见 `docs/decisions/ADR-004-hf-release-skill.md`；v0.3.0 沿革（Cursor 加入）见 `docs/decisions/ADR-003-release-scope-v0.3.0.md`；v0.2.0 沿革（含 D11 校准说明）见 `docs/decisions/ADR-002-release-scope-v0.2.0.md`；v0.1.0 沿革见 `docs/decisions/ADR-001-release-scope-v0.1.0.md`。
...
HarnessFlow v0.5.0 正式支持 **Claude Code**、**OpenCode** 和 **Cursor**（客户端面与 v0.3.0 / v0.4.0 一致）。三条路径读的是同一份 `skills/` 目录，区别只在客户端如何发现 HF（Claude Code：marketplace 插件 + 7 条 slash 命令；OpenCode：`.opencode/skills` 软链接 + 自然语言路由；Cursor：`.cursor/rules/harness-flow.mdc` alwaysApply 规则 + 自然语言路由）。

### Claude Code（推荐）

通过 marketplace 安装（用显式 HTTPS URL + `.git` 后缀强制 https 克隆；`owner/repo` 简写形式会走默认 SSH，未配 GitHub SSH key 的环境会失败）：
...
安装后会注册 7 条短 slash 命令（v0.4.0 新增 `/release`；见下方 [Slash 命令](#slash-命令claude-code)）。

> 安装命令是 `harness-flow@hujianbest-harness-flow`（plugin 名 `harness-flow` + marketplace 名 `hujianbest-harness-flow`），**不是** `harness-flow@harness-flow`。v0.2.0 把 marketplace 也命名为 `harness-flow`，与 plugin 同名触发了 Claude Code 解析器的名字碰撞 bug；v0.2.1 把 marketplace 重命名以修复此问题。完整设置说明（含 SSH 故障排查）见 `docs/claude-code-setup.md`。

本地 / 开发模式（在迭代 HarnessFlow 自身时用）：
...
完整的安装、验证、SSH/HTTPS 回退方案见 `docs/claude-code-setup.md`。
...
在 OpenCode 里运行 `/skills` 验证发现是否成功——你应当看到 24 个 `hf-*` skill（含 v0.2.0 新增的 `hf-browser-testing` 与 v0.4.0 新增的 `hf-release`）加上 `using-hf-workflow`。然后发出第一条 prompt：
...
Cursor 集成走 Cursor 的 **rules** 机制（不注册插件、不注册 slash 命令）。本仓库随包附带 `.cursor/rules/harness-flow.mdc`（frontmatter `alwaysApply: true`），每次 Cursor 会话自动加载它，告诉 agent 把 `skills/using-hf-workflow/SKILL.md` 作为入口 shell 加载，并在意图模糊时切换到 `skills/hf-workflow-router/SKILL.md`。
...
HarnessFlow 的 skill 是纯 Markdown，**理论上**可以通过把 `skills/` 当作指令文件喂给 Gemini CLI / Windsurf / GitHub Copilot / Kiro / Codex 等使用。但这些路径**不在 v0.5.0 的承诺范围内**，本版本也不提供对应的 setup 文档。这 4 家继续延后到 v0.6+（沿用 ADR-002 D11 "一次进一家" 立场——Cursor 已在 v0.3.0 落地，其余客户端待真实使用反馈成熟后再批量进）。
...
`docs/principles/` 属于 **HarnessFlow 仓库自身**（HF 自己的 design reference，按 ADR-001 D11：不是运行时依赖、不是发版门禁、不是 SKILL.md 合规基线）。Vendor 时**不**需要复制 `docs/principles/`。
...
Claude Code 插件注册 7 条短别名（6 条由 ADR-001 D4 锁定，`/release` 由 ADR-004 D4 新增）。前 6 条都是 **bias，不是 bypass**——router 仍然会用磁盘工件证据校验上游前置条件。`/release` 是例外：它 **direct invoke** `hf-release`，**不**经 router（ADR-004 D3 — `hf-release` 与 router 解耦）。

| 命令 | 偏向触发 | 备注 |
|---|---|---|
| `/hf` | `using-hf-workflow` → `hf-workflow-router` | 默认。不确定下一步该去哪个节点时用它。 |
| `/spec` | `hf-specify` | spec 起草 / 修订。 |
| `/plan` | `hf-design`（规格声明 UI surface 时并行 `hf-ui-design`）或 `hf-tasks` | 合并的 planning 命令——design 与 tasks 刻意合成一条命令（ADR-001 D4 理由）。 |
...
| `/release [version]` | **direct invoke** `hf-release`（**不**经 router） | 切 vX.Y.Z 工程级 release：汇总 `workflow-closeout` 的 feature、起草 scope ADR、跑 release-wide regression、同步 CHANGELOG / release notes / ADR 状态、产 tag-ready pack。**不**做部署 / staged rollout / 监控 / 回滚（仍归 v0.6+ planned `hf-shipping-and-launch`，**当前尚未实现**）。 |
...
- 没有 `/ship-to-prod`（或类似部署命令）——部署 / staged rollout / 监控 / 回滚是 v0.6+ planned `hf-shipping-and-launch`（**当前尚未实现**）。

OpenCode 与 Cursor 都不注册任何 slash 命令文件；同样的意图通过自然语言 + `using-hf-workflow` 触达（ADR-003 D6 让 Cursor 走与 OpenCode 一致的 NL + router 模型，而不是仿造 Claude Code 的短 slash 命令）。`using-hf-workflow` 入口 shell 第 5 步 entry bias 表已加一行"切版本 / 出 release / 打 tag" → direct invoke `hf-release`，与 `/release` 命令同语义但通过 NL 触达（ADR-004 D3）。
