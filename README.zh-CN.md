...
> ## 范围声明（v0.4.0 pre-release）
>
> - **版本**：`v0.4.0`，在 GitHub Releases 上标记为 **pre-release**。v0.4.0 是 v0.2.0 之后第一次扩 skill 集合的版本——新增 **`hf-release`** 作为 release-tier **standalone** skill（工程级 release：scope ADR + release-wide regression + 发布文档聚合 + tag readiness pack），并新增 Claude Code 第 7 条 slash 命令 `/release`。`hf-release` 与 `hf-workflow-router` **完全解耦**——**不**进主链 FSM，**不**进 router transition map。Skill 集合：23 → **24** `hf-*` + `using-hf-workflow`。Slash 命令：6 → **7**。
> - **正式支持的客户端**：**Claude Code**、**OpenCode**、**Cursor**（与 v0.3.0 一致）。其余 4 家延后客户端（Gemini CLI / Windsurf / GitHub Copilot / Kiro）继续延后到 v0.5+。
> - **主链终点仍是 `hf-finalize`**——**单 feature 工程级 closeout**。`hf-release` 是 release-tier 独立 skill，**汇总**多个 `workflow-closeout` 的 feature 成 vX.Y.Z 版本切片，**不**在主链中。发布管线、部署、可观测、事故响应、安全加固、性能门禁、debugging-and-error-recovery、deprecation-and-migration **仍然是 v0.5+ planned `hf-shipping-and-launch` / 等等（当前尚未实现）**。
> - 这种窄而硬的范围是刻意选择（ADR-001 D1 / ADR-002 D1 / ADR-003 D2 / ADR-004 D2 — "P-Honest，窄而硬"）。HarnessFlow 拒绝把"代码合并 / 工程 closeout"伪装成"上线到生产"；`hf-release` 是"切版本号 + 写发布文档"，**不是**"上线到生产"。
>
> 完整 v0.4.0 发版范围决策见 `docs/decisions/ADR-004-hf-release-skill.md`；v0.3.0 沿革（Cursor 加入）见 `docs/decisions/ADR-003-release-scope-v0.3.0.md`；v0.2.0 沿革（含 D11 校准说明）见 `docs/decisions/ADR-002-release-scope-v0.2.0.md`；v0.1.0 沿革见 `docs/decisions/ADR-001-release-scope-v0.1.0.md`。
...
HarnessFlow v0.4.0 正式支持 **Claude Code**、**OpenCode** 和 **Cursor**（客户端面与 v0.3.0 一致）。三条路径读的是同一份 `skills/` 目录，区别只在客户端如何发现 HF（Claude Code：marketplace 插件 + 7 条 slash 命令；OpenCode：`.opencode/skills` 软链接 + 自然语言路由；Cursor：`.cursor/rules/harness-flow.mdc` alwaysApply 规则 + 自然语言路由）。
...
HarnessFlow 的 skill 是纯 Markdown，**理论上**可以通过把 `skills/` 当作指令文件喂给 Gemini CLI / Windsurf / GitHub Copilot / Kiro / Codex 等使用。但这些路径**不在 v0.4.0 的承诺范围内**，本版本也不提供对应的 setup 文档。这 4 家继续延后到 v0.5+（沿用 ADR-002 D11 "一次进一家" 立场——Cursor 已在 v0.3.0 落地，其余客户端待真实使用反馈成熟后再批量进）。
...
| `/release [version]` | **direct invoke** `hf-release`（**不**经 router） | 切 vX.Y.Z 工程级 release：汇总 `workflow-closeout` 的 feature、起草 scope ADR、跑 release-wide regression、同步 CHANGELOG / release notes / ADR 状态、产 tag-ready pack。**不**做部署 / staged rollout / 监控 / 回滚（仍归 v0.5+ planned `hf-shipping-and-launch`，**当前尚未实现**）。 |
...
- 没有 `/ship-to-prod`（或类似部署命令）——部署 / staged rollout / 监控 / 回滚是 v0.5+ planned `hf-shipping-and-launch`（**当前尚未实现**）。
...
> **范围说明**：当前 Workflow Shape 的终点是 `hf-finalize`（**单 feature** 工程级 closeout）。**发布与上线**侧仍分两层：(1) **release-tier 版本切片**（多 feature → vX.Y.Z scope ADR + release-wide regression + 发布文档聚合 + tag readiness）由 v0.4.0 新增的 standalone skill `hf-release` 承担，与主链 **解耦**（不进本 Workflow Shape；通过 Claude Code 的 `/release` 或 OpenCode / Cursor 的 entry shell bias 行触发）。(2) **部署管线 / 可观测 / 事故响应 / 度量回流 / 上线后运维** 仍是 v0.5+ planned `hf-shipping-and-launch` / `hf-ci-cd-and-automation` / 等等——**当前尚未实现**。这与 `docs/principles/soul.md` 的"现状脚注"一致——HF 必须把这一缺口显式抛回用户，而**不能**悄悄把"代码合并 / 工程 closeout"当作"上线"；`hf-release` 是"切版本号 + 写发布文档"，**不是**"上线到生产"。
