...
> ## 范围声明（v0.6.0 pre-release）
>
> - **版本**：`v0.6.0`，在 GitHub Releases 上标记为 **pre-release**。v0.6.0 是 v0.5.1 之上的 **minor release**——HF 首次大规模 **author-side discipline 升级**，把在 [code-yeongyu/oh-my-openagent (OMO)](https://github.com/code-yeongyu/oh-my-openagent) 已经验证的 5 类机制翻译成 HF 体系内的可执行范围（Atlas wisdom-notebook / Metis gap-analysis / Momus 4-dim rubric / Prometheus interview FSM / `/init-deep` hierarchical context）。完整 release scope 决策：[`docs/decisions/ADR-011-release-scope-v0.6.0.md`](docs/decisions/ADR-011-release-scope-v0.6.0.md)（8 项决策）；路线图锁定 + 永久删除 v0.8：[`docs/decisions/ADR-008-omo-inspired-roadmap-v0.6-onwards.md`](docs/decisions/ADR-008-omo-inspired-roadmap-v0.6-onwards.md)；fast-lane 治理：[`docs/decisions/ADR-009-execution-mode-fast-lane-governance.md`](docs/decisions/ADR-009-execution-mode-fast-lane-governance.md)；v0.7 runtime sidecar 边界：[`docs/decisions/ADR-010-harnessflow-runtime-sidecar-boundary.md`](docs/decisions/ADR-010-harnessflow-runtime-sidecar-boundary.md)。
> - **v0.6.0 范围**——HF skill 总数 **24 → 28**（4 新 SKILL.md：[`hf-wisdom-notebook`](skills/hf-wisdom-notebook/SKILL.md) 跨 task 知识沉淀（5 文件强 schema + stdlib python validator）/ [`hf-gap-analyzer`](skills/hf-gap-analyzer/SKILL.md) author-side self-check（6 维 rubric）/ [`hf-context-mesh`](skills/hf-context-mesh/SKILL.md) 层次化 AGENTS.md 生成器（3 客户端 × 3 层模板）/ [`hf-ultrawork`](skills/hf-ultrawork/SKILL.md) explicit-opt-in fast-lane 节点（Hard Gates 段本地 enumerate 5 类不可压缩项 + 6 escape 条件））。7 个既有 SKILL.md surgical 修改（`hf-tasks-review` momus + N=3 rewrite loop / `hf-specify` 5 状态 Interview FSM / `hf-workflow-router` step-level recovery + category_hint + wisdom_summary + progress.md schema / `hf-code-review` CR9 AI Slop Detection rubric / `hf-test-driven-dev` FR-002 wisdom-notebook 集成 / `hf-completion-gate` §6.2 调 validator / `using-hf-workflow` 步骤 5 加 fast-lane 行）。Slash 命令仍 **7**（不增加）。宪法层（`docs/principles/{soul,methodology-coherence,skill-anatomy}.md`）**不变**。`install.sh` / `uninstall.sh` / `.cursor/rules/harness-flow.mdc` / `.claude-plugin/marketplace.json` **不变**（NFR-003 严守）。三客户端正式支持不变（Claude Code / OpenCode / Cursor）。
> - **永久从路线图删除**（ADR-008 D1 + ADR-011 D3）：6 项工程化末段 skill —— `hf-shipping-and-launch` / `hf-ci-cd-and-automation` / `hf-security-hardening` / `hf-performance-gate` / `hf-debugging-and-error-recovery` / `hf-deprecation-and-migration`。**不是延后——永远不实现**。HF 拒绝假装是部署工具；部署 / staged rollout / 监控 / 回滚演练 / health check / 上线后观察 等**显式 out-of-scope**，由项目自身 ops 流程承担。
> - **正式支持的客户端**：**Claude Code**、**OpenCode**、**Cursor**（与 v0.3.0 一致）。其余 4 家客户端扩展（Gemini CLI / Windsurf / GitHub Copilot / Kiro）按 ADR-011 D5 延后到 **v0.9**。
> - **v0.7 runtime sidecar（ADR-010）**：可选 `harnessflow-runtime` OpenCode plugin（TypeScript/Bun，参照 OMO `src/tools/hashline-edit/` 等实现），增加 hash-anchored editing / evidence-bus / progress-store / 5 类 lifecycle hook。**与本版 v0.6.0 markdown release 解耦**——独立 npm 包，独立版本节奏（按 ADR-010 D5 声明 `compatible_hf_versions`）。OpenCode 用户在 v0.7 release 落地后另行安装；Cursor / Claude Code 用户走 markdown-only 路径（HYP-002 PASS 已证明 markdown-only fast lane 在无 runtime 时可用）。
> - **主链终点仍是 `hf-finalize`**——**单 feature 工程级 closeout**。v0.5.0 增加了 closeout HTML companion（每次 closeout 在 `closeout.md` 旁同步落盘 `closeout.html`，按 [ADR-005](docs/decisions/ADR-005-release-scope-v0.5.0.md)）；v0.6.0 不再扩 `hf-finalize` 的输出契约。`hf-release` 是 release-tier 独立 skill，**汇总**多个 `workflow-closeout` 的 feature 成 vX.Y.Z 版本切片，**不**在主链中。
> - 这种窄而硬的范围是刻意选择（"P-Honest，窄而硬"——按 ADR-001 D1 / ADR-002 D1 / ADR-003 D2 / ADR-004 D2 / ADR-005 D7 / **ADR-008 D1** / ADR-011 D3）。HarnessFlow 拒绝把"代码合并 / 工程 closeout"伪装成"上线到生产"；v0.5.0 的 closeout HTML 是 closeout pack 的**视觉渲染**，**不是**部署记录；v0.6.0 的 fast lane 是**架构师 explicit opt-in**，**绝不绕过** Fagan author/reviewer 分离 + 3 类 gate verdict。
>
> v0.6.0 release scope 决策见 `docs/decisions/ADR-011-release-scope-v0.6.0.md`；v0.6 OMO-inspired 方向决策见 `docs/decisions/ADR-008..010-*.md`；v0.5.1 patch 范围见 `docs/decisions/ADR-006-skill-anatomy-v2-and-vendoring-fix.md`；v0.5.0 见 `docs/decisions/ADR-005-release-scope-v0.5.0.md`；v0.4.0 沿革（hf-release 引入）见 `docs/decisions/ADR-004-hf-release-skill.md`；v0.3.0 沿革（Cursor 加入）见 `docs/decisions/ADR-003-release-scope-v0.3.0.md`；v0.2.0 沿革（含 D11 校准说明）见 `docs/decisions/ADR-002-release-scope-v0.2.0.md`；v0.1.0 沿革见 `docs/decisions/ADR-001-release-scope-v0.1.0.md`。
...
| [code-yeongyu/oh-my-openagent (OMO)](https://github.com/code-yeongyu/oh-my-openagent) — Atlas wisdom-notebook / Metis gap-analysis / Momus 4 维 rubric / Prometheus interview FSM / `/init-deep` 层次化上下文 / `comment-checker` ai-slop hook / boulder-loop fast-lane 机制 | v0.6 author-side discipline 升级 — `hf-wisdom-notebook`（5 文件跨 task 知识 schema）/ `hf-gap-analyzer`（6 维 author-side self-check）/ `hf-tasks-review`（momus 4 维 rubric + N=3 rewrite loop）/ `hf-specify`（5 状态 Interview FSM + spec.intake.md）/ `hf-context-mesh`（3 客户端 × 3 层 AGENTS.md）/ `hf-code-review`（CR9 AI Slop Detection rubric）/ `hf-ultrawork`（explicit-opt-in fast lane，本地 enumerate 5 类不可压缩 + 6 escape conditions） |
...
| Skill | 核心方法论 |
|---|---|
| `hf-specify` | EARS、BDD / Gherkin、MoSCoW Prioritization、Socratic Elicitation、INVEST、ISO/IEC 25010 + Quality Attribute Scenarios、Success Metrics & Key Hypotheses Framing、RICE / ICE / Kano（承接自 discovery）、**5 状态 Interview FSM（v0.6；OMO Prometheus interview-mode 启发）+ spec.intake.md 持久化** |
...
| `hf-tasks-review` | INVEST Validation、Dependency Graph Validation、Traceability Matrix、Structured Walkthrough、**Momus 4 维 boolean cliff rubric + N=3 rewrite loop（v0.6；OMO Momus 启发；Clarity 100% / Verification 90% / Context 80% / Big Picture 100% / Zero-tolerance 0%；第 4 次 rejected-rewrite → 阻塞 → 升级架构师，与 fast-lane escape #5 对齐）** |
| `hf-gap-analyzer`（v0.6 新增） | Author-side Self-Check (pre-Fagan；OMO Metis 启发)、6 维 Gap Rubric（Implicit Intent / AI Slop / Missing Acceptance / Unaddressed Edge Cases / Scope Creep / Dangling Reference）、Anchored Findings、Explicit Absorption Markers — **不是 Fagan review 节点**；输出 `<artifact>.gap-notes.md`，作者 absorb / reject 每条 finding 后再提交对应 review |

### 执行与评审

| Skill | 核心方法论 |
|---|---|
| `hf-test-driven-dev` | TDD、Walking Skeleton、Test Design Before Implementation、Fresh Evidence Principle、Two Hats（Beck/Fowler）、Opportunistic + Boy Scout Refactoring、Preparatory Refactoring、Clean Architecture Conformance、Escalation Boundary、**Wisdom Notebook 5 文件 delta per task（v0.6；FR-002 集成 hf-wisdom-notebook）+ tasks.progress.json step-level state 同步** |
| `hf-test-review` | Fail-First Validation、Coverage Categories、Risk-Based Testing、Structured Walkthrough |
| `hf-code-review` | Fagan Code Inspection、Design Conformance Check、Defense-in-Depth Review、Clean Architecture Conformance Check、Two Hats / Refactoring Hygiene Review、Architectural Smells Detection、Separation of Author/Reviewer Roles、**CR9 AI Slop Detection rubric（v0.6；OMO comment-checker hook 启发；中英双语禁用模式 + 4 例外段）** |
...
| Skill | 核心方法论 |
|---|---|
| `hf-wisdom-notebook`（v0.6 新增） | Cross-task Knowledge Accumulation（OMO Atlas notepads 启发）、Append-only Audit Trail、Schema-Validated Persistence（5 文件强 schema：learnings / decisions / issues / verification / problems）、Handoff Summary Injection — 配套 stdlib python validator (`scripts/validate-wisdom-notebook.py`)，由 `hf-completion-gate` 调用 |
| `hf-context-mesh`（v0.6 新增） | Per-Client Template Set（OMO `/init-deep` 启发）、3 层层次结构（root / mid / leaf）、Skeleton-Only Generation、Architect-Driven Client Choice — 给宿主项目生成 `AGENTS.md`（OpenCode）/ `.cursor/rules/*.mdc`（Cursor）/ `CLAUDE.md`（Claude Code）骨架；**绝不替架构师写 conventions** |
| `hf-ultrawork`（v0.6 新增） | Architect-Explicit Opt-in（Execution Mode preference + 关键词识别）、Decision-Point Interception、Verdict-Then-Escape Check（按 ADR-009 D3 第 4 项 6 条 escape conditions）、Approval Artifact Persistence、Append-Only Audit Trail（`progress.md ## Fast Lane Decisions`，按 ADR-009 D4） — Hard Gates 段本地 enumerate **5 类不可压缩项**（8 Fagan reviews / 3 gate verdicts / closeout pack / approval 工件落盘 / Hard Gates "标准不清抛回"）；markdown-only 宣告式路径（HYP-002 PASS 已验证）— runtime hook 在 v0.7 落地（ADR-010） |

### v0.6 router 内部升级

`hf-workflow-router`（既有 skill，v0.6 surgical 升级）：step-level recovery via `tasks.progress.json`（FR-003）+ `category_hint` handoff 字段（FR-015 SHOULD；对齐 OMO category routing）+ `wisdom_summary` 注入（近 N=3 task 摘要 ≤ 1500 token，从 `notepads/` 读）+ `progress.md ## Wisdom Delta` + `## Fast Lane Decisions` schema 标准化（FR-010）。

`using-hf-workflow`（既有 skill，v0.6 surgical 升级）：步骤 5 entry bias 表加 1 行 `Execution Mode = auto` direct-invoke 到 `hf-ultrawork`（步骤 3 / 6 不动，按 ADR-009 D5；不增加 slash 命令）。
