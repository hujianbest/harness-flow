...
1. 建立证据基线
   ...
   - Input: 测试代码、`features/<id>/implementation-log.md`（含实现交接块）、`features/<id>/evidence/...`、ar-design-draft.md（测试设计章节）、requirement.md、`docs/component-design.md`、`AGENTS.md`
   - Output: 输入清单 + 实现交接块完整性
   - Stop / continue: 缺实现交接块或 evidence → blocked-content，下一步 `mwf-tdd-implementation`
...
2. Precheck
   ...
   - Stop / continue:
     - 缺关键测试资产或交接块 → blocked-content，下一步 `mwf-tdd-implementation`
     - route / stage / profile 冲突 → blocked-workflow，`reroute_via_router=true`，下一步 `mwf-workflow-router`
...
3. 多维评分
   ...
   维度（详见 `references/test-check-rubric.md`）：
   ...
   | TC7 Stability & Maintainability | 测试可独立运行 / 不依赖外部状态 / 命名清晰 / 不存在 flaky 信号 |

4. 正式 checklist 审查
   - Object: findings
   - Method: Checklist-Based Review
   - Input: rubric
   - Output: findings 含 severity / classification / rule_id（TC1-TC7 + 子项）/ anchor / 描述 / 建议修复
...
6. 写 review 记录
   - Output: `features/<id>/reviews/test-check.md`，按 `templates/mwf-review-record-template.md`
