...
防止"修好了本地但破坏了相邻模块"。在最小回归验证范围内收集 fresh evidence，判断回归面是否健康。运行在 traceability-review 之后。hybrid batch 下，回归面覆盖 `Batch Quality Scope` 中所有已实现 task 的合并影响面。
...
Profile-aware 回归范围：
- `full`：batch traceability 识别的所有区域
- `standard`：batch scope 的直接相关模块
- `lightweight`：batch scope 的最小 build/test 入口
...
检查：上游 review / traceability 记录是否齐全、batch scope 是否可恢复、每个实现交接块是否稳定、worktree 状态与当前验证位置是否一致。

- 上游结论缺失、batch scope 不完整或 route/stage/profile 冲突 → `阻塞`，下一步 `hf-workflow-router`
...
记录保存到 项目声明的 verification 路径；若无项目覆写，hybrid batch 默认使用 `features/<active>/verification/regression-batch-YYYY-MM-DD.md`（legacy 单 task 复审可继续写 `regression-task-NNN.md`）。若项目无专用格式，默认使用本 skill 模板 `references/verification-record-template.md`。
