...
description: 适用于 traceability review 通过后需回归验证、用户要求 regression check 的场景。不适用于判断任务完成、状态收尾、阶段不清。
---
...
不适用：判断任务完成 → 完成门禁；状态收尾 → 收尾归档；阶段不清 → 上游编排者编排者。
...
- 上游结论缺失或 route/stage/profile 冲突 → `阻塞`，下一步 上游编排者
- worktree-active 但 evidence 无法锚定同一 `Worktree Path` → `阻塞`，下一步 回归门禁
- precheck 通过 → 继续定义回归面
...
|---|---|---|---|
| build / typecheck / lint 失败 | 失败命令、退出码、关键报错摘录 | `需修改` | 实现层 TDD |
| 测试通过但覆盖率低于 项目级覆盖率门槛或当前任务门槛 | 覆盖率命令、实际结果、门槛来源 | `需修改` | 实现层 TDD |
| `lightweight` 且仅文档 / 配置类变更 | 最小相关验证（如 docs build、lint、config parse）+ 明确未覆盖区域 | 结果驱动 | 通过时 完成门禁，否则 实现层 TDD |
| 强制集成 / e2e 验证因环境不可用而未跑 | 项目约定 / DoD 是否允许降级；若允许，给出替代验证结果；若不允许，写明阻塞原因 | 无降级许可 → `阻塞`；有许可则按结果判断 | 无降级许可 → 回归门禁 |
| `worktree-active` 但证据来自其他目录或旧代码状态 | 当前 `Worktree Path`、证据来源路径 / 时间锚点 | `阻塞` | 回归门禁 |
| 上游 review/gate 缺失，或 route / stage / profile 冲突 | 缺失项或冲突项清单 | `阻塞` | 上游编排者 |
...
| 文件 | 用途 |
|------|------|
| `references/verification-record-template.md` | regression verification record 模板（与 完成门禁 同形态） |
