# mwf Profile And Route Map

> 配套 `mwf-workflow-router/SKILL.md`。展开 mwf 各 Workflow Profile 的合法节点集合、主链与支线，以及 hard stops 的具体表现。
...
```text
{ mwf-specify, mwf-spec-review, mwf-ar-design, mwf-ar-design-review,
  mwf-tdd-implementation, mwf-test-checker, mwf-code-review,
  mwf-completion-gate, mwf-finalize }
```
...
注意：组件实现设计是 AR 实现设计的输入。AR 实现设计**不得**临时改写组件架构；必须先经过 `mwf-component-design`。
...
using-mwf-workflow
  -> mwf-workflow-router
  -> mwf-problem-fix
  -> (可选) mwf-ar-design -> mwf-ar-design-review
  -> mwf-tdd-implementation
  -> mwf-test-checker
  -> mwf-code-review
  -> mwf-completion-gate
...
`mwf-problem-fix` 至少完成：复现路径或无法复现说明、根因、最小安全修复边界、是否需要补 AR 实现设计或组件实现设计、明确回流节点。
...
| `component-impact` | 发现是 DTS | 仍 `component-impact` 兼 hotfix 性质，启动 `mwf-problem-fix` 子线 |
...
1. 需求输入不清且涉及方向 / 范围 / 验收 → 停在 `mwf-specify`，回需求负责人
2. IR / SR / AR 追溯关系冲突 → 阻塞，回需求负责人
3. AR 不属于唯一组件 → 阻塞
4. 缺组件实现设计但当前修改影响组件边界 → 进 `mwf-component-design`
5. AR 实现设计未含测试设计章节 → 回 `mwf-ar-design`
6. TDD 完成后测试用例未经 `mwf-test-checker` → 不得进 `mwf-code-review`
...
| 来源节点 | 派发节点 |
|---|---|
| `mwf-specify` | `mwf-spec-review` |
| `mwf-component-design` | `mwf-component-design-review` |
| `mwf-ar-design` | `mwf-ar-design-review` |
| `mwf-tdd-implementation` | `mwf-test-checker` |
| `mwf-test-checker`（通过） | `mwf-code-review` |
