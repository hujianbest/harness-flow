# mwf Requirement Rows Contract

> 配套 `mwf-specify/SKILL.md`。规定 `features/<id>/requirement.md` 的需求条目最小字段、嵌入式 NFR 写法和分类。
...
`Component Impact` 不为 `none` 时，规格必须在 `Component Impact Assessment` 章节显式说明，并由 `mwf-workflow-router` 决定是否升级 component-impact profile。
...
`blocking` 问题在 `mwf-spec-review` 通过前必须闭合或显式回到需求负责人；reviewer 不得放过 `blocking` 问题给出 `通过`。
