...
| **UI Implementation Contract** | 将视觉方向转成下游可验证的页面/组件不变量、token 映射、禁用模式、截图目标和 DOM/网络断言，防止"文档通过、实现漂移" | Design QA / Design System handoff practice | 步骤 5.3 — 实现合同 |
...
- 每个关键页面/组件必须有 **UI Implementation Contract**：视觉不变量、token 消费规则、禁用模式、截图目标路由 / viewport、DOM anchors、console/network 断言和允许偏离范围
...
- 不把"有 Hero / 有卡片 / 有按钮"当作 UI 设计完成；必须说明这些元素如何符合视觉不变量与 token 合同
...
**5.3 UI Implementation Contract**

按 `references/ui-implementation-contract.md` 为每个关键页面/组件补充实现合同。最少包含：

- visual invariants：主色、排版节奏、grid、surface/elevation、motion 的必守规则
- token consumption：每个关键 UI 元素使用哪些 token；若使用 Tailwind / 组件库 utility，必须写出等价 token 映射
- forbidden drift：明确写出禁止的视觉漂移，例如未批准渐变、未批准品牌色、额外阴影层级、默认 dashboard 模板
- evidence targets：浏览器路由 / story、viewport、截图路径建议、DOM anchors、console/network 断言
- deviation handling：实现想偏离时必须先回 `hf-ui-design` 或 `hf-ui-review`，不得在代码里静默偏离

没有 UI Implementation Contract 的 UI surface 不得进入 `hf-tasks`。这不是像素级视觉稿要求，而是下游可验证的设计落地合同。
...
- UI Implementation Contract（页面/组件视觉不变量 + token 消费 + 禁用模式 + 截图/DOM/网络证据目标）
...
- [ ] UI Implementation Contract 已覆盖关键页面/组件，且写明 visual invariants、token mapping、forbidden drift、screenshot/DOM/network evidence targets
...
| UI 实现合同 | `references/ui-implementation-contract.md` | 关键页面/组件准备进入任务拆解前；定义 visual invariants、token mapping、截图/DOM/网络证据 |
...
- UI Implementation Contract 缺失，或只写"按设计实现"而未给出 visual invariants / token mapping / forbidden drift / evidence targets
- 设计合同没有禁止明显风险漂移（如文档主色为橙色但没有禁止蓝紫渐变 hero），导致下游只能检查元素存在
...
- UI Implementation Contract（关键页面/组件 visual invariants、token mapping、forbidden drift、runtime visual evidence targets）
...
- [ ] UI Implementation Contract 已覆盖关键页面/组件，足以让 tasks/code-review/traceability/gates 判断设计是否落地
- [ ] WCAG 2.2 AA 逐项声明（对比度 / 键盘 / 语义 / 焦点 / reduced motion）；触控/移动端 hit target ≥ 44×44px
