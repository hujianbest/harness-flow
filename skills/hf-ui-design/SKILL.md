---
name: hf-ui-design
description: 适用于规格含 UI surface（页面/组件/交互/前端）且设计未批准、或 UI 评审 返回需修改/阻塞需修订的场景。不适用于纯后端/脚本/API-only（不激活本节点）、架构与 API 契约、规格仍是草稿、设计已批准需拆任务、只需执行 UI 评审、阶段不清或证据冲突。
---
...
本 skill 是 **design stage 的 conditional peer**：与 设计阶段（技术/架构设计）**同层并行**，不 bypass 主链。两者都通过各自 review 才能进入 任务规划（联合 design approval）。
...
| **ADR（继承自 设计阶段）** | UI 层关键决策（导航范式、组件库选型、布局模式、视觉方向）用同一 ADR 模板记录，含可逆性评估 | Nygard | 步骤 4 — 选定方案；步骤 6 — 编写文档 |
...
- UI 评审 返回 `需修改` 或 `阻塞`，需要按 findings 修订
- 设计阶段 已进入起草、需要并行起 UI 设计（parallel 默认模式）

不使用：

- 规格未声明 UI surface（API-only / 脚本 / 数据管道 / CLI / 纯后端）→ 本节点不激活，只走 设计阶段
- 规格仍是草稿/待批准 → 规格阶段 / 规格评审
- 架构/模块/API 契约/数据模型 → 设计阶段
- 设计已批准，需要任务计划 → 任务规划
- 只要求执行 UI 评审 → UI 评审
- 阶段不清或证据冲突 → 上游编排者编排者
...
- 设计阶段 当前最新稿（读 API 契约、错误模型、鉴权模型、状态形状；parallel 模式下可读草稿，标记"待 peer 锁定"条目）
...
产出：见下方 Output Contract。

**联合 approval 规则**：设计评审 与 UI 评审 **同时通过**后才能进入 `设计真人确认`；任一未过，另一方可继续稳定部分，但 approval step 不解锁。

## Hard Gates

- UI 设计未 UI 评审 通过前，不得拆解任务或编写前端实现代码
- 设计评审 与 UI 评审 双通过前，不发起 `设计真人确认`
- 未经 上游编排者 或 上游编排者 入口判断（含 UI surface 激活条件判定），不直接开始 UI 设计
- 规格未声明 UI surface 时不得以"反正要有界面"为由主动激活本节点；应先回 规格阶段 显式补齐 UI surface 声明
...
- 在文档中显式区分规格层（做什么）、UI 设计层（界面如何承载）、任务层（分步实施，属 任务规划）
...
- 不越界承担架构/API 契约决策（那是 设计阶段 的职责）；需要时在文档标注"依赖 设计阶段 锁定 <X>"
...
读取 项目级路径约定（若项目已声明）、feature `progress.md`（默认 `features/<active>/progress.md`）、已批准规格（默认 `features/<active>/spec.md`）相关部分、设计阶段 当前最新稿（若已有，默认 `features/<active>/design.md`）。
...
规格若缺以下信息，不假设默认：
- 会改变 UI surface 边界 / 验收标准 / 关键交互 / 目标设备 → 回到 上游编排者
...
选定后用 ADR 模板（见 `references/adr-template.md`，与 设计阶段 共用）记录关键决策：视觉方向、组件库选型、导航范式、布局范式、主题策略、动效策略等。
...
若是因 UI 评审 打回而重入：先读 findings → 修复阻塞问题 → 不重做未受影响的部分。
...
| 层级 | 组件 | 来源（复用 DS / 扩展 DS / 新增） | 依赖 token | 对应任务（待 任务规划 细化） |
|---|---|---|---|---|
...
明确区分规格层（做什么）、UI 设计层（界面如何承载）、任务层（分步实施，属 任务规划）。
...
- 与 设计阶段 的 peer 依赖交接块：本 UI 设计依赖对方锁定的 X/Y/Z；本 UI 设计已锁定并可供对方依赖的 A/B/C
- task planning readiness：哪些 UI 边界、组件粒度、状态矩阵已足以支撑后续任务规划
...
- [ ] 与 设计阶段 的 peer 依赖交接块已写明（依赖的、已锁的、冲突的）
- [ ] task planning readiness 已明确，不把未定 UI 硬推给任务规划
...
准备好后，启动独立 reviewer subagent 执行 UI 评审，不在父会话内联评审。
...
| ADR 模板 | `../设计阶段/references/adr-template.md` | 记录 UI 关键决策时（与 设计阶段 共用） |
...
- 把 设计阶段 的 API 契约/数据模型决策写进 UI 文档
...
- feature `progress.md` 更新：`Current Stage` → UI 设计；`Next Action Or Recommended Skill` → UI 评审

若 设计阶段 也在并行，feature `progress.md` 以最新进入的 skill 为 `Current Stage`，两条 skill 的 Next Action 分别登记在各自设计文档的状态字段中，由父会话/router 在联合 approval 时汇总。

推荐输出：

```markdown
UI 设计文档草稿已起草完成，下一步应派发独立 reviewer subagent 执行 UI 评审。
...
| "WCAG 2.2 AA 太严格，先按 A 写。" | Hard Gates: 无障碍基线为 WCAG 2.2 AA；降级会被 UI 评审 判 fail。 |
| "Atomic Design / Design Tokens 跳过。" | Hard Gates: IA + Atomic Design + Design Tokens + interaction state inventory 是 UI design 必需输出。 |
| "Heuristic Evaluation 和 review 一起做。" | Hard Gates (soul.md): 作者不能验收自己；启发式评估的 verdict 由 UI 评审 独立产出。 |

## Verification

- [ ] UI 设计草稿已保存到约定路径（非规格文件、非 设计阶段 文件、非任务文件）
