...
- 仍在做产品发现 / 决定要不要做这个 AR → 回需求负责人，df 不承担产品发现
...
| 用户意图 | 默认偏向 leaf |
|---|---|
| 澄清需求 / 整理 AR 规格 | `df-specify` |
| 评审需求规格 | `df-spec-review` |
| 写 / 修组件实现设计 | `df-component-design` |
| 评审组件实现设计 | `df-component-design-review` |
| 写 / 修 AR 实现设计（含测试设计章节） | `df-ar-design` |
| 评审 AR 实现设计 | `df-ar-design-review` |
| TDD 实现 / 改代码 | `df-tdd-implementation` |
| TDD 后审查测试用例有效性 | `df-test-checker` |
| C / C++ 代码检视 | `df-code-review` |
| 判断能否完成 / completion gate | `df-completion-gate` |
| 收口 / closeout / handoff | `df-finalize` |
| 紧急缺陷 / hotfix 复现与根因 | `df-problem-fix` |
...
| 文件 | 用途 |
|---|---|
| `skills/docs/df-workflow-shared-conventions.md` | df 工件路径、canonical 字段、handoff 字段、profile 与节点对应表 |
| `df-workflow-router/SKILL.md` | authoritative runtime routing |
| `docs/df-principles/00 soul.md` | df soul / 角色边界 |
| `docs/df-principles/04 workflow-architecture.md` | profile 与主链路 |
