...
HF workflow family 中 verify 阶段的 runtime evidence side node。对触碰前端表面的 active task，在 hf-test-driven-dev 的 GREEN 之后产生一份新鲜的浏览器运行时证据 bundle，供后续 batch hf-regression-gate / hf-completion-gate 消费。**不是 review，不是 gate，不签发 verdict**。
...
最小 smoke 选择规则：
- 页面 / route 任务：至少访问当前 task 触碰的 route，以及一个入口 route（通常 `/` 或导航入口）
- App 根组件 / provider / router / store 任务：至少访问一个依赖该 provider 的页面，并记录是否白屏
- 表单任务：至少覆盖空提交、明显无效输入、一次正常可达的提交前状态；不得使用真实敏感凭据
- API client / proxy / env 任务：至少触发一条关键网络请求，记录 expected host / actual URL / method / status
- 全栈任务：先记录服务启动 / health check，再跑一个 walking-skeleton 用户流
...
最低判读要求：
- body / root mount 为空、主要内容缺失或页面白屏 → `blocking` DOM observation
- uncaught exception、framework setup error、未处理 promise rejection → `blocking` console observation
- console error 不能被忽略；若判断为历史噪音，仍需记录为 observation 并说明 scope
- 关键 API 请求命中错误 host / port / path、connection refused、返回 HTML 但调用方期待 JSON、4xx/5xx 未被预期声明 → `major` 或 `blocking` network observation
- 表单找不到可提交控件、基础校验无法触发、提交导致白屏 → `blocking` 或 `major` observation
...
- canonical next action:
  - 0 blocking + 0 major → hf-workflow-router (hybrid batch decision)
...
- `references/playwright-smoke-example.cjs`：可复制到项目侧的最小 Playwright smoke 示例；仅作参考，不让 HarnessFlow 本体依赖 Playwright。
