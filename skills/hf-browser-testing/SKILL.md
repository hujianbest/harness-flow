...
description: Use when 实现层 TDD finishes GREEN on a frontend-touching active task whose spec declares a UI surface, and fresh browser runtime evidence (screenshot / console log / network trace) is required for downstream gates. Not for issuing verdicts (gates do that), not for replacing 测试评审's test-quality review, not for backend-only tasks.
---

# HF 浏览器端运行时证据节点

HF workflow family 中 verify 阶段的 runtime evidence side node。对触碰前端表面的 active task，在 实现层 TDD 的 GREEN 之后产生一份新鲜的浏览器运行时证据 bundle，供 回归门禁 / 完成门禁 消费。**不是 review，不是 gate，不签发 verdict**。

## When to Use

适用：

- 当前 active task 是前端 / UI 表面，且 spec 显式声明 UI surface（规格阶段 输出的 spec 中含 UI surface 声明，或 UI 设计 的设计已批准）。
- 实现层 TDD 已完成当前 task 的 GREEN，单元测试新鲜证据已在交接块中写回。
- 下游 gate（回归门禁 / 完成门禁）需要 runtime DOM / 控制台 / 网络层证据。

不适用：

- 后端 / CLI / 库类 task（无浏览器表面） → 跳过本 skill，直接由 实现层 TDD 交接到下游。
- spec 未声明 UI surface 且 UI 设计 也未启动 → 越权，回 上游编排者。
- 已经在审 verdict / 评测测试质量 → 测试评审。
- 回归证据汇集 / DoD 判定 → 回归门禁 / 完成门禁。
- 阶段不清 / 证据冲突 → 上游编排者编排者。
...
- 主链触发时，实现层 TDD 当前 task 的 GREEN 与新鲜单元证据必须先存在；缺位则不应启动本 skill。
...
- 不修改实现代码；如发现实现缺陷 → 写 finding 并 reroute 回 实现层 TDD，不在本 skill 内"顺手修一下"。
- 不修改测试代码；如测试覆盖不足 → 写 finding 并 reroute 回 测试评审 / 实现层 TDD。

## Object Contract

- Primary Object：当前 active task 的浏览器运行时 evidence bundle。
- Frontend Input Object：实现层 TDD 的 GREEN 交接块（含 SUT Form / Refactor Note / 单元测试 fresh evidence）+ 已批准 spec / ui-design / tasks 工件。
...
- Object Boundaries：不修改 spec / design / tasks / 实现代码 / 测试代码；不签发 verdict；不替代下游 gate 的 evidence bundle 汇集逻辑（回归门禁 仍会重新读 evidence 做影响面判断）。
...
读 `features/<active>/progress.md` 的 Current Active Task → 校验 实现层 TDD 是否完成 GREEN（交接块存在 + 单元 fresh evidence）→ 校验 spec / ui-design 是否声明 UI surface → 否则 reroute 回 上游编排者。

- Object：active task + 交接块。
- Method：evidence-based precondition check。
- Input：`features/<active>/progress.md`、最新 实现层 TDD 交接块。
...
- [observation] <现象简述>
  ...
  - suggested next: 实现层 TDD | 测试评审 | UI 评审 | gate
```
...
- canonical next action:
  - 0 blocking + 0 major → 回归门禁
  - any blocking → 实现层 TDD (with finding)
  - major-only → 测试评审 or UI 评审 (per suggested next)
```
...
| 借口 | 反驳 / Hard rule |
|------|-------------------|
| "单元测试都 GREEN 了，浏览器跑一下没必要。" | Hard Gates: spec 声明 UI surface 时本 skill 是 实现层 TDD 后的 evidence 必经路径；省略导致下游 gate 缺 evidence。 |
...
| "我看着没问题，verdict pass。" | Hard Gates (soul.md / Fagan): 本 skill 只产 observation，不签 verdict；verdict 由 回归门禁 / 完成门禁 决定。 |
| "我顺手把 console error 对应的 bug 修了。" | Hard Gates: 不修改实现 / 测试代码；发现缺陷写 finding 并 reroute 回 实现层 TDD。 |
