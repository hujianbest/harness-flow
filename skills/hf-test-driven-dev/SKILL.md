...
description: 适用于任务计划获批后的单任务实现、受控 hotfix 修复实现、review/gate 回流修订。不适用于任务计划未批准、hotfix 无复现路径、需并行多任务。
---

# HF 测试驱动开发与实现入口

HF workflow family 唯一实现入口。把单个活跃任务从"准备实现"推进到"已写回新鲜证据与 canonical 下一步"。不是任务循环控制器——跨 task 切换由 上游编排者 决定。
...
| **Escalation Boundary** | 跨 task 范围的结构性重构、ADR 变更、模块边界变更不在 task 内做，escalate 到 增量变更 | 项目化实践（HF 节点边界约定） | 步骤 4A — escalation 触发即停 task；步骤 5 — Refactor Note 标记 |
...
- 受控 hotfix 的修复实现
- 来自 test-review/code-review/traceability-review/regression-gate/completion-gate 的回流修订
- 用户要求"开始实现这个 active task"

不适用：任务计划未批准 → 回上游；hotfix 无复现路径 → 热修复；需并行多任务 → 上游编排者编排者。
...
- 若 worktree 隔离要求成立，动手前必须先准备 worktree
- Red-Green-Refactor 前必须完成测试设计确认
- 测试设计中必须声明 **SUT Form**（`naive` / `pattern:<战术模式名>` / `emergent`）；`pattern:<name>` 只允许承接已批准设计中列出的战术模式（Aggregate / ValueObject / Repository / DomainService / ApplicationService / DomainEvent），禁止引入 GoF 代码模式名
- 写回 fresh evidence 和稳定交接块前不得声称完成
- **Two Hats**：同一 RGR 步骤内不得混戴 Changer 与 Refactor 帽；GREEN 步内不做 cleanup；preparatory refactor 必须独立成步
- **Escalation Boundary**：跨 ≥3 模块的结构性重构 / 修改 ADR / 修改已批准模块边界或接口契约 / 引入设计未声明的新抽象层 → 立即停 task，把状态写入 Refactor Note 的 Escalation Triggers 段后退出本 skill，不在 task 内"先修着试试"
...
- `worktree-required`：按本 skill `references/worktree-isolation.md` 准备 worktree（若 项目声明项目等价路径，优先遵循）
- `in-place`：仅干净工作区时继续
...
- 以上之外的命名（`pattern:Strategy / pattern:Factory / pattern:Adapter / pattern:Observer / pattern:Decorator / pattern:Builder / pattern:Singleton` ...）→ 退回 `emergent`；这些 GoF 模式必须在 REFACTOR 步按 Fowler 重构词汇（Replace Conditional with Polymorphism / Extract Factory Method / ...）emergent 浮现。HF 立场（已内联 `设计阶段/SKILL.md` Methodology *Emergent vs Upfront Patterns*）：GoF 代码模式刻意不在 design 阶段前置决策，由 REFACTOR 步在 Two Hats 纪律下按需浮现
...
2. **Architectural Conformance Check**：对照已批准 设计阶段（默认 `features/<active>/design.md`）与相关 ADR（默认 `docs/adr/`），逐项确认本轮实现仍遵循：
...
4. **Escalation Decision**：对每条识别到的问题，按 `references/refactoring-playbook.md` 的 Escalation 边界归档：
   ...
   - **Required escalation**：触发 escalation 边界（跨 ≥3 模块 / 改 ADR / 改模块边界或接口契约 / 引入设计未声明的新抽象层）→ **立即停 task**，把当前状态写入 Refactor Note，handoff 上游编排者，由 router 路由到 增量变更 或 设计阶段
...
- Boy Scout Touches:
  - <文件:范围> — <清理类型>
- Architectural Conformance: <与 设计阶段 中依赖方向 / 模块边界 / 接口契约的一致性结论；偏离需写明理由与可追溯锚点>
- Documented Debt:
  - <smell 名> @ <影响范围> — <为什么不在本 task 内修>
- Escalation Triggers: <None | escalate to 增量变更 | escalate via 上游编排者>
...
- 出现 Escalation Triggers 非 `none` 时，本轮交接块应在 Pending Reviews And Gates 段标 `escalation pending`，并把 Refactor Note 写完整；后续路由由上游编排者根据 Escalation Triggers 内容决定。

**Verify 拐点（v0.2.0 / ADR-002 D7）**：当 spec 声明 UI surface 且当前 active task 触碰前端表面时，由 router 在 GREEN 之后插入 浏览器测试 作为 verify side node，产出 runtime evidence bundle（DOM / Console / Network 三层）；该节点**不签发 verdict**，仅产 observation 与 canonical next action。激活与回流规则见 `skills/agents/references/profile-node-and-transition-map.md` 的「浏览器测试 激活与回流」一节。本 skill 在 GREEN 交接块中不需要主动声明是否触发 浏览器测试，由 router 依据 spec / tasks 工件证据判断。
...
- **跨模块 / 改 ADR / 改模块边界的"顺手"重构在 task 内被默默做掉**，绕过 增量变更
...
| "顺手把跨 3 个模块的依赖结构改了，反正在改这块代码。" | Hard Gates (Escalation Boundary): 跨 ≥3 模块的结构性重构 / 修改 ADR / 修改已批准模块边界 → 立即停 task，回上游编排者；不在 task 内"先修着试试"。 |
...
- [ ] 触发 escalation 时已停 task，Next Action 指向 上游编排者，不强行进入 测试评审
