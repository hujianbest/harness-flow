...
HF workflow family 默认实现入口。把单个活跃任务从"准备实现"推进到"已写回新鲜证据与 canonical 下一步"。当 router 明确判定任务适合 fresh implementer subagent 时，可改用 `hf-subagent-driven-dev`；跨 task 切换与批量 review/gate 启动仍由 `hf-workflow-router` 决定。

三层职责：1) 默认实现入口 2) TDD 执行入口 3) 向 review/gate 输出证据与 handoff 的交接入口。
...
不适用：任务计划未批准 → 回上游；hotfix 无复现路径 → `hf-hotfix`；router 已证明适合 fresh implementer subagent → `hf-subagent-driven-dev`；需并行多任务 → `hf-workflow-router`。
...
- 当前任务完成 TDD 实现交接、wisdom delta 与 task-progress 同步前不得切换到下一任务；完整 review/gate verdict 可按 router 的 hybrid batch 规则延后到所有 approved tasks 实现完成后统一执行
- task 完成时必须写 `hf-wisdom-notebook` delta（5 文件容器存在 + 至少 learnings 或 verification 任一）+ 更新 `tasks.progress.json` step-level state；否则视为 task 未完成（v0.6 / FR-002 集成）
...
写回稳定交接块，并为本 task 的最终 summary 与 batch quality chain 提供输入。`hf-test-driven-dev` 不写 review / gate verdict，但必须给后续 batch-level thin verdict matrix 留下可引用的实现、测试和 evidence anchors。
...
- 与任务计划测试种子的差异:
- 剩余风险 / 未覆盖项:
- Batch Quality Status: pending
- Pending Reviews And Gates: <full/standard: hf-test-review, hf-code-review, hf-traceability-review, hf-regression-gate, hf-doc-freshness-gate, hf-completion-gate | lightweight: hf-regression-gate, hf-doc-freshness-gate, hf-completion-gate>
...
Next Action 用 canonical skill ID：主链实现交接完成后一律写 `hf-workflow-router`，由 router 判断 next-ready task vs batch quality chain；回流修订完成 → 写回触发回流的那个 node 或 `hf-workflow-router`（当需要重新做 batch decision）。不把下一任务的实现写成输出。

报告形态约束：本节点只写实现交接块与 task summary 输入，不为后续 batch review / gate 预创建长报告。通过态的人读入口应由 task summary + batch thin verdict matrix 聚合；详细报告只在后续 review / gate `需修改` / `阻塞` 或出现关键 finding 时展开。
...
- 先写实现再补失败测试
- 旧绿测结果当当前证据
- 实现交接块、wisdom delta、task progress 未同步就说"task 实现完成了"，或 batch quality chain 前就说"workflow 完成了"
...
- [ ] 实现交接块已写回，含 `Batch Quality Status` 与 canonical Next Action
