...
4. **Escalation Decision**：对每条识别到的问题，按 `references/refactoring-playbook.md` 的 Escalation 边界归档：
   ...
   - **Required escalation**：触发 escalation 边界（跨 ≥3 模块 / 改 ADR / 改模块边界或接口契约 / 引入设计未声明的新抽象层）→ **立即停 task**，把当前状态写入 Refactor Note，handoff 上游编排者，由 router 路由到 增量变更 或 `hf-design`
