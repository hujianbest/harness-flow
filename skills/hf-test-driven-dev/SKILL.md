...
HF workflow family 默认实现入口。把单个活跃任务从"准备实现"推进到"已写回新鲜证据与 canonical 下一步"。当 router 明确判定任务适合 fresh implementer subagent 时，可改用 `hf-subagent-driven-dev`；跨 task 切换仍由 `hf-workflow-router` 决定。
