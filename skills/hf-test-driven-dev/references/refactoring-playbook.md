...
源：Joshua Kerievsky, *Refactoring to Patterns*；Kent Beck, *TDD by Example* 后记；Martin Fowler, *Refactoring*。治理立场：`skills/principles/emergent-vs-upfront-patterns.md`。
...
| "sut_form 写 pattern:Strategy 比较清楚" | 不合法。sut_form 的 pattern:<name> allowlist 只含 design § 4.5 战术模式；GoF 模式只能作为 Pattern Actual 浮现结果出现，立场见 `skills/principles/emergent-vs-upfront-patterns.md` |
