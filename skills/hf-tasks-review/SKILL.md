...
description: 适用于任务计划草稿需正式 review verdict、任务计划被退回需复审、或用户显式要求评审任务计划的场景。不适用于写/修任务计划、阶段不清。
---
...
- route 明确但缺稳定任务计划 → 写最小 blocked record，下一步 任务规划
- precheck 通过 → 继续正式审查
...
- `需修改`：findings 可定向修订 → `next_action_or_recommended_skill=<task-planning-skill>`，`needs_human_confirmation=false`
- `阻塞`：核心任务结构有问题/findings 无法定向回修 → `next_action_or_recommended_skill=<task-planning-skill>`，`needs_human_confirmation=false`；若问题本质是 route/stage/profile/上游证据冲突 → `next_action_or_recommended_skill=<orchestrator>`，`reroute_via_orchestrator=true`
...
回传结构化摘要时遵循当前 skill pack 中 `agents/references/reviewer-return-contract.md`：`next_action_or_recommended_skill` 只写一个 canonical 值；`通过` 时设 `needs_human_confirmation=true`；workflow blocker 必须显式写 `reroute_via_orchestrator=true`。
...
| "依赖图缺 1-2 条边没关系。" | Workflow stop rule: 依赖图缺边会让 实现层 TDD 在错误任务上启动；任何缺失都必须落 finding。 |
