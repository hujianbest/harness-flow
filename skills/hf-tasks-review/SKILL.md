...
6 维度 0-10 评分：任务可执行性、任务合同完整性（含 UI Implementation Contract 承接）、验证与测试设计种子（含 runtime / UI conformance evidence tier）、依赖/顺序正确性、追溯覆盖度、router 重选就绪度。任一关键维度 < 6 不得通过。
