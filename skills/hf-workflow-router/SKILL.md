...
### 4A. 决定 Design Stage 是否含 UI Surface（full profile 主链进入 design stage 时）

主链从 `规格真人确认` 完成进入 design stage 时（full profile），router 还需判断是否激活 `hf-ui-design` 这个 **conditional peer skill**。

支线回流（`hf-hotfix` / `hf-increment` 带入的 UI 层修订）另见 `references/ui-surface-activation.md` §"回流与支线交互"，在 lightweight / standard profile 下也可能触达 `hf-ui-design`，不受本节"full profile 主链"限定排除。
