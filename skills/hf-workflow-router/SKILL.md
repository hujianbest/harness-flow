...
只读路由所需的最少内容：项目级路径约定（若项目已声明）、用户请求、工件状态、顶层导航（档 2 优先 `docs/index.md`，档 0/1 用仓库根 `README.md`，缺失时扫 `features/` 兜底）、`features/<active>/progress.md`、`features/<active>/reviews/` 与 `features/<active>/verification/` 等 review/verification artifacts。不先做大范围代码探索。**read-on-presence**：`docs/` 中未启用的可选资产缺失不阻塞路由。证据冲突时按未批准处理、选择更上游节点、必要时升级 profile。
...
判定：先执行 项目级强制规则（若项目已声明） → 沿用已有 profile → 按证据选择 → 冲突选更重。只允许升级，不允许降级。
...
与 Profile 正交，不混写成复合值。归一化顺序：用户显式要求 → 项目级默认 → 已有值 → 默认 `interactive`。
...
可选 coordination state。读取 feature `progress.md`（默认 `features/<active>/progress.md`）已有值 + 项目约定 + 当前请求类型。

决策：已有 worktree-active 且路径一致 → 沿用；进入 `hf-test-driven-dev` / `hf-subagent-driven-dev` 且命中 full/standard/代码修改/脏状态 → `worktree-required`；仅 lightweight+干净 → `in-place`。不静默降级。

参考：`hf-test-driven-dev/references/worktree-isolation.md`（实现侧权威指南）；若 项目声明项目等价路径，优先遵循。要点摘要：`Workspace Isolation` ∈ `in-place` / `worktree-required` / `worktree-active`；`worktree-required` 必须在进入实现前完成 worktree provisioning；`worktree-active` 下游节点必须复用 `Worktree Path`，不得静默退回主工作区；review dispatch 携带 `Worktree Path` / `Worktree Branch`；closeout 不擅自删除 worktree，只记录 disposition。
...
路由原则：支线优先于主链 → review/gate 恢复优先于实现 → 缺失上游优先于下游 → 冲突选更保守 → 实现交接后的 task reselection 优先于 batch quality chain → completion 后的 batch scope 确认优先于 finalize。
...
实现节点选择：默认仍为 `hf-test-driven-dev`；当 approved task 可被完整封装给 fresh implementer subagent、上下文路径可读、且不存在强耦合/并行任务需求时，可选择 `hf-subagent-driven-dev`。若 eligibility 不清，选更保守的 `hf-test-driven-dev`。
...
关键分支：
- `hf-test-driven-dev` / `hf-subagent-driven-dev` GREEN 后：若当前 task / 代码影响面触碰前端运行面（route、App 根组件、UI provider、API client、Vite proxy/env、表单、浏览器存储等），且工件已声明 runtime surface / DoD → 激活 `hf-browser-testing`；若代码事实触碰运行面但 spec / tasks 未声明 → 回上游补工件，不静默跳过。runtime side evidence 完成后仍回 router 做 hybrid batch decision。
- 实现交接完成后：把当前 task 标记为 `implemented-pending-batch-quality`，若存在唯一 approved 且 dependency-ready 的下一 task，则更新 `Current Active Task` 并进入合格实现 leaf；若没有剩余 ready / pending task，则进入当前 profile 的 batch quality chain（full/standard 从 `hf-test-review` 开始，lightweight 从 `hf-regression-gate` 开始）
- `conclusion=通过` + `needs_human_confirmation=true`：interactive 等待/auto 写 record 再继续
- completion gate 通过后：确认 batch scope 覆盖所有 approved tasks 且无剩余 ready / pending task → `hf-finalize`；若发现新 task、batch scope 不完整或任务证据冲突 → `hf-workflow-router`
- task summary / thin verdict blocks 只改变人读入口，不改变 router 消费 verdict、record_path、next action 的结构化规则
- 用户提出新范围/缺陷 → 重新判断支线

参考：`references/review-dispatch-protocol.md`、`references/reviewer-return-contract.md`

### 9. review 节点派发 `hf-reviewer`

不在父会话内联 review。构造最小 review request，带入 Workspace Isolation 上下文，派发 `agents/hf-reviewer.md` 定义的独立 subagent，消费结构化 summary。

### 10. 连续执行与暂停点

路由结论不是独立用户交互：非暂停点 → 同一轮进入目标 skill；review 节点 → 立刻派发；approval step → 按 Mode 处理；task reselection / batch quality chain 启动 → 同一轮继续。只有 hard stop 才等待。
...
- 父会话内联执行 review，而不是派发 `hf-reviewer`
- profile 不再成立却不升级
...
| `references/review-dispatch-protocol.md` | `hf-reviewer` 派发协议 |
| `references/reviewer-return-contract.md` | reviewer 返回结果契约 |
| `../hf-subagent-driven-dev/references/implementer-return-contract.md` | implementer subagent 返回状态契约 |
...
- [ ] 实现交接后已做 task reselection 或进入 batch quality chain；completion gate 后已确认 batch scope 再进入 finalize
