...
激活判定实际上是三向的，必须在同一视图下可见：

| 情形 | 命中条件 | 决策 |
|------|---------|------|
| 激活 | "激活信号矩阵" 任一命中，且 "不激活信号矩阵" 未命中 | 激活 `hf-ui-design`，按 Design Execution Mode 起草 |
| 不激活 | "不激活信号矩阵" 全部命中（或规格显式"不含 UI surface"） | design stage 只走 `hf-design` 单节点 |
| 含糊回退 | 两矩阵均未显著命中（既无 UI surface 声明也无 API-only 声明） | 回 `hf-specify` 补齐 UI surface 声明，不自行判定 |

不得用"默认激活"或"默认不激活"兜底含糊情况；含糊等价于规格不完整。
...
规格明确"复用既有 API / 不改契约 / 仅新增前端"时：

- 仍落在 **parallel**，不强行切 ui-first（因为没有"倒逼后端"的必要）
- 但 router 应在路由输出中显式说明 **`hf-design` 职责范围收敛**：客户端状态映射 / 错误模型复核 / i18n 数据结构 / Design System 集成点；不展开全新架构
- `hf-ui-design` 承担绝大多数工作量；peer 依赖交接块仍需保留，便于 `hf-design` 对前端需要的后端契约做 readiness 声明
