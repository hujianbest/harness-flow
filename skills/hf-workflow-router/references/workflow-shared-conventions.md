...
HF 把 4 条 LLM coding 行为准则（Think Before Coding / Simplicity First / Surgical Changes / Goal-Driven Execution）作为**所有 `hf-*` skill 共同遵守的横切行为基线**，而不是一个独立的 workflow 节点。每个 skill 在自己的 `SKILL.md` 中已经吸收并体现：

1. **Think Before Coding**：进入实现前显式列出假设、未知、待澄清，不静默选定方向；多解读时回到 user 或 router。
2. **Simplicity First (YAGNI)**：只解当前轮已确认需求；"未来可能需要"显式标记为过度工程候选。
3. **Surgical Changes**：变更范围锁定到当前 task / artifact，不顺手扩展模块边界、不旁路修改无关工件。
4. **Goal-Driven Execution**：始终对齐"从 idea 到产品高质量落地"；fresh evidence 优先于"看起来在动"。
...
| `Batch Quality Scope` | 已完成实现交接、等待统一 review/gate 的 task 集合 | 仅列 task ID 与状态；每个 task 必须有可回读 handoff / evidence anchor |
...
- 若项目必须使用别名，应在项目级约定中声明一对一映射，并保证 router（及读入时的 **legacy 合并路由** 别名）可唯一归一化
...
### Optional coordination field: `Batch Quality Scope`

说明：

- hybrid batch quality 下，每个 task 完成 TDD 实现交接后先进入 `Batch Quality Scope`，状态写作 `implemented-pending-batch-quality`
- 该字段不替代 review / gate verdict；它只声明后续 batch quality chain 必须覆盖哪些 task
- 进入 batch quality chain 前，router 必须能从 task summaries、implementation handoffs、`tasks.progress.json` 与 task board 恢复每个 task 的 evidence anchors
- 若 `Batch Quality Scope` 与 task board / tasks plan 冲突，或任一 task 缺 handoff / wisdom delta / fresh evidence，回到 `hf-workflow-router` 或对应实现 leaf，不得开始批量 review
...
- 共享规则以 `hf-test-driven-dev/references/worktree-isolation.md` 为准；若 项目声明项目等价路径，优先遵循
...
- 推荐优先级：用户当前请求中的显式模式 >项目级约定中的默认策略 > 已存在 progress state 中的值
...
如果项目需要英文映射，可在项目级约定中声明 `pass` / `revise` / `blocked` 的等价词，但 live HF family 文档应保持 verdict vocabulary 稳定。
...
- `hf-test-driven-dev` / `hf-subagent-driven-dev`：需要 fresh RED / GREEN evidence
...
## Task Summary And Thin Verdict Blocks

### 目标

当一个 feature 被拆成多个 task 时，默认人读入口应是一份 task-level summary + 一份 batch-level thin verdict matrix，而不是每个 review / gate 节点各产一份长报告。

约束：

- **不压缩判定事实**：review / gate 仍必须产出独立 verdict、唯一 next action、record path 与必要 evidence anchor；hybrid batch 只改变 scope 与时机，不删除 verdict。
- **压缩报告形态**：`通过` 且无关键 finding 时，默认写 thin verdict block，并把它聚合到 batch quality matrix，同时由各 task summary 索引。
- **异常才展开**：`需修改` / `阻塞`、存在 `critical` / `important` finding、证据缺口、环境阻塞、route/profile 冲突时，才写详细诊断段或独立详细报告。
- **summary 不是 verdict 替代物**：task summary 只索引与聚合 verdict block；不能用自然语言“总体看通过”替代 reviewer / gate 的结构化结论。

### Task Completion Summary 最小字段

建议路径：`features/<active>/summaries/task-<TASK-ID>.md`。项目若已有等价路径，可在项目约定中覆写。

最少包含：

| 字段 | 内容 |
|---|---|
| `Task ID` | 当前 `Current Active Task` |
| `Scope` | 本 task 承诺完成的垂直切片 / DoD |
| `Changed Artifacts` | 触碰代码、测试、文档、验证工件 |
| `TDD Evidence` | RED / GREEN / REFACTOR 摘要与 evidence path |
| `Batch Quality Status` | `pending` / `covered-by-batch:<record_path>` / `requires-rework:<finding-id>` |
| `Thin Verdict Matrix` | batch-level test-review / code-review / traceability-review / regression-gate / doc-freshness-gate / completion-gate 的 conclusion + record_path |
| `Key Findings` | 只列仍需人读的 finding；通过态无关键 finding 写 `none` |
| `Remaining Risk` | 未覆盖或降级验证项；无则写 `none` |
| `Next Task Decision` | `unique next-ready task` / `enter batch quality chain` / `blocked: ambiguous next task` |

### Thin Verdict Block schema

通过态 review / gate 默认写 3-8 行：
...
- record_path:
- evidence_anchor:
- key_findings: none | [finding-id...]
```

不允许省略 `record_path`；不允许把多个 next action 合并成自然语言。
...
- implementation and quality: `hf-test-driven-dev` / `hf-subagent-driven-dev` / `hf-browser-testing` / `hf-test-review` / `hf-code-review` / `hf-traceability-review` / `hf-regression-gate` / `hf-doc-freshness-gate` / `hf-completion-gate` / `hf-finalize`
...
- `docs/` 放**项目长期资产**（跨 feature 周期、慢演化）：principles / adr / architecture / arc42 / runbooks / slo / postmortems / release-notes / insights / index 等子目录
- `features/<NNN>-<slug>/` 放**单 feature 周期内的过程交付件**：spec / design / tasks / progress / reviews / approvals / verification / evidence / closeout 等

`<NNN>` 为三位顺序号（`001` 起，仓库级唯一、不复用）；`<slug>` 为 kebab-case 短主题名。`<active>` 在 workflow 周期开始时由 router 锁定为具体 feature 目录名。

除非 项目已声明等价路径，否则默认推荐（`tier` 标记对应"档 0 / 档 1 / 档 2"的最小 `docs/` 启用范围；档 0 = 不可省，档 1 = 推荐起步，档 2 = 按需启用）：
...
| 项目自身原则锚点 | `docs/principles/`（如 `architecture-principles.md` / `product-principles.md`，由项目自定义） | 可选 |
...
`<kind>` 受控词表：`spec` / `design` / `ui` / `tasks` / `code` / `test` / `traceability` / `discovery`（review）；`spec` / `design` / `tasks`（approval）；`regression` / `completion` / `hotfix`（verification）。
...
feature 周期内对 `docs/` 中长期资产的修改按以下时机划分。所有同步遵循 **read-on-presence + sync-on-presence** 原则：HF skill 读取 `docs/` 资产时，缺失视为"未启用"而非阻塞；`hf-finalize` 同步时只对**已存在**的子目录或本 feature 触发了升级条件的子目录做同步。
...
- 若项目通过项目级声明映射了等价路径，仍应保持这些最小语义可回读
...
对 `hf-test-driven-dev`、`hf-subagent-driven-dev`、`hf-regression-gate`、`hf-completion-gate` 这类非 reviewer 节点，回流约定统一为：
...
- 若项目必须保留别名，必须在项目级约定中声明映射

## Practical Checklist

当你在编写或审阅某个 `hf-*` skill 时，至少确认：

- [ ] 该 skill 行为遵守 4 条横切行为基线（Think Before Coding / Simplicity First / Surgical Changes / Goal-Driven Execution），且没有把它们写进 canonical schema 或 next-action vocabulary
