...
当一个 feature 被拆成多个 task 时，默认人读入口应是一份 task-level summary，而不是每个 review / gate 节点各产一份长报告。

约束：

- **不压缩判定事实**：review / gate 仍必须产出独立 verdict、唯一 next action、record path 与必要 evidence anchor。
- **压缩报告形态**：`通过` 且无关键 finding 时，默认写 thin verdict block，并把它聚合到 task summary。
...
| `Thin Verdict Matrix` | test-review / code-review / traceability-review / regression-gate / doc-freshness-gate / completion-gate 的 conclusion + record_path |
| `Key Findings` | 只列仍需人读的 finding；通过态无关键 finding 写 `none` |
| `Remaining Risk` | 未覆盖或降级验证项；无则写 `none` |
| `Next Task Decision` | `unique next-ready task` / `no remaining task` / `blocked: ambiguous next task` |
...
- implementation and quality: `hf-test-driven-dev` / `hf-subagent-driven-dev` / `hf-browser-testing` / `hf-test-review` / `hf-code-review` / `hf-traceability-review` / `hf-regression-gate` / `hf-completion-gate` / `hf-finalize`
