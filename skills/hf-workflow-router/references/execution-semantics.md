...
任务边界本身也不是默认暂停点：若当前任务刚完成实现交接（含 RED/GREEN、Refactor Note、wisdom delta 与 task progress），且 router 能唯一锁定下一个 `Current Active Task`，则应在同一轮继续进入合格实现 leaf（默认 `hf-test-driven-dev`，subagent eligible 时可为 `hf-subagent-driven-dev`），而不是把“一个任务实现完”误当成自然暂停。

`Build one task` 是实现原子边界，不是 build 会话的停止条件。build 会话应按“单 task 实现交接 → task summary 输入 → router 选择唯一 next-ready task → 下一轮合格实现 leaf”循环；只有 approval step、hard stop、下一任务不唯一或无剩余任务时才停止实现循环。无剩余任务不是 finalize 条件，而是进入 batch quality chain 的条件。

task summary 不是 verdict，也不是人工确认点；它先索引本 task 的实现证据与 batch pending 状态，等 batch quality chain 完成后再聚合 thin verdict blocks 与 batch conclusion。
...
7. **下一个任务不唯一或 ready 判定冲突**：实现交接已完成，但剩余任务候选不唯一、依赖状态冲突，或 task board / 任务计划无法稳定判断唯一 `next-ready task`
...
- 当前任务实现交接完成后，若 router 能唯一锁定 `next-ready task`，则回 router 并在同一轮继续进入合格实现 leaf
- 当前 task summary 已索引实现证据与 batch pending 状态，且下一任务唯一；summary 只是人读入口，不构成人工确认点
- 所有 approved tasks 均实现完成后，进入 batch quality chain 的首个 review/gate 节点；这不是人工确认点
- 恢复编排协议判断出唯一下一推荐节点时
...
- **下一任务选择歧义**：当前任务虽已完成实现交接，但 approved / ready 的剩余任务不唯一，或依赖状态与 `Current Active Task` 投影冲突；此时停止自动推进，报告缺少的决定性证据
...
- 需要进行 profile 升级
- 当前任务实现交接刚完成，需要判断是重选下一任务还是进入 batch quality chain
- `hf-completion-gate` 已通过，需要确认 batch scope 是否覆盖所有 approved tasks，再进入 `hf-finalize`
