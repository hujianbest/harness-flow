...
description: 适用于 regression gate 之后需判断任务完成走向、确认任务可宣告完成、用户要求\"能不能算完成\"的场景。不适用于缺回归记录/实现交接块需补齐、需状态收尾、阶段不清。
---
...
不适用：缺 regression 记录/实现交接块 → 补齐；需回归验证 → 回归门禁；需状态收尾 → 收尾归档；阶段不清 → 上游编排者编排者。
...
- 上游结论缺失或 route/stage/profile/任务范围冲突 → `阻塞`，下一步 上游编排者
- worktree-active 但 completion evidence 无法锚定同一 `Worktree Path` → `阻塞`，下一步 完成门禁
- precheck 通过 → 继续执行验证
...
### 6.1 文档新鲜度门禁 verdict 在 evidence bundle 中的承接（Phase 0 / ADR-0003）

文档新鲜度门禁 是 router 主链上位于 回归门禁 之后、本节点之前的独立 gate（ADR-0003 / `skills/文档新鲜度门禁/SKILL.md`）。其 verdict 路径必须作为 completion evidence bundle `Upstream Evidence Consumed` 段的一项被显式 reference。承接规则：
...
- **`blocked` verdict**：本节点**不**应消费此 verdict——`文档新鲜度门禁=blocked` 时由 router 直接路由回 `实现层 TDD`（或 增量变更 / 追溯评审 / 上游编排者，按 doc-freshness gate 自身的 next 表）；本节点不会被进入。如果父会话误把 blocked verdict 强行带入本节点 evidence bundle，本节点应判 `阻塞` + next = 上游编排者 让 router 重派
- **未通过 doc-freshness gate 的场景**：若 `Upstream Evidence Consumed` 中找不到 doc-freshness verdict 路径（既不是 pass/partial/N/A，又不是 blocked，而是缺失），说明 router 路径异常，本节点判 `阻塞` + next = 上游编排者
...
|---|---|---|---|
| 本轮没有 fresh verification evidence，也没运行能直接证明 completion claim 的命令 | `需修改` | 实现层 TDD | `record_path`、缺失的 fresh evidence、需要重跑的验证命令 |
| 声称“刚跑过且全绿”，但只有口头陈述，或终端 / 输出记录已不可核实 | `阻塞` | 完成门禁 | `record_path`、不可核实原因、需要重新生成的验证输出 |
| review 都过了，但本轮没运行能直接证明 completion claim 的命令 | `需修改` | 实现层 TDD | `record_path`、缺失的验证命令、为什么 review 不能替代 verification |
| 验证命令有失败项，或结果不能直接支持 completion claim | `需修改` | 实现层 TDD | `record_path`、失败摘要、未满足的完成条件 |
| 强制验证步骤因环境 / 工具链问题未完成，且 项目约定 / DoD 无降级许可 | `阻塞` | 完成门禁 | `record_path`、阻塞原因、未覆盖区域、恢复后需重跑什么 |
| 当前任务证据充分，但 next-ready task 候选不唯一，或 ready 判定冲突 | `阻塞` | 上游编排者 | `record_path`、候选任务清单、冲突证据、为什么本 skill 不能替 router 选任务 |
| 当前任务证据充分，且仍有唯一 next-ready task | `通过` | 上游编排者 | `record_path`、completion claim、evidence bundle、`Remaining Task Decision=唯一 next-ready task` |
| 当前任务证据充分，且已无剩余 approved tasks | `通过` | 收尾归档 | `record_path`、completion claim、evidence bundle、`Remaining Task Decision=无剩余任务` |
...
- `auto`：若 remaining-task 证据不唯一，直接 `阻塞` 并回 上游编排者
- 若输出不能映射到上表中的一行，说明 verdict 还没收敛好，不能返回

### 7. 门禁判断

- 证据支持完成 + 有唯一 next-ready task → `通过`，下一步 上游编排者
- 证据支持完成 + 无剩余任务 → `通过`，下一步 收尾归档
- 证据不足/仍需实现 → `需修改`，下一步 `实现层 TDD`
- 环境/工具链问题 → `阻塞`，下一步 完成门禁
- 上游编排/profile/证据链问题，或剩余任务不唯一 → `阻塞`，下一步 上游编排者
...
| 文件 | 用途 |
|------|------|
| `references/verification-record-template.md` | completion verification record 模板（与 回归门禁 同形态） |
...
| 借口 | 反驳 / Hard rule |
|------|-------------------|
| "DoD 大部分满足就放行，剩下几条 finalize 阶段补。" | Hard Gates: DoD 是闭合判据；任一未满足即判 fail，不延后到 收尾归档。 |
| "regression-gate 没跑，我替它默认 pass。" | Hard Gates: 上游 gate evidence 必须存在；缺位 → reroute 回 回归门禁 / 文档新鲜度门禁。 |
