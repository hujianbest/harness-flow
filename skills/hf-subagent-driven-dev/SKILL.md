...
description: 适用于已批准 task plan 中单一活跃任务可被完整封装给 fresh implementer subagent 的实现场景。不适用于任务强耦合、缺少可读上下文、需并行多任务、或 reviewer/gate verdict 可被跳过的场景。
---

# HF Subagent-Driven Development

HF 的可选实现 leaf：父会话保持 controller 角色，把一个已锁定 `Current Active Task` 封装给 fresh implementer subagent 执行，并在实现完成后回到既有 HF review/gate 链。它不替代 `hf-test-driven-dev` 的 TDD 纪律，也不改变 Fagan review、gate、approval 或 closeout 的 verdict 责任。
...
| **Controller / Worker Separation** | 父会话只负责编排、补上下文、消费状态；实现由 implementer subagent 完成 | 项目化实践（role separation） | 步骤 1-6 |
| **TDD Delegation** | implementer subagent 必须遵守 HF TDD、fresh evidence、Two Hats 与 Refactor Note 纪律 | `hf-test-driven-dev` | 步骤 3-4 |
| **Status-Driven Recovery** | implementer 返回结构化状态，父会话按状态补上下文、升级模型、拆任务或停下 | Superpowers implementer status contract | 步骤 4 |
...
- implementer 要并行处理多个任务 → 回 `hf-workflow-router` 重新切任务
- 用户要求 review/gate skip、作者自审、或跳过 RED/GREEN evidence → 停下并回对应 review/gate
- hotfix 无复现路径 → `hf-hotfix`

## Hard Gates

- 只允许一个 `Current Active Task`；不得并行派发多个 implementer subagent 写代码。
- 父会话必须提供完整 task text 与最小必要上下文；不得让 subagent 自行从大型 plan 中猜任务边界。
- implementer subagent 必须遵守 `hf-test-driven-dev` 的测试设计、RED/GREEN、Two Hats、Refactor Note、wisdom delta 与 `tasks.progress.json` 规则。
- implementer 的 self-review 不能替代 `hf-test-review`、`hf-code-review`、`hf-traceability-review` 或任何 gate verdict。
- implementer 返回 `NEEDS_CONTEXT` / `BLOCKED` 时，父会话必须改变输入、拆任务、升级 route 或停下；不得原样重试。
...
- **Backend Output Object**: implementer return summary + 实现交接块 + fresh evidence anchors + canonical next action。
- **Object Transformation**: 父会话把 approved task 封装成 bounded prompt；implementer 在隔离上下文中完成 TDD；父会话按状态推进或回修。
...
### 2. 构造 implementer dispatch request
...
- 必须遵守的 `hf-test-driven-dev` TDD 与 Refactor Note 规则
- 期望返回状态：`DONE` / `DONE_WITH_CONCERNS` / `NEEDS_CONTEXT` / `BLOCKED`

### 3. 派发 fresh implementer subagent

subagent 在给定 worktree 中实现、测试、提交或准备可提交 diff，并写实现交接块。它可以提问；父会话必须回答后用更新后的 request 重新派发，而不是让 subagent 猜。

### 4. 消费 implementer status
...
确认实现交接块包含 RED/GREEN evidence、Refactor Note、wisdom delta、`tasks.progress.json` 更新、changed artifacts、remaining risk、pending reviews/gates 与 canonical next action。缺任一项则回 implementer 修订。

### 6. 回到 HF review/gate 链

full/standard profile 通常 `Next Action Or Recommended Skill = hf-test-review`；lightweight profile 通常进入 `hf-regression-gate`。若触碰 runtime surface，仍由 router 判定是否插入 `hf-browser-testing`。后续 review/gate 均按既有 independent reviewer / gate contract 执行。

## Output Contract

| 输出 | 默认位置 / 形式 | 要求 |
|---|---|---|
| implementer return summary | 父会话消费的结构化摘要 | 状态、changed artifacts、evidence anchors、concerns/blockers |
...
- 派发多个 implementer subagent 同时修改代码
- 让 implementer 自己读完整 plan 并挑任务
- implementer 说 `BLOCKED` 后原样重试
- `DONE_WITH_CONCERNS` 未处理就进入 review
- implementer self-review 被当成 code review verdict
...
| 借口 | 反驳 / Hard rule |
|------|-------------------|
| "subagent 已经 self-review 了，可以跳过 code review。" | Hard Gates: implementer self-review 不是 Fagan review；仍必须进入 `hf-code-review` 或 profile 对应 gate。 |
| "任务都在同一个 plan 里，派几个 subagent 并行更快。" | Hard Gates: HF 一次只有一个 `Current Active Task`；并行实现会破坏 task 边界与 evidence recovery。 |
| "subagent 说缺上下文，但我可以让它先猜。" | Status-driven recovery: `NEEDS_CONTEXT` 必须补上下文后重派；猜测会污染实现与验收边界。 |
...
- [ ] implementer 使用 fresh context，未继承父会话历史
- [ ] implementer status 已按 return contract 消费
- [ ] RED/GREEN evidence、Refactor Note、wisdom delta、`tasks.progress.json` 均已同步
- [ ] 未把 self-review 当成 review/gate verdict
