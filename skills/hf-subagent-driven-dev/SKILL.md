...
HF 的可选实现 leaf：父会话保持 controller 角色，把一个已锁定 `Current Active Task` 封装给 `hf-implementer` 执行，并在实现完成后回到 router 做 hybrid batch decision。它不替代 `hf-test-driven-dev` 的 TDD 纪律，也不改变 Fagan review、gate、approval 或 closeout 的 verdict 责任。
...
- 用户要求 review/gate skip、作者自审、或跳过 RED/GREEN evidence → 停下并回 router；hybrid batch 只延后 review/gate，不删除 verdict
...
- `hf-implementer` 的 self-review 不能替代后续 batch quality chain 中 `hf-reviewer` 执行的 `hf-test-review`、`hf-code-review`、`hf-traceability-review` 或任何 gate verdict。
...
- **Backend Output Object**: `hf-implementer` return summary + 实现交接块 + fresh evidence anchors + batch pending status + canonical next action。
- **Object Transformation**: 父会话把 approved task 封装成 bounded prompt；`hf-implementer` 在隔离上下文中用 `hf-test-driven-dev` 完成 TDD；父会话按状态回到 router 做 next-ready task vs batch quality chain 决策。
- **Object Boundaries**: 本 skill 不写 review/gate verdict，不选择下一任务，不修改上游 spec/design/tasks，不替代 `hf-ultrawork` 的 fast-lane audit。
- **Object Invariants**: task ID 不变；worktree path 不变；review/gate verdict 不被跳过；所有完成声明必须可从仓库工件恢复。
...
### 6. 回到 router 做 hybrid batch decision

实现交接完成后，下一步固定回 `hf-workflow-router`。router 若发现唯一 next-ready task，则进入下一实现 leaf；若无剩余 approved task，则启动当前 profile 的 batch quality chain。full/standard profile 的 batch chain 从 `hf-test-review` 开始，lightweight profile 从 `hf-regression-gate` 开始。若触碰 runtime surface，仍由 router 判定是否在 batch decision 前插入 `hf-browser-testing`。
...
| batch quality pending status | progress / task summary | task 被纳入 `Batch Quality Scope`，后续由 batch review/gate 覆盖 |
...
| 借口 | 反驳 / Hard rule |
|------|-------------------|
| "`hf-implementer` 已经 self-review 了，可以跳过 `hf-reviewer`。" | Hard Gates: implementer self-review 不是 Fagan review；hybrid batch 结束时仍必须由 `hf-reviewer` 执行 `hf-test-review` / `hf-code-review` / `hf-traceability-review` 或 profile 对应 review。 |
...
- [ ] canonical next action 回到 `hf-workflow-router`
- [ ] self-review 未被当成后续 batch review/gate verdict
