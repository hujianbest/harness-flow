...
通读被评审工件与其上游(评设计要读规格,评代码要读设计与规格)。读 `progress.md` 的"已加载扩展",把各扩展 SKILL.md 中声明的对应阶段评审检查项**并入本轮 checklist**。逐条过 checklist,每发现一个问题记一条 finding:
...
- `通过` → 规格与设计评审在 `interactive` 模式下向用户展示 1-2 句结论摘要并等待确认;`auto` 模式下自动确认。**确认必须落盘**:在对应 `reviews/*.md` 追加一行 `- 用户确认: <日期>`(auto 模式写 `- 用户确认: auto-approved <日期>`),这是下一阶段"已批准"判定的依据。代码评审通过后进入 `hf-ship`。
