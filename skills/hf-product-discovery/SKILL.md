...
description: 适用于仍在判断产品问题、目标用户、wedge、关键假设或 probe 方向，尚未收敛到 formal spec 的场景。不适用于已明确进入 spec/design/tasks、或只需评审已有 discovery 草稿。
---

# HF 产品发现

把模糊的产品想法收敛成一份可评审的 discovery 草稿，明确问题、用户、wedge、关键假设与进入 formal spec 前仍需验证的事项。本 skill 不写正式规格，不替代 规格阶段。
...
| **Assumption Surfacing** | 显式写出已确认事实、未确认假设和 open questions，为后续 规格阶段 提供稳定 bridge | 项目化实践（HF 上游证据约定） | 步骤 3；discovery 正文 section 5–6 / 13；Bridge to Spec |
...
不适用：
- 已明确要写 formal spec / design / tasks → 规格阶段 / 对应下游 skill
- 只需评审已有 discovery 草稿 → 产品发现评审
- route / stage / 证据冲突，需要 authoritative 判断 → 上游编排者编排者
...
若候选 opportunity / solution ≥ 2 个且仅凭 MoSCoW 难以收敛，引入 **RICE / ICE** 做量化辅助（见 `references/prioritization-quant.md`）；分数必须带来源，严禁纯凭直觉。低 confidence 的候选不进入 wedge 候选，而是转成假设交给 实验探针。
...
这些度量直接成为 `Bridge to Spec` 的上游输入，对应后续 规格阶段 正文中的 Success Criteria / Success Metrics。
...
- 已明确 `Bridge to Spec` 中哪些结论可进入 规格阶段
- 若存在高风险、低 confidence 的核心假设，已标注"建议走 实验探针 先验证"

## Output Contract

完成时产出：
- discovery 草稿（默认路径：`docs/insights/YYYY-MM-DD-<topic>-discovery.md`；若 项目级覆盖，优先遵循）
  - discovery 阶段 feature 尚未创建，因此 discovery 草稿落到 `docs/insights/` 长期资产目录；只有当 discovery 通过评审并决定推进为 feature 时，才由规格阶段 创建 `features/<NNN>-<slug>/`
- 文档中明确的 spec bridge 小节，作为后续 规格阶段 起草 `features/<NNN>-<slug>/spec.md` 的输入
...
- 高风险假设 confidence 很低却仍被直接写进 candidate wedges，而不是转成 实验探针 假设
...
| 借口 | 反驳 / Hard rule |
|------|-------------------|
| "用户只是要个 MVP，跳过 JTBD 直接开 spec 更快。" | Hard Gates: 必须沉淀 Job / Outcome / Opportunity Tree 才能进入 产品发现评审。Verification: discovery record 落盘且包含可回读的 hypothesis。 |
| "目标用户已经很清楚了，不必走 RICE / ICE。" | Workflow stop rule: 优先级证据缺失就不能产出 backend output object（discovery findings）。 |
| "我可以同时帮用户拍板技术栈。" | Hard Gates: discovery 阶段不替用户做方向 / 取舍 / 标准（soul.md），技术栈是 规格阶段 / 设计阶段 的对象。 |
...
- [ ] bridge 语义已明确，足以支撑 规格阶段 读取；Desired Outcome / Success Threshold 已并入 Bridge to Spec
- [ ] 高风险、低 confidence 关键假设已标注"建议走 实验探针 先验证"
- [ ] discovery 正文未混入 formal spec / design 细节
- [ ] discovery 阶段进度记录（若使用）已同步到 产品发现评审；进入 feature 后续 progress 落到 `features/<NNN>-<slug>/progress.md`
