---
name: hf-test-review
description: v0.7 主链上不再默认激活；仅当用户显式直呼"独立 test review"或 high-risk task 项目策略要求测试维度独立深审时使用。新会话默认走 hf-task-review（合成评审包含测试维度）。不适用于写/修测试（→ hf-test-driven-dev）、评审代码（→ hf-code-review）、合成 per-task 评审（→ hf-task-review）、阶段不清（→ hf-workflow-router）。
---
...
自 v0.7 起，本 skill **不再是 `standard` / `full` profile 主链上的默认 per-task 节点**。主链上的 per-task 评审已由 `hf-task-review` 合成承担（测试质量是其三个 sub-rubric 之一，复用本 skill 的 `references/review-checklist.md` 与维度评分）。

本 skill 保留两类用途：

1. **Reference rubric 库**：`references/review-checklist.md` 仍是 `hf-task-review` 测试维度的权威 rule set（TT1–TT6 / TA1–TA4）；不要改文件路径
2. **独立直呼场景**：用户显式要求"我只想要一次纯测试评审"，或 `Risk Tag = high-risk` task 配合项目策略要求测试维度独立深审时

不在 v0.7 主链上触达本 skill 的场景：
- 普通 per-task 评审 → `hf-task-review`
- Feature 终局的全量 spec↔impl 矩阵 → `hf-traceability-review`

router 迁移表见 `hf-workflow-router/references/profile-node-and-transition-map.md` 的"v0.7 主链关键改动总结"段。

---

评审测试资产，判断 fail-first、行为覆盖和风险覆盖是否足以支持下游评审 / 门禁。可吸收项目已有的缺陷模式记录或风险清单作为风险输入，但不以其存在为前置。
...
适用：
- 用户显式直呼"独立 test review"
- `Risk Tag = high-risk` task 项目策略要求测试维度独立深审
...
Direct invoke 信号："只 review 测试 不审代码"、"独立 test review"、"帮我审一下测试质量"。

## Hard Gates

- v0.7 主链上不默认激活；父会话误 dispatch → precheck blocked，`reroute_via_router=true`
- 输入工件不足不得开始评审
- reviewer 不修测试、不继续实现
...
检查：是否存在稳定实现交接块、可定位测试资产、route/stage/profile 与上游 evidence 是否一致、调用来源是否合法（v0.7 主链上仅独立直呼或 high-risk 项目策略激活）。
...
- v0.7 主链上对普通 task 误 dispatch 本 skill → 路由异常，`reroute_via_router=true`，下一步 `hf-workflow-router`（普通 task 应走 `hf-task-review`）
- precheck 通过 → 继续正式审查
...
3.3 **风险覆盖与边界**：是否覆盖已识别的风险（来自项目缺陷模式记录、风险清单或上游 review/hotfix 历史）？边界/null/错误路径？
3.4 **测试设计质量**：mock 是否限定在真正边界？测试是否独立可重复？是否存在 provider mock、child component overmock、mock fetch、fixture contract drift 等掩盖真实 App 装配 / API 契约 / browser runtime 的风险？
3.5 **下游就绪度**：测试质量是否足以让下游做可信判断？

### 4. 形成 verdict

- `通过`：所有维度 >= 6 → `next_action_or_recommended_skill=hf-workflow-router`（v0.7：主链上独立 test review 通过后由 router 决定下一步），`needs_human_confirmation=false`
...
| 文件 | 用途 |
|------|------|
| `references/review-checklist.md` | test review checklist 与 rule IDs；同时被 `hf-task-review` 复用作 test_quality sub-rubric |
...
- 评审中修测试
- 返回多个候选下一步
- v0.7 主链上对普通 task 仍 dispatch 本 skill（应该走 `hf-task-review`）
...
- [ ] 结论足以让父会话路由（v0.7：独立直呼通过后默认让 router 决定下一步）
