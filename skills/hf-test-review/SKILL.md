---
name: hf-test-review
description: 适用于实现完成后判断测试质量、code review 前的测试评审、用户显式要求评审测试的场景。可吸收已有缺陷模式记录或项目级风险清单作为风险输入，但不以其存在为前置。不适用于写/修测试（→ hf-test-driven-dev）、评审代码（→ hf-code-review）、阶段不清（→ hf-workflow-router）。
---

# HF Test Review

评审测试资产，判断 fail-first、行为覆盖和风险覆盖是否足以支持 `hf-code-review`。hybrid batch 下，本 review 一次覆盖 `Batch Quality Scope` 中所有已实现 task，而不是每个 task 单独执行。可吸收项目已有的缺陷模式记录或风险清单作为风险输入，但不以其存在为前置。
...
读 `Batch Quality Scope`、每个 task 的实现交接块、新增/修改测试、项目已声明的缺陷模式记录或风险清单（若项目有维护，按项目约定路径读取，不存在则跳过）、项目级测试约定、规格/设计片段（默认 `features/<active>/spec.md` / `design.md`）、feature `progress.md`（默认 `features/<active>/progress.md`）。

### 1.5 Precheck：能否合法进入 review

检查：batch scope 是否可恢复、每个 task 是否存在稳定实现交接块、可定位测试资产、route/stage/profile 与上游 evidence 是否一致。

- route/stage/证据冲突 → 写最小 blocked precheck record，`reroute_via_router=true`
- route 明确但任一 task 缺稳定交接块或关键测试资产 → 写最小 blocked record，下一步 `hf-test-driven-dev`（finding 必须定位到 task ID）
- precheck 通过 → 继续正式审查
...
保存到 项目声明的 review record 路径；若无项目覆写，hybrid batch 默认使用 `features/<active>/reviews/test-review-batch-YYYY-MM-DD.md`（legacy 单 task 复审可继续用 `test-review-task-NNN.md`）。若项目无专用格式，默认使用 `references/review-record-template.md`。
...
完成时产出：
- Review 记录（保存到 项目声明的 review record 路径；若无项目覆写，hybrid batch 默认使用 `features/<active>/reviews/test-review-batch-YYYY-MM-DD.md`）
