# mwf Workflow Shared Conventions

本文档汇总 `mwf-*` skills 在团队日常需求开发 / 问题修改工作流中共享的约定。每个 mwf skill 通过本文档锚定相同的工件路径、字段名、handoff 字段和角色边界。

- 上位原则: `docs/mwf-principles/00 soul.md`
...
mwf 服务的项目是多组件多仓库工程。**工件边界以组件 git 仓库为单位**，本文中默认路径假设当前活跃 work item 所在的组件仓库为根。
...
`features/<id>/` 是单次需求 / 问题修改的过程目录；`docs/` 是组件仓库的长期资产。任何与项目本地约定不一致的路径，由该项目 `AGENTS.md` 显式声明覆盖；mwf skill 必须**优先读取项目 `AGENTS.md` 声明的路径映射**，本文路径仅作为默认逻辑布局。
...
features/AR<id>-<slug>/
  ...
  requirement.md              # mwf-specify 写入：需求规格澄清
  ar-design-draft.md          # mwf-ar-design 写入：AR 实现设计草稿（含测试设计章节）
  traceability.md             # IR/SR/AR/Design/Code/Test 追溯矩阵
  implementation-log.md       # mwf-tdd-implementation 写入：RED/GREEN/REFACTOR 证据摘要
  reviews/
    spec-review.md
    component-design-review.md  # 仅在 component-impact route 出现
    ar-design-review.md
    test-check.md             # mwf-test-checker 写入
    code-review.md
  evidence/
    unit/
    integration/
    static-analysis/
    build/
  completion.md               # mwf-completion-gate 写入
  closeout.md                 # mwf-finalize 写入
```
...
features/DTS<id>-<slug>/
  README.md
  progress.md
  reproduction.md             # mwf-problem-fix 写入：复现路径
  root-cause.md               # mwf-problem-fix 写入：根因
  fix-design.md               # mwf-problem-fix 写入：最小修复边界
...
`features/<id>/progress.md` 必须使用以下 canonical 字段名。`mwf-workflow-router` 与所有 leaf skill 都消费这些字段。
...
| `Current Stage` | 当前 canonical `mwf-*` 节点 | 必填 |
| `Pending Reviews And Gates` | 待完成 review / gate | 列表 |
| `Next Action Or Recommended Skill` | 唯一 canonical `mwf-*` 节点名 | 不允许自由文本 |
| `Blockers` | 阻塞项摘要 | 可空 |
| `Last Updated` | 时间戳 | 必填 |

任何 mwf skill 完成节点工作时，**必须**用 canonical 字段名同步 `progress.md`，禁止把自由文本下一步写进 `Next Action Or Recommended Skill`。

## 角色 / 责任边界

| 角色 | 拍板权 | mwf 行为 |
|---|---|---|
| 模块架构师 | 组件边界、SOA 接口、组件实现设计、跨组件影响 | mwf 工程化执行 + 提示阻塞，不替架构师决定 |
| 开发负责人 / 需求负责人 | AR 是否进入开发、范围边界、SR/IR/AR 追溯 | mwf 提供澄清 + 待决问题列表 |
| 开发人员 | AR 实现设计、代码、单测、修复说明 | mwf 提供模板执行 + 证据收集 |
| mwf | 不拍板任何专业判断 | 把判断落成可执行 / 可验证 / 可追溯的工程过程 |

## Canonical 节点名

```text
using-mwf-workflow            # 公开入口（不写入 handoff）
mwf-workflow-router           # 运行时编排
mwf-specify
mwf-spec-review
mwf-component-design          # 仅 component-impact route
mwf-component-design-review   # 仅 component-impact route
mwf-ar-design
...
mwf-completion-gate
mwf-finalize
mwf-problem-fix               # 仅 hotfix / DTS route
```

## Handoff 摘要最小字段

每个 mwf leaf skill 完成时返回的结构化 handoff 至少包含：
...
- `blockers`：未闭合的阻塞项
- `next_action_or_recommended_skill`：唯一 canonical `mwf-*` 节点名
- `reroute_via_router`：`true` / `false`，下一步无法唯一映射时为 `true` 并指向 `mwf-workflow-router`

`next_action_or_recommended_skill` **不得**写入 `using-mwf-workflow`，**不得**写入自由文本。
...
`lightweight` **不允许跳过** test-checker 与 code-review；只能压缩文档量，不能压缩证据。Profile 由 `mwf-workflow-router` 决定，下游 leaf skill 不得自行降级。

## Hard Stops（任一命中必须停下）

- 需求输入不清且涉及方向 / 范围 / 验收 → 停在 `mwf-specify` 或回需求负责人
- IR / SR / AR 追溯关系冲突 → 阻塞
- AR 不属于唯一组件 → 阻塞
- 缺组件实现设计但本次修改影响组件边界 → 进 `mwf-component-design`
- AR 实现设计未含测试设计章节 → 回 `mwf-ar-design`
- TDD 完成后测试用例未经 `mwf-test-checker` 审查 → 不得进入 `mwf-code-review`
- 代码修改破坏 SOA 边界或引入未解释跨组件依赖 → review 阻塞
- 存在未解释的 critical 静态分析 / 编译告警 / 编码规范违反 → completion 阻塞
- review / gate 结论无法唯一映射下一步 → 回 `mwf-workflow-router`

## 测试设计是 AR 实现设计的章节

mwf 不维护独立的 `test-design.md`。测试用例、覆盖目标、预期 I/O、mock / stub / 仿真说明、RED / GREEN 证据要求都作为 `ar-design-draft.md`（过程版）和 `docs/ar-designs/AR<id>-<slug>.md`（正式版）的章节存在。

`mwf-tdd-implementation` 必须以 AR 实现设计中的测试设计章节为驱动，不得跳过。
