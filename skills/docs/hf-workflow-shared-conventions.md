...
HF 把 Karpathy 风格的 4 条 LLM coding 行为准则（Think Before Coding / Simplicity First / Surgical Changes / Goal-Driven Execution）落到 `skills/principles/coding-principles.md`，作为**所有 `hf-*` skill 共同遵守的横切行为基线**，而不是一个独立的 workflow 节点。
...
- 与 `skills/principles/soul.md` / `AGENTS.md` 显式声明冲突时让位。
- 旧工件未显式提及这 4 条不视为违规；本节只约束新产出的 skill 行为，不强制回写历史工件。

详细条款与每条原则在各 `hf-*` skill 的承接点见 `skills/principles/coding-principles.md`。
...
布局采用双根目录二分（详见 `skills/principles/sdd-artifact-layout.md`）：
...
除非 `AGENTS.md` 已声明等价路径，否则默认推荐（`tier` 标记对应 `skills/principles/sdd-artifact-layout.md` 中的 *Minimal `docs/` Tiers*）：
...
| HF 自带原则锚点 | `skills/principles/`（随 HF skills 包发布；含 9 份原则文档） | skills 包安装即得 |
...
feature 周期内对 `docs/` 中长期资产的修改，按以下时机划分（详见 `skills/principles/sdd-artifact-layout.md`）。所有同步遵循 **read-on-presence + sync-on-presence** 原则：HF skill 读取 `docs/` 资产时，缺失视为"未启用"而非阻塞；`hf-finalize` 同步时只对**已存在**的子目录或本 feature 触发了升级条件的子目录做同步。
...
- [ ] 该 skill 行为遵守 `skills/principles/coding-principles.md` 的 4 条横切基线，且没有把它们写进 canonical schema 或 next-action vocabulary
...
- [ ] 工件路径遵循 `skills/principles/sdd-artifact-layout.md` 的双根目录布局：feature 周期内工件落到 `features/<active>/...`，长期资产落到 `docs/...`
