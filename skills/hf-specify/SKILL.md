---
name: hf-specify
description: 适用于尚无已批准规格、现有规格仍是草稿、或规格被 规格评审 退回需修订的场景。不适用于已有批准规格、需要任务计划或阶段不清。
---

# HF 需求澄清

创建一份定义"做什么、为什么做、做到什么程度算完成"的需求规格说明，准备到可交独立规格评审 的状态。
...
| **量化优先级 RICE / ICE / Kano**（Phase 0 新增，承接自 产品发现） | 在多条 Must 候选间做可冷读的量化取舍，不替代 MoSCoW | Intercom (RICE)；Sean Ellis (ICE)；Noriaki Kano (Kano) | 步骤 4 — Priority 行；步骤 5 — 延后判断；详见 `产品发现/references/prioritization-quant.md` |
...
- 尚无已批准需求规格
- 现有规格仍为草稿或待收敛
- 规格评审返回 `需修改` 或 `阻塞`，需按 findings 修订
- 用户需要先澄清范围、验收标准、边界、约束、非目标

不适用：

- 已有批准规格，问题已在实现层 (HOW) 而非需求层 (WHAT)
- 规格和设计都已批准，正在拆解或执行任务
- 修复线上缺陷或处理范围 / 验收 / 约束的增量变化
- 还在判断产品是否值得做（讨论 thesis / wedge / probe 方向）
...
- Success Metrics + Key Hypotheses 章节不可省略；不得"以后补"（见 `references/success-metrics-and-hypotheses.md`）
- Blocking 假设仍未验证时，spec 不允许通过评审
- 核心 NFR 必须 ISO 25010 归类 + Quality Attribute Scenario 五要素 + Acceptance 一致；Response Measure 不允许"足够快""合理"
...
当存在多条 `Must` 候选且互相冲突、或超出当前轮容量时，引入 **RICE / ICE** 做量化取舍（见 `产品发现/references/prioritization-quant.md`）。分数必须带来源；分数相近（±20%）时不自动拍板，补定性理由。MoSCoW 决定"进不进当前轮"，RICE / ICE 决定"同等优先级里先打哪个"。
...
按 `references/reviewer-handoff.md` 派发独立 reviewer subagent 执行 规格评审，不内联执行。reviewer 返回后按协议处理结果。
...
| reviewer 派发协议 | `references/reviewer-handoff.md` | 准备派发 规格评审 时 | 全档必读（派发时机） |
...
| 跨 skill：量化优先级 | `产品发现/references/prioritization-quant.md` | 多条 Must 候选冲突、或需要 RICE / ICE 辅助取舍时 | 按需；默认不加载 |
...
- [ ] `features/<active>/progress.md` 已按 canonical schema 同步，下一步为 规格评审
