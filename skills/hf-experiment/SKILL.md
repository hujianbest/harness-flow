---
name: hf-experiment
description: 适用于产品发现或规格阶段存在低 confidence、高风险或 Blocking 的关键假设，需要一次最小可验证 probe 再决定是否推进主链。不适用于无假设驱动的纯澄清、需新起 discovery、进入正式实现、阶段不清。
---

# HF Experiment

在进入 规格阶段 前（或 规格阶段 内部 Blocking 假设未关闭时），为一条或少数几条**关键假设**产出一份结构化 **probe plan**：明确假设是什么、如何最小验证、多长时间、什么结果算通过、证据落在哪、结果如何回流。
...
- 产品发现 或 产品发现评审 识别出低 confidence / 高风险假设，正式推进前值得先验证
- 规格阶段 中 Key Hypotheses 存在 `Blocking? = 是` 的假设，规格不能在未验证前通过评审
- 用户明确要求"先做个小验证再决定"
...
- 问题本质是澄清需求，而不是验证假设 → 规格阶段
- 需要完整新做 discovery → 产品发现
- 进入实现阶段 → 实现层 TDD
- 阶段 / route / 证据冲突 → 上游编排者编排者
...
- 若验证结果与假设相反，禁止"合理化"推进主链；必须显式回流到 产品发现 或 规格阶段
- 若请求已明显跨过假设验证进入设计 / 实现，回到 上游编排者
...
- 下游回流目标：probe 完成后该回到 产品发现 / 规格阶段 / 上游编排者 的哪一个
...
| 结果 | 回流目标 |
|---|---|
| Pass（且对应假设不再 Blocking） | 规格阶段（若 spec 已在起草）或 产品发现（discovery 阶段） |
| Fail（假设被证伪） | 回 产品发现：修订 OST、重写候选方向；或回 规格阶段 修订对应 FR/NFR；必要时回 上游编排者 重新路由 |
| Inconclusive（超时 / 样本不够 / 方法缺陷） | 决定：(a) 追加一次小 probe；(b) 显式接受风险并写入 Key Hypotheses 的 "accepted-risk" 标记；(c) 回 上游编排者 |
...
- 进度同步：
  - 若在 discovery 阶段，discovery 进度记录的 `Next Action Or Recommended Skill` 暂指向 实验探针，probe 回流后改回上游评审或规格阶段
  - 若在 spec 阶段，feature `progress.md` 的 `Next Action Or Recommended Skill` 暂指向 实验探针，probe 回流后改回规格阶段或规格评审
...
- 本 skill 仅在 `full` profile 下激活（standard / lightweight 不激活 实验探针）
...
| 借口 | 反驳 / Hard rule |
|------|-------------------|
| "假设很明显，跳过 probe 直接进 spec。" | Workflow stop rule: 高风险 / 阻塞性假设未验证时 规格阶段 不应启动；实验探针 是其前置条件。 |
