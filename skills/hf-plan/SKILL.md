---
name: hf-plan
description: HarnessFlow plan 阶段。frame 完成、建造模式且风险档位 ≥2 时使用:档位 2 产出一份 plan.md(需求+设计+任务清单),档位 3 先产出 spec.md 评审通过后再产出 design.md。也用于 plan 层评审返回"需修改"后的修订。档位 1 与探索模式不使用本技能。前提不满足(gate check --to plan 失败)时回 hf-frame。
---
