...
description: 通过结构化访谈对齐想法/方案，并将结果写入 CONTEXT.md 与 ADR。HarnessFlow 主链第二步；内驱 hf-grilling 与 hf-domain-modeling。绿地项目先运行 gate init；确认后写入 progress.md，下一步进入 hf-to-product-architecture（完整产品层）或开特性（存量/仅地图）。
---
...
7. 下一步（完整产品层）:进入 `hf-to-product-architecture`（技能约束），再开特性走 `hf-to-spec`。存量热修且采用仅架构地图时，可直接开特性并冷读地图。无强制 gate。
