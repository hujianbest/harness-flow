# mwf Traceability Matrix

使用说明：

- 这是 `features/<Work Item Id>-<slug>/traceability.md` 模板。
- mwf 要求 IR -> SR -> AR -> 组件设计 -> AR 设计 -> 代码 -> 测试 -> 测试有效性审查 -> 代码检视 全链路可回读。
- 由 `mwf-specify` 初始化骨架，`mwf-component-design` / `mwf-ar-design` / `mwf-tdd-implementation` / `mwf-test-checker` / `mwf-code-review` 各自补充本节点对应行。
