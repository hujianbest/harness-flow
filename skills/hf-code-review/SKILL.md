---
name: hf-code-review
description: v0.7 主链上仅当当前 task Risk Tag = high-risk 时作为 hf-task-review 之后的追加深审节点；其他情况由 hf-task-review 的代码质量 sub-rubric 承担。也可用户显式直呼。不适用于普通 per-task 评审（→ hf-task-review）、评审测试（→ hf-test-review）、写/修代码（→ hf-test-driven-dev）、阶段不清（→ hf-workflow-router）。
---
# HF Code Review

## v0.7 状态说明（READ FIRST）

自 v0.7 起，本 skill 在主链上**只在两种场景下激活**：

1. **`Risk Tag = high-risk` 追加深审**：`hf-task-review` 通过后，由 router 自动激活本 skill 做 CR7（架构健康 + 重构纪律）/ CR8（UI 一致性）的独立深审；本 skill 与 `hf-task-review` 是串行（不并行）
2. **用户显式直呼**：用户明确要求"我只想要一次纯 code review"

**普通 per-task 评审不触达本 skill**；代码质量 8 维评分（CR1–CR9 / CA1–CA10）由 `hf-task-review` 的 code_quality sub-rubric 承担（复用本 skill 的 `references/review-checklist.md` / `references/clean-architecture-guardrails.md` / `references/ai-slop-rubric.md`）。

本 skill 保留两类用途：

1. **Reference rubric 库**：`references/review-checklist.md` 等仍是 `hf-task-review` 代码维度的权威 rule set；不要改文件路径
2. **High-risk 追加深审入口**：在 `Risk Tag = high-risk` 链路上仍是必经节点（详见 router 迁移表）

router 迁移表见 `hf-workflow-router/references/profile-node-and-transition-map.md` 的"Risk Tag 链路"段。本 skill 的 record 在 v0.7 主链上**强制 `record_mode=file`**（高风险必留 audit trail），不允许 snapshot。

---

评审实现代码质量。判断正确性、设计一致性、状态/错误/安全、可读性、下游追溯就绪度，**架构健康与重构纪律（CR7）**，以及 UI surface 的实现一致性（CR8）。在 v0.7 主链上运行在 `hf-task-review` 之后（high-risk task），决定是否可进入 `hf-regression-gate`。
...
适用：
- v0.7 主链上 `Risk Tag = high-risk` task 的 `hf-task-review` 之后追加深审
- 用户显式直呼独立 code review

不适用：普通 per-task 评审 → `hf-task-review`；评审测试 → `hf-test-review`；写/修代码 → `hf-test-driven-dev`；阶段不清 → `hf-workflow-router`。

## Hard Gates

- code review 通过前不得进入 `hf-regression-gate`（v0.7 high-risk 深审路径）
- 输入工件不足不得开始
- reviewer 不改代码、不继续实现
- v0.7 主链上本 skill 强制 `record_mode=file`（高风险必留 audit trail）；不允许 snapshot mode

## Workflow

### 1. 建立证据基线

读实现交接块（含 Refactor Note）、代码变更、上游 `hf-task-review` 记录（snapshot 锚点 或 `features/<active>/reviews/task-review-task-NNN.md`）、项目级 coding 约定、规格/设计片段（默认 `features/<active>/spec.md` / `design.md`）、相关 ADR（默认 `docs/adr/`）、feature `progress.md`（默认 `features/<active>/progress.md`）。
...
检查：是否存在稳定实现交接块（含 Refactor Note）、可定位代码变更、route/stage/profile 与上游 evidence 是否一致、Risk Tag 是否为 `high-risk`（v0.7 主链上本 skill 仅服务 high-risk task）。
...
- 实现交接块的 Refactor Note 中 Escalation Triggers 非 `none` 但 Next Action 仍指向 `hf-task-review` → route/stage 与 escalation 边界冲突，`reroute_via_router=true`，下一步 `hf-workflow-router`
- Risk Tag ≠ `high-risk` 但父会话仍 dispatch 本 skill → 路由异常，`reroute_via_router=true`，下一步 `hf-workflow-router`（普通 task 应在 `hf-task-review` 完成后直接进 regression）
- precheck 通过 → 继续正式审查
...
3.5 **下游就绪度**：代码是否足以让回归门禁做可信判断？实现交接块是否完整？
...
3.8 **Comment 质量 / AI Slop Detection (CR9)**：按 `references/ai-slop-rubric.md` 跑 grep / rg 4 类禁用模式（英文冗余形容词 / 中文冗余词 / 解释性自然语言注释 / em-dash + en-dash）于 PR diff 范围。命中按例外段决策：
- 用户文档 / README / docs / examples / test assertion message / i18n 文案 → `[exempt][ai-slop-NN]` 不计 finding
- source code 注释 / spec / design / SKILL.md / 工程内部 prose 命中 → `[minor][LLM-FIXABLE][ai-slop-NN]` finding
- 单 PR 命中 ≥ 5 条 → 升级 `[important][LLM-FIXABLE][ai-slop-cluster]` 单条 finding 提示 author 整体过一遍 wording

### 4. 形成 verdict

- `通过`：所有维度 >= 6（含 CR7 主维度 >= 8、所有 CR7 子维度 >= 6；触碰 UI 时 CR8 >= 8），代码足以进入回归门禁 → `next_action_or_recommended_skill=hf-regression-gate`（v0.7：high-risk 深审通过后直接进 regression；全量追溯改在 feature 终局批跑），`needs_human_confirmation=false`
...
- `阻塞`：核心逻辑错误/安全漏洞/findings 无法定向回修 → `next_action_or_recommended_skill=hf-test-driven-dev`，`needs_human_confirmation=false`；若问题本质是 stage/route/profile/上游证据冲突，**或 CR7 触发 escalation-bypass（CA8）/ 实质修改 ADR / 模块边界 / 接口契约 / 跨 ≥3 模块结构性变更**，**或同 task 已是第 3 轮回流（remediation budget 耗尽）** → `next_action_or_recommended_skill=hf-workflow-router`，`reroute_via_router=true`

Findings 带 severity 和 USER-INPUT/LLM-FIXABLE 分类。给出代码风险和回归提示。

### 5. 写 review 记录

保存到 项目声明的 review record 路径；若无项目覆写，默认使用 `features/<active>/reviews/code-review-task-NNN.md`（v0.7 同 task 多轮加 `-r2` 后缀）。**v0.7 强制 `record_mode=file`**。若项目无专用格式，默认使用 `references/review-record-template.md`。

回传结构化摘要时遵循当前 skill pack 中 `hf-workflow-router/references/reviewer-return-contract.md`：`next_action_or_recommended_skill` 只写一个 canonical 值；`record_mode=file`；`remediation_round_count` 写明 1 / 2 / 3；workflow blocker 必须显式写 `reroute_via_router=true`。
...
| precheck blocked：Risk Tag ≠ high-risk 但父会话仍 dispatch 本 skill | `阻塞` | `hf-workflow-router` | `true` | `record_path`、路由异常说明、`key_findings` |
| precheck blocked：Refactor Note 中 Escalation Triggers 非 `none` 但 Next Action 误指 `hf-task-review` | `阻塞` | `hf-workflow-router` | `true` | `record_path`、escalation 触发说明、`key_findings` |
| 正式审查后 `通过` | `通过` | `hf-regression-gate`（v0.7 高风险深审路径） | `false` | `record_path`、主要通过依据、非阻塞优化项（若有） |
| 正式审查后 `需修改` | `需修改` | `hf-test-driven-dev` | `false` | `record_path`、`key_findings`、`finding_breakdown`、代码风险与回归提示 |
...
| 正式审查后 `阻塞`：同 task 第 3 轮 review（remediation budget 耗尽） | `阻塞` | `hf-workflow-router` | `true` | `record_path`、`remediation_round_count=3`、`key_findings` |
| 正式审查后 `阻塞` 且问题本质属于重编排 | `阻塞` | `hf-workflow-router` | `true` | `record_path`、关键冲突说明、`key_findings` |

固定规则：
- 字段名统一使用 `conclusion`、`next_action_or_recommended_skill`、`record_mode`（v0.7 强制 `file`）、`record_path`、`key_findings`、`needs_human_confirmation`、`reroute_via_router`、`remediation_round_count`
...
| 文件 | 用途 |
|------|------|
| `references/review-checklist.md` | code review checklist 与 rule IDs（CR1-CR9、CA1-CA10）；同时被 `hf-task-review` 复用作 code_quality sub-rubric |
...
- v0.7 主链上对非 high-risk task 仍 dispatch 本 skill（应该在 `hf-task-review` 后直接进 regression）
- 在本 skill 使用 `record_mode=snapshot`（v0.7 强制 file mode；高风险必留 audit trail）
...
| "Risk Tag 没卡死也跑本 skill 更稳。" | Hard Gates (v0.7): 主链上仅服务 high-risk task；普通 task 在 hf-task-review 后直接进 regression；越权 dispatch → precheck blocked。 |

## Verification

- [ ] review record 已落盘（v0.7 强制 file mode）
- [ ] 给出明确结论、findings、code risks 和唯一下一步
- [ ] findings 已标明 severity / classification / rule_id（含 CR7 / CR7.x / CA6-CA10 时遵循同一规则）
- [ ] 结构化摘要含 record_path / record_mode=file / remediation_round_count / next_action_or_recommended_skill
...
- [ ] Risk Tag = high-risk 已确认（v0.7 路由前提）
- [ ] workflow blocker 时已显式写明 reroute_via_router
- [ ] 第 3 轮 review 时已设 `reroute_via_router=true` 并 `next_action_or_recommended_skill=hf-workflow-router`（remediation budget 耗尽）
