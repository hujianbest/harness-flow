...
评审实现代码质量。判断正确性、设计一致性、状态/错误/安全、可读性、下游追溯就绪度，**架构健康与重构纪律（CR7）**，以及 UI surface 的实现一致性（CR8）。运行在 `hf-test-review` 之后，决定是否可进入 `hf-traceability-review`。hybrid batch 下，本 review 覆盖 `Batch Quality Scope` 中所有已实现 task。
...
读 `Batch Quality Scope`、每个 task 的实现交接块（含 Refactor Note）、代码变更、test-review 记录（hybrid batch 默认 `features/<active>/reviews/test-review-batch-YYYY-MM-DD.md`）、项目级 coding 约定、规格/设计片段（默认 `features/<active>/spec.md` / `design.md`）、相关 ADR（默认 `docs/adr/`）、feature `progress.md`（默认 `features/<active>/progress.md`）。
...
检查：batch scope 是否可恢复、每个 task 是否存在稳定实现交接块（含 Refactor Note）、可定位代码变更、route/stage/profile 与上游 evidence 是否一致。

- route/stage/证据冲突 → 写最小 blocked precheck record，`reroute_via_router=true`
- route 明确但任一 task 缺稳定交接块、核心代码范围不可定位、或 Refactor Note 缺失 → 写最小 blocked record，下一步 `hf-test-driven-dev`（finding 必须定位到 task ID）
- 实现交接块的 Refactor Note 中 Escalation Triggers 非 `none` 但 Next Action 未指向 `hf-workflow-router` → route/stage 与 escalation 边界冲突，`reroute_via_router=true`，下一步 `hf-workflow-router`
- precheck 通过 → 继续正式审查
...
保存到 项目声明的 review record 路径；若无项目覆写，hybrid batch 默认使用 `features/<active>/reviews/code-review-batch-YYYY-MM-DD.md`（legacy 单 task 复审可继续用 `code-review-task-NNN.md`）。若项目无专用格式，默认使用 `references/review-record-template.md`。
...
| precheck blocked：Refactor Note 中 Escalation Triggers 非 `none` 但 Next Action 未指向 `hf-workflow-router` | `阻塞` | `hf-workflow-router` | `true` | `record_path`、escalation 触发说明、`key_findings` |
