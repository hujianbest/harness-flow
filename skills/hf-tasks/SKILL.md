---
name: hf-tasks
description: 适用于规格与设计都已批准、需要在编码前产出可评审任务计划的场景。不适用于规格/设计未稳定、任务计划已批准需进入实现、或阶段不清。
---

# HF 任务拆解

创建任务计划，把已批准设计转化为可执行、可追溯、可验证的工作单元，准备到可交独立任务评审 的状态。
...
适用：
- 规格与设计都已批准，需要任务计划
- 任务评审返回 `需修改` 或 `阻塞`，需按 findings 修订
- 需要为 实现层 TDD 准备任务输入和测试设计种子

不适用 → 改用：
- 规格未稳定/未批准 → 规格阶段 / 规格评审
- 设计未稳定/未批准 → 设计阶段 / 设计评审
- 任务计划已批准，需进入实现 → 实现层 TDD
- 阶段不清/证据冲突 → 上游编排者编排者
...
- 每个 task 必须有可读 Definition of Done 落盘
- 依赖图 + 关键路径是任务计划必需输出
- INVEST 不达标的 task 不得作为活跃任务启动（Independent / Negotiable / Valuable / Estimable / Small / Testable）
- Acceptance 必须可观察、可判断、可形成通过/不通过的客观裁决
...
**若 UI surface 被激活**（存在 UI 设计 的已批准文档）：除技术设计外，也需读取 UI 设计文档，提取组件粒度、关键页面 wireframe、交互状态矩阵、Design Token 映射；前端任务拆解必须承接 Atomic 分层与状态矩阵，避免把"实现某页面"当作单任务。
...
- 代码型任务优先拆成可直接支撑 实现层 TDD 的最小闭环：`fail-first test -> 确认失败 -> 最小实现 -> verify green`
...
- router 可稳定重选下一 task
- 测试设计种子足以帮助 实现层 TDD 进入测试设计

按 `references/reviewer-handoff.md` 派发独立 reviewer subagent 执行 任务评审。
...
注意：只有 review 通过且 approval step 完成后，才进入 实现层 TDD。
...
- [ ] feature `progress.md` 已按 canonical schema 同步，下一步为 任务评审
