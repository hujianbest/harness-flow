# mwf Code Review Rubric

> 配套 `mwf-code-review/SKILL.md`。展开 8 维度评分细则与 rule IDs。
...
- `CR3.1` 不绕过组件 SOA 接口
- `CR3.2` 不引入未声明的跨组件依赖
- `CR3.3` 接口实现的错误码 / 时序约束与 docs/interfaces.md 一致
- `CR3.4` 内部细节未通过公共符号泄漏
...
- `LLM-FIXABLE`：代码结构 / 错误处理 / 命名 / 边界 / 防御性检查 / Refactor Note 字段补全 / in-task 范围 smell → 由 `mwf-tdd-implementation` 回修
