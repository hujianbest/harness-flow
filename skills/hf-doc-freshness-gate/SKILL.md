...
description: 适用于已通过 回归门禁、即将进入 完成门禁、需要为本任务/本 feature 引入的对外可见行为变化判定相关用户文档（仓库根 README 产品介绍段、模块层 README、公共 API doc / OpenAPI / docstring、i18n 副本、CONTRIBUTING / onboarding doc、用户文档站 source）是否同步刷新的场景。不适用于尚未通过 回归门禁、纯 prose feature 但 reviewer 误以为强制 lint 工具链、阶段不清或证据冲突。
---

# HF Doc Freshness Gate

判定本任务 / 本 feature 引入的 user-visible behavior change 是否已在**对外可见文档**中同步刷新，并按 sync-on-presence + profile 分级 + 与 完成门禁 / 收尾归档 / 增量变更 / 代码评审 / 追溯评审 显式分工的纪律输出 `verdict + fresh evidence`。

**职责边界**：本 gate 只负责"对外可见文档的同步 verdict"。同 tier 的 回归门禁 负责"行为不回归"；下游 完成门禁 消费本 gate verdict 作为 evidence bundle 一项；收尾归档 在 closeout 阶段同步既有合同覆盖的长期资产（CHANGELOG / ADR 状态翻转 / 顶层导航 / docs/architecture / docs/runbooks 等），**不重叠**本 gate 负责的对外可见文档维度。完整责任划分见 `references/responsibility-matrix.md`。
...
| 方法 | 核心原则 | 落地步骤 |
|---|---|---|
| **Three-Section Skill Contract** (HF native) | 与 回归门禁 / 完成门禁 同形态：Hard Gates + Verification + fresh evidence | 整 SKILL 段落布局 |
...
| **Evidence Bundle Pattern** | verdict 路径作为 完成门禁 evidence bundle 一项被 reference；evidence 必须具备 fresh 性（绑定上游 record / artifact hash），不能引用历史结论 | 步骤 4 §evidence 落盘 + FR-005 |

## When to Use

适用：
- 父会话已通过 回归门禁，即将进入 完成门禁，需要本 gate verdict 作为 evidence bundle 一项
- 用户明确要求"评估本任务对外可见文档是否同步"
- reviewer subagent 被父会话按 `references/reviewer-dispatch-handoff.md` 派发执行本 gate

不适用：
- 尚未通过 回归门禁 → 回 实现层 TDD 或上游 review skill
- reviewer 误以为本 gate 强制项目方安装 lint 工具链 → NFR-003 显式声明无外部工具链依赖；按 `Red Flags` 列出的"误以为强制 lint 工具"模式自纠
- 阶段不清 / 证据冲突 → 上游编排者编排者

## Hard Gates

- 本 gate 必须由独立 reviewer subagent (readonly) 派发；不允许父会话或实现节点 (`实现层 TDD`) 自评（FR-008 + CON-006）
...
- spec/tasks/commits 三类输入全缺 → verdict = blocked，next = 追溯评审（FR-001 负路径）
- spec 与 commits 实质不一致 → verdict = blocked，next = 增量变更（FR-007 负路径）
- 本 gate 不承担 收尾归档 既有合同覆盖的同步动作（ADR 状态翻转 / CHANGELOG / 顶层导航 / docs/architecture / docs/runbooks 等）；不修改 收尾归档 / 完成门禁 / 代码评审 / 追溯评审 / 增量变更 的 SKILL.md（CON-001 + FR-006 + FR-007）
...
按 `references/reviewer-dispatch-handoff.md`（复用既有 `上游编排者/references/review-dispatch-protocol.md` 的本 gate 适配点）派发独立 reviewer subagent，readonly 模式。
...
形成 user-visible behavior change list（FR-001）。若三类来源全缺 → 进入步骤 4 输出 verdict = `blocked`，next = 追溯评审。
...
| 关键维度漂移（如仓库根 README 产品介绍段与本次行为相关部分明显过期） | `blocked` | next = `实现层 TDD`（补文档变更） |
| spec 与 commits 实质不一致 | `blocked` | next = 增量变更（FR-007） |
...
- `pass` / `partial` / `N/A` → next = 完成门禁（verdict 路径作为 evidence bundle 一项被 reference；FR-005）
- `blocked` (内容) → next = `实现层 TDD`（补文档变更；FR-005 第三条 acceptance）
- `blocked` (workflow) → next = 上游编排者 (`reroute_via_orchestrator=true`)

**重要**：`blocked` verdict **不**进入 完成门禁 evidence bundle；由本 gate 直接路由回 实现层 TDD，避免 completion-gate 引入 doc-freshness blocked 的额外判定分支（design §11 boundary constraints）。
...
- **reviewer-return JSON**：返回给父会话；含 `conclusion` / `next_action_or_recommended_skill` / `record_path` / `dimension_breakdown` / `reroute_via_orchestrator`
- **完成门禁 evidence bundle**（仅当 verdict ∈ {pass, partial, N/A}）：verdict 路径作为 evidence bundle 一项被 reference
...
- **误以为强制 lint / 翻译 / docs 生成工具链** — NFR-003 显式声明无外部工具链依赖；可选工具仅由项目级约定声明
...
- **承担 收尾归档 既有合同覆盖的同步动作** — CON-001 + FR-006；spec §6.2 责任矩阵
- **承担 增量变更 的范围变更分析** — CON-001 + FR-007
...
- **blocked verdict 误进入 完成门禁 evidence bundle** — design §11 + spec FR-005 第三条 acceptance；blocked 直接回 实现层 TDD

## Common Rationalizations

| 借口 | 反驳 / Hard rule |
|------|-------------------|
| "README / spec 没改但代码改了，反正描述一致。" | Hard Gates: 文档新鲜度由 evidence-based 比对决定；必须显式比对最近 implementation 与文档差量。 |
| "examples/ 里的 demo 文档过期不在我职责范围。" | Workflow stop rule: 文档新鲜度覆盖 spec / design / tasks / readme / examples，缺漏即判 fail。 |
| "skills/<name>/references 偶尔过期没关系。" | Hard Gates: references 影响下游 agent 决策，过期 → finding。 |
...
- [ ] `pass` / `partial` / `N/A` → next = 完成门禁，verdict 路径已 reference 到 completion-gate evidence bundle
- [ ] `blocked` → next = `实现层 TDD`（FR-005 第三条）或 增量变更（FR-007）或 上游编排者（precheck workflow 阻塞）
- [ ] reviewer 未承担 收尾归档 / 代码评审 / 追溯评审 / 增量变更 既有职责
