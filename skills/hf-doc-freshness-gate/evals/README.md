...
本目录采用 `test-prompts.json`（裸 array，无 wrapper）+ `README.md` 形式，与既有 30+ HF skill 的 evals 目录形态一致（参见 `skills/hf-tasks-review/evals/` 等）。`skills/principles/skill-anatomy.md` §evals/ 推荐结构（`evals.json` 含 wrapper + fixtures/）属更重型适配，本 prose-only skill 不需要该完整形态。
