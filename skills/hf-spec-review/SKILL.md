---
name: hf-spec-review
description: 适用于规格草稿已完成需正式 review verdict、reviewer subagent 被派发执行规格评审的场景。不适用于缺规格草稿或只需继续写、阶段不清或证据冲突、已有已批准规格需设计评审。
---

# HF 需求评审

评审需求规格，判断范围清晰度、需求可测性、验收标准、success criteria、assumptions 与开放问题闭合度，以及进入 approval step 的准备度。本 skill 是需求规格冻结门禁，目标是把规格准备到可交给 approval step 作为 设计阶段 候选已批准输入的状态。不替代 规格阶段（写规格）或 设计阶段（做设计）。
...
适用：
- 规格阶段 已完成规格草稿，需正式 review verdict
- 用户明确要求"review 这份 spec"
- reviewer subagent 被派发来执行规格评审

不适用：缺规格草稿或只需继续写 → 规格阶段；阶段不清/证据冲突 → 上游编排者编排者；已有已批准规格、当前需要设计评审 → 设计评审。
...
- reviewer 不代替父会话完成 approval step，不顺手开始下游工作
...
读取并固定：当前规格（默认 `features/<active>/spec.md`）、deferred backlog（若存在，默认 `features/<active>/spec-deferred.md`）、项目约定、feature `progress.md`（默认 `features/<active>/progress.md`）、少量上下文用于确认状态和锚点。不只根据聊天记忆判断。
...
- route 明确但缺稳定规格 → 写最小 blocked record，下一步 规格阶段
- precheck 通过 → 继续正式 rubric

### 2. 确定当前规格的结构契约

先判断项目是否通过 项目声明了骨架/字段/优先级体系，当前规格是否遵循。只要语义可回读，不强迫文档长得和默认模板一模一样。
...
| 有用但不完整，findings 可 1-2 轮定向修订 | `需修改` | 规格阶段 |
| 过于模糊/核心范围未定/findings 无法定向回修 | `阻塞`(内容) | 规格阶段 |
...
- `reroute_via_orchestrator=true` 时只说明 workflow blocker
