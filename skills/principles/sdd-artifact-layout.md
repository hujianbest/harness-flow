...
- 关联:
  ...
  - Skill 写作原则: `skills/principles/skill-anatomy.md`
  - SDD + TDD 设计原则: `skills/principles/hf-sdd-tdd-skill-design.md`

> **范围说明（独立部署边界）**：本文件已随 HF skills 包发布于 `skills/principles/sdd-artifact-layout.md`，是 skills 自包含锚点的一部分（不再依赖宿主项目根 `docs/`）。本文档下文出现的 `docs/principles/` / `docs/adr/` / `docs/architecture.md` 等路径，描述的是**采用了 HF 的宿主项目**自身应组织的工件位置，不是 skills 包的依赖。HF skills 自身使用的 9 份原则文档已全部位于 `skills/principles/`，宿主项目无需复制。
...
```text
docs/
  principles/                         # 可选；仅当宿主项目要落自有的设计/产品原则时启用
    architecture-principles.md        # 可选
    product-principles.md             # 可选
  adr/
    0001-record-architecture-decisions.md
    NNNN-<slug>.md
```

> HF 自带的 9 份原则文档（soul / skill-anatomy / sdd-artifact-layout / coding-principles / methodology-coherence / emergent-vs-upfront-patterns / architectural-health-during-tdd / hf-sdd-tdd-skill-design / skill-node-define）随 skills 包发布于 `skills/principles/`，**不**强制宿主项目复制到 `docs/principles/`。宿主项目根 `docs/principles/` 仅承载项目自身的设计/产品原则。
...
- `skills/principles/sdd-artifact-layout.md` 是工件管理约定本身，随 HF skills 包发布；`AGENTS.md` 中所有路径覆盖与 skill 中"HF 自带原则锚点"的引用都落到 `skills/principles/`。宿主项目 `docs/principles/` 仅承载**项目自身**的设计/产品原则（例如 `docs/principles/architecture-principles.md`），未启用时由 soul + sdd-artifact-layout 兜底。
...
- `docs/principles/` 是 `hf-design` 中"**项目自身**设计原则锚点"的默认落点（如 `architecture-principles.md` / `product-principles.md`）；HF 自带的 9 份原则文档（soul / skill-anatomy / sdd-artifact-layout / coding-principles / methodology-coherence / emergent-vs-upfront-patterns / architectural-health-during-tdd / hf-sdd-tdd-skill-design / skill-node-define）随 skills 包发布于 `skills/principles/`，不要求宿主项目复制到 `docs/principles/`。`AGENTS.md` 中可写明 `design principles path: docs/principles/`，未启用时由 `skills/principles/` + 项目级 README/AGENTS.md 兜底。
...
- 没有 HF skills 包提供的 `skills/principles/sdd-artifact-layout.md` 锚点，或宿主项目缺 `docs/adr/` 就声称已采用本约定（前者由 skills 包自带，后者属档 0 最小集，任何档位下都不可省）。
...
- [ ] HF skills 包已安装（含 `skills/principles/sdd-artifact-layout.md` 等 9 份原则锚点）
