---
name: hf-to-tickets
description: 把已评审的规格和架构拆成带阻塞边的垂直切片任务票。HarnessFlow 主链步骤；默认写入 features/<id>/tickets.md。无强制 gate。
---
...
1. 宜在规格与特性架构评审通过后拆票；可选跑 `check --to to-tickets` 自检（不拦截）。
2. 默认发布到 `features/<id>/tickets.md`（机器可读的 `- [ ] T-NN` 行）；若已配置任务跟踪器，则按其配置发布，并在特性目录保留索引副本便于 `status` 探测。
...
可选择在 `tickets/` 下为每张任务票创建详情文件；**`status`/`check` 只解析 `tickets.md` 中的 `- [ ] T-NN` 行**。

**真实任务跟踪器**——每张任务票对应一个议题，并使用跟踪器原生的阻塞关系；保留 `tickets.md` 镜像便于进度探测。
