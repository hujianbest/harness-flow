...
读取：已批准规格（默认 `features/<active>/spec.md`）、feature `progress.md`（默认 `features/<active>/progress.md`）、`AGENTS.md` 路径映射、其中声明的项目级设计原则锚点（默认 `docs/principles/`，承载**项目自身**的 architecture/product principles 等）、当前架构概述（按 *Minimal docs Tiers*：档 1 读 `docs/architecture.md`，档 2 读 `docs/arc42/`；二者择一存在），外加最少必要技术上下文。当 `hf-ui-design` 被激活时，也读取其最新草稿以标记 peer 依赖条目。
