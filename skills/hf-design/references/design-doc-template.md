...
- 章节 4.5（Tactical Model per Bounded Context）在战术触发条件满足时必填（Bounded Context ≥ 2 / 单 Context 多实体 + 一致性约束 / 并发或事务边界 / 领域事件 / 跨聚合不变量）；不触发时显式跳过并说明理由。**GoF 代码模式不写入本章节**（见 `skills/principles/emergent-vs-upfront-patterns.md`）。
...
- 把 GoF 代码模式（Strategy / Factory / Adapter / Observer / Decorator / Builder / Singleton 等）作为前置决策列入本章节——这类模式属于实现层 emergent 浮现，见 `skills/principles/emergent-vs-upfront-patterns.md`
