...
description: 适用于设计草稿已完成需要正式 review verdict、或被指定为 reviewer subagent 执行设计评审的场景。不适用于需继续写或修设计、需拆任务或编码、阶段不清或证据冲突。
---
...
**职责边界**：本 skill 只评审 **架构 / 模块 / API 契约 / 数据模型 / 后端 NFR**。若规格声明 UI surface 且 UI 设计 被激活，UI 评审 会作为 **design stage 的并行 review peer** 独立评审 IA / 交互 / 视觉 / 前端 a11y 等 UI 维度。**两条 review 均通过后，父会话才发起联合 `设计真人确认`**；任一未过，对应起草 skill 回修，另一条可继续其稳定部分。不得跨权评审 UI 设计 的职责范围；发现 peer 交接块不一致时，只在 findings 中标注，不代位评审。
...
- 设计阶段 已完成设计草稿，需要正式 review verdict
...
前提条件：存在当前设计草稿、已批准规格、项目相关约定（若项目已声明）。缺设计草稿 → 设计阶段；缺已批准规格或阶段不清 → 上游编排者编排者。
...
读取并固定证据来源：已批准规格（默认 `features/<active>/spec.md`）、当前设计文档（默认 `features/<active>/design.md`）、项目约定、feature `progress.md`（默认 `features/<active>/progress.md`）状态、必要技术上下文。
...
- route 明确但缺稳定设计草稿 → 写最小 blocked record，下一步 设计阶段
- precheck 通过 → 继续正式审查
...
- `需修改` → 设计阶段
- `阻塞`（设计内容） → 设计阶段
- `阻塞`（需求漂移/规格冲突） → 上游编排者编排者（`reroute_via_orchestrator=true`）
...
- 忽略缺失的接口或模块边界
- 忽略无法追溯到已批准规格的新增设计内容
- 设计评审刚"通过"就直接进入 任务规划（跳过 approval step）
- 文档长度长就认为设计充分
...
| "verdict pass，回头实现时再发现问题再说。" | Verification: design verdict 是后续 任务规划 / 实现层 TDD 的 precondition；草率 pass 会回流大量返工。 |
