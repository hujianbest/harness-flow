...
- 关联:
  - 运行时共享约定（progress schema / verdict 词表 / record_path 语义等）: `skills/hf-workflow-router/references/workflow-shared-conventions.md`
...
它不替代 `skills/hf-workflow-router/references/workflow-shared-conventions.md` 里的 progress schema、verdict 词表、record_path 语义等运行时约束；它只回答"工件物理布局与生命周期"这一层。

它也不引入隐藏目录（如 `.sdd/`）、frontmatter schema 或 CI 校验。约束完全靠：

- 模板（每个 skill 自身的 `references/` 目录，例如 `skills/hf-specify/references/feature-readme-template.md`）
...
- `docs/principles/sdd-artifact-layout.md` 是工件管理约定本身，项目级路径覆盖与 skill 中"项目级原则锚点"的引用都落到这里。
...
详细 skill 适配规则见 `skills/hf-workflow-router/references/workflow-shared-conventions.md` 的 *长期资产同步规则（promotion rules）* 与各 skill `SKILL.md`。
...
- `docs/principles/` 是 `hf-design` 中"项目级设计原则锚点"的默认落点。项目可在自己的 guidelines / 宿主 sidecar 中声明 `design principles path: docs/principles/`。
...
- **closeout pack 文件名为 `closeout.md`**，不再叫 `finalize-closeout-pack.md`。模板内容以 `skills/hf-finalize/references/finalize-closeout-pack-template.md` 为准。
...
1. **模板就是最强的强制力**。每个 skill 在自己的 `references/` 目录下提供该阶段的模板（spec、design、tasks、review、verification、closeout pack 等），保持完整与最新；agent 与人创建工件时复制模板，骨架自然一致。本约定**不**把模板抽到仓库级 `templates/` 或 `docs/templates/`，模板就近随 skill 分发，项目可在自己的项目级约定中声明等价覆盖路径（沿用 HF 现有覆盖语义）。
...
`skills/hf-workflow-router/references/workflow-shared-conventions.md` 中的 *Default 逻辑工件布局* 表格描述的是 HF 出厂默认路径。本约定通过项目级约定声明覆盖，使其按下表落地：
...
| 项目原则锚点 | （未指定，由项目级约定声明） | `docs/principles/` |
...
```text
[ 新 feature 启动 ]
  └─> 在 features/NNN-<slug>/ 下创建 README.md + spec.md（来自 hf-specify / hf-tasks 等 skill 各自 references/ 内的模板）
       │
       ▼
...
- [ ] 项目级约定（例如 `AGENTS.md` 或等价 sidecar）已声明本约定要求的路径覆盖
...
- [ ] 已启用的 `docs/arc42/` / `docs/diagrams/` / `docs/runbooks/` / `docs/slo/` / `docs/postmortems/` / `docs/release-notes/` / `docs/insights/` / `docs/index.md` 各自的同步语义在 closeout 中体现
