# mdc-workflow (mwf) Skill Anatomy

- 定位: 定义 `mdc-workflow`（简称 `mwf`）中 `mwf-*` skill 的目标态写法；对 workflow skill 来说，它是 `01 skill-node-define.md` 在 `SKILL.md` 层的落地 anatomy。
- 关联:
...
本文定义 mwf skills 的写作标准，让 `mwf-*` skills 能被 opencode agent 稳定触发、正确执行、留下证据，并能在团队日常开发链路中被恢复编排。

mwf skill 不是一次性提示词，也不是团队流程制度的散文复述。它是一个运行时 contract：告诉 agent 何时加载、处理什么对象、采用什么方法、按什么 todo list 做、产出什么记录、何时必须停止。

## 核心原则

1. **Skill 是团队工程纪律的可执行化。** 团队已有模板、流程、代码规范和验证要求不能只停留在制度里，必须被 skill 转成可执行步骤和证据要求。
2. **mwf 不替团队角色拍板。** 模块架构师、开发负责人和开发人员的判断权不能被 skill 偷偷接管。
...
mwf workflow skill 通常是 `Technique + Pattern`。当节点依赖团队模板、测试有效性 rubric、代码检视 rubric 时，再引入 `Reference`。

## mwf Node Roles

| 角色 | 代表 skill | 写作重心 |
|---|---|---|
| Public Entry | `using-mwf-workflow` | 判断 direct invoke 还是 route-first |
| Router | `mwf-workflow-router` | 恢复当前 AR / 问题修改状态，决定下一节点 |
| Specify | `mwf-specify` | 澄清明确输入、IR/SR/AR 追溯、待决问题 |
| Spec Review | `mwf-spec-review` | 规格清晰性、可追溯性、可设计性审查 |
| Component Design | `mwf-component-design` | 组件实现设计、SOA 边界、组件接口和依赖 |
| AR Design | `mwf-ar-design` | AR 代码层设计、测试设计、与组件设计一致性 |
| Implementation | `mwf-tdd-implementation` | 基于 AR 测试设计做 C / C++ TDD |
| Test Checker | `mwf-test-checker` | TDD 后测试用例有效性、覆盖性、可维护性审查 |
| Code Review | `mwf-code-review` | C / C++ 质量、SOA 边界、嵌入式风险 |
| Gate | `mwf-completion-gate` | evidence bundle 与完成判断 |
| Finalize | `mwf-finalize` | 状态收口、交接、长期记录同步 |
...
- Boundaries: do not revise component implementation design unless explicitly routed to `mwf-component-design`.
...
- Test Design Before Implementation: define test cases in the AR implementation design before `mwf-tdd-implementation`.
```
...
- TDD 完成后未经过 `mwf-test-checker` 却进入代码检视。
- `mwf-test-checker` 试图补写测试或修改生产代码。
...
| 组件设计和 AR 设计边界不清 | 开发人员可能越权改组件架构 | 明确 reroute 到 `mwf-component-design` |
| TDD skill 自己补测试设计 | 跳过 AR 设计职责 | 缺测试设计时回 `mwf-ar-design` |
| TDD 后不做测试有效性审查 | 测试可能只是跑通但覆盖不足 | 路由到 `mwf-test-checker` |
| `mwf-test-checker` 补写测试或改生产代码 | reviewer 角色越权 | 返回 findings，让实现节点回修 |
