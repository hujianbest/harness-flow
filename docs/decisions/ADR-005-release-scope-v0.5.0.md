...
- 关联文档：
  ...
  - `skills/hf-finalize/scripts/render-closeout-html.py`（本版**唯一**新增脚本——把 closeout pack 渲染为视觉伴生 HTML；按 D10 引入的 skill-owned 工具约定，与 hf-finalize 同 vendor）
...
本 ADR 一次性锁定 v0.5.0 范围的 10 项决策。

## 决策

### Decision 1 — `hf-finalize` 新增"closeout HTML 工作总结报告"必出制品

`hf-finalize` 的 workflow 在 step 6（产出 closeout pack）之后**必须**额外执行 step 6A：调用 `skills/hf-finalize/scripts/render-closeout-html.py <feature-dir>`（脚本物理位置见 D10）渲染 `features/<active>/closeout.html`，作为 `closeout.md` 的视觉伴生制品。Hard Gates / Verification / Output Contract / Red Flags 同步加入这一条。
...
### Decision 2 — 渲染脚本以 `skills/hf-finalize/scripts/render-closeout-html.py` 落地，仅依赖 Python stdlib

新增 `skills/hf-finalize/scripts/render-closeout-html.py`（物理位置约定见 D10——skill-owned 工具放 skill 目录下，确保 vendoring 路径完整）：
...
### Decision 10 — HF skill anatomy 扩展为 4 类子目录；引入 `skills/<name>/scripts/` 约定

HF skill anatomy 自 v0.2.0 起约定的 3 类子目录（`SKILL.md` + `references/` + `evals/`）在 v0.5.0 扩展为 **4 类**，新增 `skills/<name>/scripts/` 作为 **skill-owned 工具** 子目录：

- `SKILL.md`（必出）—— skill 行为契约
- `references/`（按需）—— 模板 / 参考材料
- `evals/`（按需）—— eval 用例
- `scripts/`（按需，**v0.5.0 新增**）—— **本 skill** 专属的工具型脚本（典型场景：被本 skill hard gate 调用且不被其它 skill 复用的 stdlib-only 渲染 / 校验 / 转换脚本）
...
- `skills/hf-finalize/SKILL.md` step 6A / Hard Gates / Reference Guide / Verification 中的脚本路径全部更新
- `skills/hf-finalize/references/finalize-closeout-pack-template.md` 中的脚本路径全部更新
- `scripts/audit-skill-anatomy.py` 顶部 docstring 加段说明新约定（行为不变——只读 `SKILL.md`，不遍历子目录）

理由：

- **vendoring 修正**：HF 三种集成路径里有两种（OpenCode `.opencode/skills/` 软链接、Cursor `.cursor/rules/` + `skills/` 复制；CONTRIBUTING.md 的 "vendor by copying" 路径同理）**只链接 / 复制 `skills/` 目录**，不带 `scripts/`。把 `render-closeout-html.py` 留在仓库根 `scripts/` 会导致这两条路径下 hf-finalize step 6A 的 hard gate **跑不通**——脚本不在 vendoring 树里。把脚本搬进 `skills/hf-finalize/scripts/` 一次性修干净。
- **逻辑耦合反映物理位置**：`render-closeout-html.py` 由 hf-finalize step 6A **唯一调用**，**唯一**消费 hf-finalize 产出的 closeout pack；与 hf-finalize 是 1:1 强耦合，物理放一起反映逻辑放一起。
- **维护者工具与用户工具区分**：`audit-skill-anatomy.py` 是维护者跨 skill 工具（受众是 HF 的 contributor），用户侧不需要跑；`render-closeout-html.py` 是用户工具（受众是每一个用 hf-finalize 的项目）。两者按"工具受众"分目录是更稳定的语义边界——v0.4.0 把它们放在同一个 `scripts/` 目录是 v0.5.0 起草时的判断失误，本 D10 修正之。
- **不破坏 audit-skill-anatomy.py**：审计脚本只读 `<skill>/SKILL.md`、不遍历子目录；新加的 `skills/*/scripts/` 子目录对它完全透明；不需要给 audit 加白名单。仅在 audit 顶部 docstring 加文档说明新约定。
- **v0.5.0 内修复，不留 v0.5.1 patch**：v0.5.0 在 ADR-005 起草时尚未打 tag；与其先发 v0.5.0 再发 v0.5.1 修 vendoring 漏洞，不如在同一版内修干净（"发版前修，不发版后 patch"工程立场）。
...
R1（已完成）：`skills/hf-finalize/scripts/render-closeout-html.py` + `skills/hf-finalize/scripts/test_render_closeout_html.py` 落地（按 D10 skill-owned 工具约定）；17 单元测试全绿；`audit-skill-anatomy.py` + `test_audit_skill_anatomy.py` 不退化。
