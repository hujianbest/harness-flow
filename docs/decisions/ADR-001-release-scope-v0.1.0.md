...
- 状态：已接受（2026-04-28，第二轮 review 关闭 D4/D9a 子项；2026-04-29 增补 D10 Q1=B）
- 决策人：用户（架构师角色）
...
需要在 `docs/principles/skill-anatomy.md` 的"主文件骨架"表中把 `Common Rationalizations` 从"默认不建议扩散"改为"workflow skill 推荐"，以保持 anatomy 一致。（PR #21 合入后路径已从 `02 skill-anatomy.md` 重命名为 `skill-anatomy.md`；2026-04-29 PR #20 已落地此项 anatomy 修订，本条不再阻塞。）
...
### Decision 10 — Object Contract 在 v0.1.0 的强制级别（Q1=B，2026-04-29 锁定）
...
- 同步修改 `docs/principles/skill-anatomy.md`：主骨架表标注 v0.1.0 推荐 / v0.2.0 必需，并加 v0.1.0 → v0.2.0 演进说明。
- 同步修改 audit 脚本：`Object Contract` 从 hard fail 降为 P2；`Common Rationalizations` 升为 v0.1.0 release gate（hard fail 之外的发版门禁）。
- v0.1.0 anatomy hard checks 通过率从 0/24 提升到 24/24；v0.1.0 release gate 通过率仍为 0/24（因 D8 的 CR 全量补尚未实施，这是预期）。

理由：
- 选 **A（严格执行）** 会让 24 个 SKILL.md 在 v0.1.0 前都要补一段 contract 写作，工作量大且与"P-Honest 窄而硬"的发版策略冲突。
- 选 **C（核心 7 节点）** 需要再花一轮决策列出 7 节点清单，并且 anatomy 文档与 SKILL.md 之间维持长期不一致。
- 选 **B** 在标准层（anatomy 文档）做一次性放宽，后续 v0.2.0 再统一升级；标准与实现保持同步，迭代成本最低。

可逆性：高（`skill-anatomy.md` 表格条目修改与 audit 脚本分级即可回滚）。
...
| 10 Object Contract 强制级别 | 容易（anatomy 表格条目 + audit 脚本分级） |
...
> - D10 Q1 Object Contract 强制级别（2026-04-29 锁定为 B：v0.1.0 推荐 / v0.2.0 必需）
