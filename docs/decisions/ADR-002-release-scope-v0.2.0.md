...
- 状态：起草中（2026-05-06 锁定 10 项决策；2026-05-07 D11 校准撤回 D2/D3/D4/D8，由"太重 + 不完善"退回 v0.3+）
- 决策人：用户（架构师角色）
...
### Decision 2 — 客户端扩展到 7 家（**Superseded by Decision 11，2026-05-07**）

> **状态：撤回，由 D11 取代**。原文保留作为决策追溯：
...
### Decision 3 — 真实环境安装冒烟作为 v0.2.0 GA 硬门禁（**Superseded by Decision 11，2026-05-07**）
...
### Decision 4 — Agent personas 层（3 个）：作为 review skill 的 orchestration shortcut，不替代（**Superseded by Decision 11，2026-05-07**）
...
- 启用 SemVer 不变。
- v0.2.0 在 GitHub Release 仍勾选 "Set as a pre-release"，理由：Tier 1 只覆盖 1/7 ops，仍未达到 GA 承诺面（无 security / performance / shipping / debugging / deprecation 等节点）；且 v0.2.0 仅做工程纪律硬化（anatomy enforcement + audit + browser-testing），不扩展对外承诺面。
- README Scope Note 更新为：「v0.2.0 pre-release。客户端仍为 Claude Code + OpenCode 两家（D11 撤回了 7 家扩展）；主链终点仍是 `hf-finalize`；`hf-browser-testing` 是 verify 阶段的 runtime evidence 节点，不是 ship 节点。」
...
### Decision 8 — Persona 命名空间与目录拓扑（**Superseded by Decision 11，2026-05-07**）
...
### Decision 11 — v0.2.0 范围收窄：撤回 D2/D3/D4/D8（2026-05-07 锁定）

#### 校准触发

实现阶段 R3（5 家客户端 setup + Gemini CLI 命令 + README 同步）、R4（3 个 personas）、R5（install smoke 骨架）已落地后，用户在 review 时给出反馈：

> "R3 R4 R5 这部分不需要，不然显得太重的，而且不完善"

两个信号：

- **太重**：v0.2.0 同时做 (a) anatomy 强制基线、(b) Tier 1 新 skill、(c) 5 家客户端、(d) 3 personas、(e) GA 冒烟硬门禁，对单维护者节奏过载，且对外承诺面突然铺开太大，与 ADR-001 D1 / ADR-002 D1 的 "P-Honest，窄而硬" 哲学冲突。
- **不完善**：3 家无 slash 命令客户端的 setup 文档质量、personas 与 review skill 的边界、smoke 报告的真实环境验证路径，都需要更长的真实使用反馈才能稳定，强行进 v0.2.0 会以"半成品对外承诺面"的形式发版，违反 soul.md 「不让步声明」。

#### 决策

1. **撤回 D2**（5 家新客户端扩展）：v0.2.0 仍仅支持 Claude Code + OpenCode 两家（与 v0.1.0 一致）。Cursor / Gemini CLI / Windsurf / GitHub Copilot / Kiro 退回 v0.3+ roadmap，待真实用户反馈成熟后再批量进。
2. **撤回 D3**（真实环境安装冒烟硬门禁）：v0.2.0 不在 GA 时强制冒烟流程；客户端面只有 2 家时冒烟价值有限（与 v0.1.x stabilization "Known Limitations" 已知缺口一致），不增设新硬门禁。GA 验证回到"维护者按 v0.1.0 同样路径自验"。
3. **撤回 D4**（3 个 user-facing personas）：v0.2.0 不引入 personas 层。Review 角色仍由各自 `hf-*-review` skill 承担（与 v0.1.0 一致）。`agents/` 目录不在 v0.2.0 引入，命名空间保留给 v0.3+。
4. **撤回 D8**（Persona 命名空间约定）：D4 撤回后 D8 自然失去意义，一并撤回。
5. **保留 D1 / D7（hf-browser-testing）**：单 verify-stage skill 工作面可控，与 anatomy 基线硬化协同，留在 v0.2.0。
6. **保留 D5（audit script）+ D9（SKILL.md 强制结构变更）+ D10（写入 anatomy.md）**：这三条是 SKILL.md 长期质量基线的内核，**v0.2.0 真正的"窄而硬"硬化点**就是这一组。
7. **保留 D6（版本号 pre-release）**：版本号策略不变；Scope Note 文案按 D11 收窄后的承诺面更新。
8. **`docs/principles/persona-anatomy.md` 同步删除**：v0.2.0 不引入 persona 层，对应的 design reference 文档无落地价值，回到 v0.3+ 与 personas 一起重新评估。

#### 实施细则（已落到 PR #32 reverts）

- `git revert 560ac26 0c93809 18b1d99` 三个 implementation commit，移除 R3 / R4 / R5 全部产物：
  - 5 份新客户端 setup 文档（`docs/{cursor,gemini-cli,windsurf,copilot,kiro}-setup.md`）
  - 6 个 Gemini CLI 命令文件（`.gemini/commands/*.md`）
  - 4 份客户端集成 sidecar（`.cursor/rules/harness-flow.mdc`、`.windsurf/rules.md`、`.github/copilot-instructions.md`、`.kiro/skills` 符号链接）
  - 3 个 personas（`agents/hf-staff-reviewer.md` / `hf-qa-engineer.md` / `hf-security-auditor.md`）+ `agents/README.md`
  - install smoke 骨架（`docs/audits/v0.2.0-install-smoke.md`）
  - README / README.zh-CN Scope Note + Quick Start + Installation 表 7 客户端化的修改
  - `.claude-plugin/marketplace.json` 描述中 23 hf-* 的措辞（暂时回到 22 hf-* + hf-browser-testing 的 v0.2.0 语境，避免在 marketplace 上"承诺扩客户端"）
- 同步删除 `docs/principles/persona-anatomy.md`（D8 撤回后该文档无引用方）。

#### 后果

正面：

- v0.2.0 critical path 真正聚焦"质量纪律内核"：anatomy 基线 + audit + Common Rationalizations 强制 + 单 verify-stage skill。
- 与 ADR-001 D1 / D11 的 P-Honest 精神一致：少承诺面积优先于堆 skill 数量。
- 5 家客户端 setup 与 3 个 personas 的"对外承诺质量"被推迟到 v0.3+，由真实使用反馈驱动设计，而不是先发版再修。

负面：

- v0.2.0 与 AS 0.6.0 的客户端覆盖差距未在本版本缩小（仍 2 家 vs 7 家）。
- 已写好的 5 份 setup 文档 / 3 份 persona 文件在 git 历史中可回读（commit `18b1d99` / `0c93809` / `560ac26`），但不进 v0.2.0 GA；下次复用需要从 revert 之前的工作树捞出。
- v0.2.0 比 v0.1.0 多出的新公开承诺面只有 `hf-browser-testing`，对外叙事相对单薄。

中性：

- `docs/principles/persona-anatomy.md` 删除是对称行为；与 ADR-001 D11 删除 audit 脚本相同思路（决策撤回则其物质载体一并撤回）。

#### 可逆性

高。任意时点可重新引入 R3 / R4 / R5 全部内容（git 历史已包含完整版本），ADR 中也只需补一份 D12 把 D2/D3/D4/D8 重新激活即可。
...
| D2 客户端扩展到 7 家 | **Superseded by D11** |
| D3 冒烟硬门禁 | **Superseded by D11** |
| D4 Agent personas | **Superseded by D11** |
| D5 audit baseline | 容易（脚本独立，可降级或移除） |
| D6 版本号 | 容易（SemVer 自然演进） |
| D7 hf-browser-testing 边界 | 容易（边界改动只触及单个 skill 的 SKILL.md） |
| D8 Persona 命名空间 | **Superseded by D11** |
...
| D11 v0.2.0 范围收窄 | 高（任意时点可重新激活 D2/D3/D4/D8；git 历史保留完整 R3/R4/R5 实现） |
...
本 ADR 在 D11（2026-05-07）校准后，v0.2.0 实现阶段收窄为以下 4 步（R3 / R4 / R5 由 D11 撤回）：

- **R0（基线落地）**：更新 `docs/principles/skill-anatomy.md`（D10），新增 `scripts/audit-skill-anatomy.py`（D5）。**已落地**：commit `8e74bb1`。`docs/principles/persona-anatomy.md` 由 D11 撤回，不进 v0.2.0。
- **R1（SKILL.md 全量结构修订）**：23 份现有 SKILL.md 执行 D9 变更（删 `和其他 Skill 的区别` + 补 `Common Rationalizations`）；audit script 跑过。**已落地**：commits `08cd8dd` + `e4ecfb1`。
- **R2（Tier 1）**：新增 `hf-browser-testing/SKILL.md` + `references/`（按 D9 / D10 新基线写作）；接入 `hf-test-driven-dev` 与 `hf-workflow-router` 的拐点。**已落地**：commit `03f5ff1`。
- **~~R3 / R4 / R5~~**：由 D11 撤回，退回 v0.3+ roadmap。
- **R6（发版）**：CHANGELOG 切到 `[0.2.0]`、tag `v0.2.0`、GitHub Release 勾选 pre-release、Release notes 引用 ADR-002（含 D11 校准）；不再附冒烟报告。

## v0.3+ Roadmap（由 D11 退回）

下列条目从 v0.2.0 退回 v0.3+，待真实使用反馈成熟后重新评估：

| 条目 | 来源 | 状态 |
|---|---|---|
| Cursor setup + `.cursor/rules/` 引用 | ADR-002 D2（撤回） | v0.3+ |
| Gemini CLI setup + 6 commands（`/planning` 改名） | ADR-002 D2（撤回） | v0.3+ |
| Windsurf setup + `.windsurf/rules.md` | ADR-002 D2（撤回） | v0.3+ |
| GitHub Copilot setup + `.github/copilot-instructions.md` 块 | ADR-002 D2（撤回） | v0.3+ |
| Kiro setup + `.kiro/skills` 拓扑 | ADR-002 D2（撤回） | v0.3+ |
| `hf-staff-reviewer` / `hf-qa-engineer` / `hf-security-auditor` personas | ADR-002 D4 / D8（撤回） | v0.3+ |
| `docs/principles/persona-anatomy.md` | ADR-002 D8（撤回） | v0.3+ |
| 真实环境 install smoke 硬门禁 | ADR-002 D3（撤回） | v0.3+（视客户端面铺开节奏） |
| 6 项剩余 ops/release skill（shipping / ci-cd / security / performance / debugging / deprecation） | ADR-001 D1 + ADR-002 D1 | v0.3+ |
