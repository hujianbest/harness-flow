...
## [0.6.0] - 2026-05-10 — pre-release

> **Sixth public release.** Marked as a **pre-release** on GitHub Releases.
>
> v0.6.0 is a **structural-refactor** minor release that introduces **`agents/hf-orchestrator.md`** as HarnessFlow's always-on agent persona, replacing the previously-separate `using-hf-workflow` (entry shell) + `hf-workflow-router` (runtime authority) skills. This locks the **HF three-layer architecture invariant** per [`docs/decisions/ADR-007-orchestrator-extraction-and-skill-decoupling.md`](docs/decisions/ADR-007-orchestrator-extraction-and-skill-decoupling.md) D1: **Doer Skills** (12) / **Reviewer-Gate Skills** (11) / **Orchestrator Agent** (1).
>
> v0.6.0 does **not** grow the leaf skill set (still 24 `hf-*`), does **not** add slash commands (still 7), does **not** change closeout pack schema / reviewer return verdict vocabulary / `hf-release` standalone behavior, and does **not** modify any leaf skill content. ADR-007 D3 lays out a 6-step landing path; v0.6.0 is **only Step 1** (build orchestrator + 3-host always-on stubs + deprecate-alias old skills). Steps 2–6 (leaf skill `Next Action` field downgrade / Hard Gate stratification / cross-skill reference removal / physical deletion of old skills) are deferred to future increments per ADR-007 D3 (suggested v0.7.0+).
>
> Walking-skeleton regression PASS (NFR-005 / FR-003); 3-host always-on smoke gate PASS-by-construction with Cursor directly verified + Claude Code / OpenCode deferred-manual per spec § 3 Instrumentation Debt; NFR-001 wall-clock load delta is -33% (orchestrator main 14,067 bytes vs baseline 21,132 bytes, well under the × 1.20 threshold; NFR-002 character budget likewise GREEN at × 0.666).
>
> This is HF's **fourth `hf-release` dogfood** (after v0.4.0 / v0.5.0 / v0.5.1).

### Added

- **`agents/hf-orchestrator.md`** — HarnessFlow always-on agent persona, auto-loaded by every supported host. Merges `using-hf-workflow` (entry shell) + `hf-workflow-router` (runtime authority) content as a semantic-equivalent rewrite. Includes Operating Loop (10 steps), Hard Stops, Workflow Profile / Execution Mode / Workspace Isolation decision entries, FSM transition map entry, reviewer dispatch protocol entry, full Skill Catalog (24 hf-* + 11 deprecated alias references), Output Contract, Red Flags, Common Rationalizations, Verification. Identity gate anchor: "**I am the HF Orchestrator** (我是 HF Orchestrator)" on line 9. Per spec NFR-002, character budget = 14,067 bytes (66% of baseline; well under × 1.10 threshold). (ADR-007 D1 / D2)
- **`agents/references/`** — 9 reference files physically migrated from `skills/hf-workflow-router/references/` via `git mv` (history preserved): `execution-semantics.md` / `profile-node-and-transition-map.md` / `profile-selection-guide.md` / `review-dispatch-protocol.md` / `reviewer-return-contract.md` / `routing-evidence-examples.md` / `routing-evidence-guide.md` / `ui-surface-activation.md` / `workflow-shared-conventions.md`. Progressive disclosure: orchestrator main file references these by path; reviewers and downstream skills load them on demand. (ADR-007 D2)
- **`CLAUDE.md`** — Repository-root always-load file for Claude Code with `## HF Orchestrator (always on)` section + `Read agents/hf-orchestrator.md and adopt that persona` directive. (spec FR-002.b / D-Host-CC)
- **`AGENTS.md`** — Repository-root always-load file for OpenCode (and any cross-tool `AGENTS.md`-consuming agent) with the same orchestrator stub structure. New file; v0.5.x repository did not have an `AGENTS.md`. (spec FR-002.c / D-Host-OC)
- **`docs/decisions/ADR-007-orchestrator-extraction-and-skill-decoupling.md`** — v0.6.0 architectural ADR; 7 locked decisions (D1 three-layer invariant / D2 single-source `agents/` + `agents/references/` migration / D3 6-step landing path / D4 v0.6.x deprecated alias preservation / D5 release-blocking hypothesis list / D6 v0.7+ ops/release skills must follow new invariant / D7 specialist personas remain deferred). Full relationship table to ADR-001 through ADR-006 verified consistent.
- **`features/001-orchestrator-extraction/`** — HarnessFlow's first coding-family feature directory (prior `features/*` entries were release-tier). Contains spec.md, design.md, tasks.md, progress.md, README.md, plus reviews/ + approvals/ + verification/ + scripts/ subdirectories. Walking-skeleton regression script + tests under `scripts/`; 3-host smoke + load-timing + walking-skeleton regression records under `verification/`.
- **`features/001-orchestrator-extraction/scripts/regression-diff.py`** — Stdlib-only Python 3 walking-skeleton diff script with hard-coded allowlist (ISO date / 24h time / generator-script path migration text / HTML render-time comment). 3 test cases via companion `test_regression_diff.py`: self-consistency / mutation outside allowlist / allowlist timestamp — all GREEN. (spec NFR-005 / FR-003 / D-RegrLoc / D-RegrImpl)

### Changed

- **`skills/using-hf-workflow/SKILL.md`** — Original 179-line skill replaced. **Plugin-install loading channel** (revised v0.6.0 pre-tag, see ADR-007 D1 Amendment): `description` restored to HF-entry semantic so Claude Code / OpenCode skill-discovery mechanism triggers on HF intent (new session / `/hf-*` command / continue / review-gate completed / route unclear). Body redirects to load `agents/hf-orchestrator.md` and adopt that persona for the session. This file is a thin loader, not a competing source of truth — `agents/hf-orchestrator.md` remains the canonical single source. HTML loader marker on line 1. (ADR-007 D1 Amendment + D2 / D4; spec FR-004 / D-Stub revised)
- **`skills/hf-workflow-router/SKILL.md`** — Same treatment as `using-hf-workflow/SKILL.md` (replaced 180 lines with ≤30-line redirect stub).
- **`skills/hf-workflow-router/references/*.md`** — All 9 files replaced with ≤10-line redirect stubs pointing at the new `agents/references/<same-name>.md` location. HTML deprecation markers on line 1. Original content was migrated via `git mv` so git history is preserved on the new paths.
- **`.cursor/rules/harness-flow.mdc`** — Body content updated: "On every session" steps now say "Load the orchestrator agent persona: `agents/hf-orchestrator.md`. Act as that persona for the rest of the session" (replacing the previous "Load entry shell + router" pair). Hard rules updated to reference the orchestrator instead of the old entry shell + router. Scope honesty section updated with the v0.6.0 three-layer invariant introduction. Frontmatter unchanged (`alwaysApply: true`).
- **`README.md` / `README.zh-CN.md`** — Top "Scope Note" section retitled "Scope Note (v0.6.0 pre-release)"; describes the v0.6.0 minor scope (orchestrator extraction + 3-host always-on stubs + deprecated alias preservation) and explicitly lists the 6-step landing path scope boundaries (Step 1 = v0.6.0; Steps 2–6 deferred). Updated cross-version links.
- **`docs/cursor-setup.md` / `docs/claude-code-setup.md` / `docs/opencode-setup.md`** — Top scope note updated for v0.6.0; "How HF is loaded" sections updated to point at `agents/hf-orchestrator.md` instead of the old entry shell + router pair. Claude Code setup additionally describes the new `agents` field in `.claude-plugin/plugin.json` + `CLAUDE.md` dual-track always-load path. OpenCode setup adds the new `AGENTS.md` always-load file.
- **`.claude-plugin/plugin.json`** — `version` from `0.5.1` to `0.6.0`. `description` updated to mention the v0.6.0 orchestrator. **Schema fallback engaged (per spec C-005)**: Claude Code's current plugin schema rejected the `agents` array field (`agents: Invalid input` validation error during `/plugin install`); the field has been removed and the orchestrator auto-loads via `CLAUDE.md` instead — the dual-track design from spec D-Host-CC anticipated this case. The `description` now notes the fallback explicitly. JSON validates with `python3 -m json.tool`.
- **`.claude-plugin/marketplace.json`** — `description` updated to describe the v0.6.0 orchestrator-extraction structural refactor. (No version field; marketplace tracks `plugin.json`.)
- **`SECURITY.md`** — Supported Versions table: v0.5.x latest changed from `0.5.1` to `0.6.0` and a new `0.6.x` row added.
- **`CONTRIBUTING.md`** — Introduction version bump v0.5.1 → v0.6.0.
- **`features/001-orchestrator-extraction/`** scaffolding artifacts (README / progress / spec / design / tasks / reviews / approvals / verification) reflect the full hf-product-discovery → hf-discovery-review → hf-specify → hf-spec-review (Round 1+2) → hf-design → hf-design-review → hf-tasks → hf-tasks-review (Round 1+2) → hf-test-driven-dev (T1–T9) chain.

### Decided

- **ADR-007 D1 — HF three-layer architecture invariant introduced.** v0.6.0 architectural commitment; v0.7.0+ runtime enforcement (D3 Steps 2–5 strip leaf cross-references); v0.8.0+ physical deletion of old skill files (D3 Step 6). New leaf skills introduced from v0.6.0 onward must follow the invariant.
- **ADR-007 D2 — `agents/` + `agents/references/` are a new agent-persona directory category**, distinct from skill anatomy (ADR-006 D1 4-subdir `SKILL.md` + `references/` + `evals/` + `scripts/`). `audit-skill-anatomy.py` is transparent to `agents/` (does not scan it). A future `audit-agent-anatomy.py` will be decided by a separate ADR if needed.
- **ADR-007 D3 — 6-step landing path locked.** v0.6.0 = Step 1 only. Steps 2–6 deferred to future increments; physical deletion of `using-hf-workflow` / `hf-workflow-router` scheduled for v0.8.0+ to preserve external tutorial-link grace per ADR-001 D1 "narrow but hard".
- **ADR-007 D4 — `using-hf-workflow` / `hf-workflow-router` preserved as deprecated alias** in v0.6.x via redirect stubs (ADR-001 D1 stance preserved).
- **ADR-007 D5 — HYP-002 + HYP-003 declared release-blocking** for v0.6.0. Both validated in this release: walking-skeleton regression PASS (HYP-002); Cursor identity gate directly verified + Claude Code / OpenCode PASS-by-construction with deferred-manual smoke (HYP-003 with explicit deferred-manual checklist for release pre-flight).
- **ADR-007 D6 — v0.7+ ops/release skills must follow new three-layer invariant** when introduced. The 5 deferred skills (`hf-shipping-and-launch` / `hf-ci-cd-and-automation` / `hf-security-hardening` / `hf-performance-gate` / `hf-deprecation-and-migration` / `hf-debugging-and-error-recovery`) inherit this constraint.
- **ADR-007 D7 — specialist personas remain deferred.** v0.6.0 `agents/` directory contains only `hf-orchestrator.md`; the 3 deferred specialist personas (`hf-staff-reviewer` / `hf-qa-engineer` / `hf-security-auditor`) stay on the v0.7+ roadmap.
- **HF's fourth `hf-release` dogfood** (first minor release on top of v0.5.x patch lineage). Validates that `hf-release` / release-pack-template / pre-release-engineering-checklist scale to "structural refactor with release-blocking hypothesis verification" use-case.

### Notes

- **Migration**: external consumers reading `skills/using-hf-workflow/SKILL.md` or `skills/hf-workflow-router/SKILL.md` (or any of the 9 router references) will see a redirect stub directing them to `agents/hf-orchestrator.md` (or `agents/references/<same-name>.md`). All old paths remain readable through v0.6.x; physical removal scheduled for v0.8.0+ (ADR-007 D3 Step 6). If your project uses `Next Action Or Recommended Skill: hf-workflow-router` in custom workflows, the value remains accepted as a synonym for `hf-orchestrator` during v0.6.x; new code should write `hf-orchestrator`.
- **Plugin re-install**: Claude Code plugin users running `/plugin update` will pick up the v0.6.0 release. **Loading channel reality** (per ADR-007 D1 Amendment, v0.6.0 pre-tag): plugin-install hosts (Claude Code / OpenCode) load the orchestrator persona via Claude Code's skill-discovery mechanism activating `skills/using-hf-workflow/SKILL.md` (which then reads + adopts `agents/hf-orchestrator.md`). The `agents` array path in `.claude-plugin/plugin.json` was removed (schema rejected it; spec C-005 fallback). Direct-clone hosts (Cursor) continue to load `agents/hf-orchestrator.md` directly via `.cursor/rules/harness-flow.mdc`. `agents/hf-orchestrator.md` remains the canonical single source of truth. If your install predates the marketplace upload, run `/plugin marketplace update` first.
- **No leaf skill change**: this release intentionally does not touch any of the 24 `hf-*` skill files (other than `using-hf-workflow` and `hf-workflow-router` redirect stubs). Existing leaves continue to write `Next Action Or Recommended Skill: hf-<skill>` and to assume `features/<active>/` artifacts; the orchestrator consumes these as compat-period hints (ADR-007 D1 architectural commitment phase). Pure leaf decoupling (Steps 2–5) is a future increment.
- **Closeout HTML / `hf-finalize` step 6A** (introduced in v0.5.0) is unchanged. The renderer still lives at `skills/hf-finalize/scripts/render-closeout-html.py` (per ADR-006 D1 / D2). v0.6.0 closeout output for this very feature is rendered the same way.
- **First HF coding-family `features/<NNN>-<slug>/` entry**. `features/001-orchestrator-extraction/` is the first non-release-tier feature directory in the repository; prior `features/release-v0.5.0/` and `features/release-v0.5.1/` entries are release-pack-only.
