...
- **`.claude-plugin/plugin.json`** — `version` from `0.5.1` to `0.6.0`. `description` updated to mention the v0.6.0 orchestrator. New `agents` array registers `hf-orchestrator` with `alwaysActive: true` + `source: "agents/hf-orchestrator.md"`. (Schema fallback: if Claude Code's plugin schema rejects `agents` / `alwaysActive`, the orchestrator still auto-loads via `CLAUDE.md`.) JSON validates with `python3 -m json.tool`.
...
- **Plugin re-install**: Claude Code plugin users running `/plugin update` should pick up the new `agents` registration automatically. If your install predates the marketplace upload, run `/plugin marketplace update` first.
