...
- **`skills/using-hf-workflow/SKILL.md`** — Replaced full content (179 lines) with a ≤30-line redirect stub. Frontmatter `description` now reads "deprecated alias, see agents/hf-orchestrator.md (HF v0.6.0+)". HTML deprecation marker `<!-- HF v0.6.0 deprecated alias: see agents/hf-orchestrator.md -->` on line 1. Body provides deprecation notice + redirect pointer + compat-window scheduling (v0.7.0+ physical deletion per ADR-007 D3 Step 6). NFR-003 (Compatibility / Co-existence): old tutorial links remain readable, no 404. (ADR-007 D4 / spec FR-004 / D-Stub / D-Stub-Marker)
...
- **Plugin re-install**: Claude Code plugin users running `/plugin update` will pick up the v0.6.0 release. The orchestrator persona is loaded via `CLAUDE.md` always-load (the `agents` array path was removed in pre-tag fix because Claude Code's current plugin schema does not accept it; spec C-005 fallback). If your install predates the marketplace upload, run `/plugin marketplace update` first.
